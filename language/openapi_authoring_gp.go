...
func copyOpenAPIRequired(input *bool) *bool {
	if input == nil {
		return nil
	}
	copy := *input
...
func copyOpenAPIBody(input *OpenAPIBodyDeclaration) *OpenAPIBodyDeclaration {
	...
	copy := *input
	copy.FieldPath = append([]string(nil), input.FieldPath...)
	copy.Required = copyOpenAPIRequired(input.Required)
	return &copy
}
func copyOpenAPI(input *OpenAPIDeclaration) *OpenAPIDeclaration {
	...
	for i := range copy.Operations {
		...
		for j := range operation.Request.Parameters {
			item := &operation.Request.Parameters[j]
			item.FieldPath = append([]string(nil), item.FieldPath...)
			item.Required = copyOpenAPIRequired(item.Required)
		}
