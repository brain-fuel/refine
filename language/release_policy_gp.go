...
type BreakingFixApproval struct {
	ReleaseComparison
...
func releasePolicyError(message string) error {
	return fmt.Errorf("language.release_policy: %s", message)
}
...
func releasePolicyString(node schemajson.Node) (string, error) {
	text, ok := node.Text()
	if !ok {
		return "", releasePolicyError("expected a string")
	}
	result, err := text.UTF8()
	if err != nil {
		return "", releasePolicyError("metadata strings must contain Unicode scalar values")
	}
	return result, nil
}
func releasePolicyObject(node schemajson.Node, allowed []string) (map[string]schemajson.Node, error) {
	if schemajson.KindName(node.Kind()) != "object" {
		return nil, releasePolicyError("expected an object")
	}
	fields := map[string]schemajson.Node{}
	for _, member := range node.Members() {
		key, err := member.Key.UTF8()
		if err != nil {
			return nil, releasePolicyError("invalid object key")
		}
		known := false
		...
		if !known {
			return nil, releasePolicyError("unknown field " + key)
		}
...
func releasePolicyRecord(node schemajson.Node, explanation string) (ReleaseComparison, string, error) {
	...
	for _, key := range []string{"baseline", "baselineSha256", "snapshotSha256", "direction", explanation} {
		node, ok := fields[key]
		if !ok {
			return comparison, "", releasePolicyError("missing field " + key)
		}
		text, err := releasePolicyString(node)
		if err != nil {
			return comparison, "", err
		}
		values[key] = text
	...
	if !releasePolicyVersion(comparison.Baseline) {
		return comparison, "", releasePolicyError("baseline must be canonical x.y.z")
	}
	if !releasePolicyDigest(comparison.BaselineSHA256) || !releasePolicyDigest(comparison.SnapshotSHA256) {
		return comparison, "", releasePolicyError("comparison digests must be lowercase SHA-256")
	}
	if comparison.Direction != "backward" && comparison.Direction != "forward" {
		return comparison, "", releasePolicyError("direction must be backward or forward")
	}
	text := values[explanation]
	if strings.TrimSpace(text) == "" || len(text) > releasePolicyExplanationBytes {
		return comparison, "", releasePolicyError("explanation must be nonblank and at most 8192 UTF-8 bytes")
	}
...
func ParseReleasePolicyJSON(input []byte) (*ReleasePolicy, error) {
	...
	if err != nil {
		return nil, releasePolicyError(err.Error())
	}
	...
	if !ok || schemajson.KindName(version.Kind()) != "number" || version.Raw() != "1" {
		return nil, releasePolicyError("version must be 1")
	}
	policy := &ReleasePolicy{Version: 1}
	count := 0
	for _, key := range []string{"overrides", "breakingFixes"} {
		entries, exists := fields[key]
		if !exists {
			continue
		}
		if schemajson.KindName(entries.Kind()) != "array" {
			return nil, releasePolicyError(key + " must be an array")
		}
		if entries.ElementCount() > releasePolicyRecords-count {
			return nil, releasePolicyError("at most 4096 approval records are allowed")
		}
		count += entries.ElementCount()
		seen := map[string]bool{}
		explanation := "reason"
		if key == "breakingFixes" {
			explanation = "justification"
		}
		for _, node := range entries.Elements() {
			comparison, text, err := releasePolicyRecord(node, explanation)
			if err != nil {
				return nil, err
			}
			...
			if seen[identity] {
				return nil, releasePolicyError("duplicate approval record")
			}
			seen[identity] = true
			if key == "overrides" {
				policy.Overrides = append(policy.Overrides, CompatibilityApproval{ReleaseComparison: comparison, Reason: text})
			} else {
				policy.BreakingFixes = append(policy.BreakingFixes, BreakingFixApproval{ReleaseComparison: comparison, Justification: text})
			}
		}
	}
	return policy, nil
...
func FormatReleasePolicyJSON(policy *ReleasePolicy) ([]byte, error) {
	if policy == nil {
		return nil, releasePolicyError("policy must not be nil")
	}
	if len(policy.Overrides) > releasePolicyRecords || len(policy.BreakingFixes) > releasePolicyRecords-len(policy.Overrides) {
		return nil, releasePolicyError("at most 4096 approval records are allowed")
	}
	// Preflight authored strings before JSON escaping allocates its output.
	bytes := 0
	add := func(text string) bool {
		if !utf8.ValidString(text) || len(text) > releasePolicyBytes-bytes {
			return false
		}
		bytes += len(text)
		return true
	...
	for _, item := range policy.Overrides {
		if !add(item.Baseline) || !add(item.BaselineSHA256) || !add(item.SnapshotSHA256) || !add(item.Direction) || !add(item.Reason) {
			return nil, releasePolicyError("metadata exceeds 1 MiB")
		}
	}
	for _, item := range policy.BreakingFixes {
		if !add(item.Baseline) || !add(item.BaselineSHA256) || !add(item.SnapshotSHA256) || !add(item.Direction) || !add(item.Justification) {
			return nil, releasePolicyError("metadata exceeds 1 MiB")
		}
	}
	...
	if _, err := ParseReleasePolicyJSON(input); err != nil {
		return nil, err
	}
	return input, nil
}
...
func splitReleasePolicyTokens(source string, tokens []token) (string, *ReleasePolicy, bool, []token, error) {
	...
	for i := 0; i+1 < len(tokens); i++ {
		if tokens[i].kind == "@" && tokens[i+1].kind == "name" && tokens[i+1].text == "releasePolicy" {
			if index >= 0 {
				return "", nil, false, nil, releasePolicyError("duplicate footer")
			}
			index = i
		}
	}
	if index < 0 {
		return source, nil, false, tokens, nil
	}
	...
	decoded, err := value.ReadText(raw)
	if err != nil {
		return "", nil, false, nil, releasePolicyError("invalid footer literal")
	}
	text, err := decoded.UTF8()
	if err != nil {
		return "", nil, false, nil, releasePolicyError("footer must contain Unicode scalar values")
	}
	policy, err := ParseReleasePolicyJSON([]byte(text))
	if err != nil {
	...
	if index == 0 || tokens[index-1].kind != "newline" || tokens[index-1].at.Start.Offset != at.Offset-1 {
		return "", nil, false, nil, releasePolicyError("footer requires a leading LF")
	}
...
func AppendReleasePolicyFooter(base string, policy *ReleasePolicy) (string, error) {
	if _, _, present, err := SplitReleasePolicyFooter(base); err != nil {
		return "", err
	} else if present {
		return "", releasePolicyError("source already has a release-policy footer")
	}
	input, err := FormatReleasePolicyJSON(policy)
	if err != nil {
		return "", err
	}
	result := base + "\n@releasePolicy " + quote(string(input)) + "\n"
	if len(result) > 16<<20 {
		return "", releasePolicyError("source exceeds 16 MiB")
	}
	return result, nil
}
