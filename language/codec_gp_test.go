// Code generated by goplus from codec_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func TestTypedReadExecution(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.name == "nominal refined failure" {
				declarations += "accepts :: Positive -> Bool\naccepts _ = True\n"
			}
			e, result, failure := execution(t, declarations+"entry :: "+tc.typ+"\nentry = "+tc.expression, validation.Limits{})
			if failure != nil {
				t.Fatal(failure)
			}
			if shown := e.show(result, Span{}); shown != tc.want {
				t.Fatalf("got %s, want %s", shown, tc.want)
			}
		})
	}
}
...
func TestReadNeverExecutesExpressions(t *testing.T) {
	...
	for _, input := range []string{"explode", "1 + 2", "if True then 1 else explode", "let x = 1 in x", "{a = 1}.a", "1/0", "1/-2", "-- only a comment", "[1,]", "{a = 1, a = 1}"} {
		...
		if validation.StateName(report.State()) != "invalid" {
			t.Fatalf("%q: %+v", input, report.Diagnostics())
		}
	}
}
...
`)
	for _, tc := range []struct {
		...
		input string
		state string
	}{{"Secret", `"private-sensitive-text"`, "invalid"}, {"Positive", "-1", "invalid"}, {"Positive", "2", "valid"}, {"Unknown", "1", "indeterminate"}, {"Both", "1", "invalid"}, {"Positive", "0.5", "invalid"}} {
		text, _ := value.TextFromUTF8(tc.input)
		candidate, report := p.ReadData(tc.root, text, validation.Limits{})
		if validation.StateName(report.State()) != tc.state {
			t.Fatalf("%s: %+v", tc.root, report.Diagnostics())
		}
		if tc.state != "valid" {
			name, ok := candidate.Constructor()
			if !ok || name != "Null" {
				t.Fatal("invalid candidate escaped")
			}
		}
		for _, detail := range report.Diagnostics() {
			if strings.Contains(detail.Message, "private-sensitive") {
				t.Fatal("read error leaked payload")
			}
		}
	}
	e, _, failure := execution(t, "type T = Int where 1 / 0 > 0.0\nentry :: Result String T\nentry = read \"1\"", validation.Limits{})
	if failure == nil || failure.code != "read.indeterminate" || e.depth != 0 {
		t.Fatalf("unknown was accepted/caught as malformed input: %v", failure)
	}
	source := "type T = Int where True @steps 1000000\nentry :: Result String T\nentry = read \"1\""
	_, _, failure = execution(t, source, validation.Limits{Clause: 10})
	if failure == nil {
		t.Fatal("nested read reset caller budget")
	}
	...
	if validation.StateName(report.State()) != "indeterminate" {
		t.Fatal("numeric expansion escaped resources")
	}
}
...
`)
	...
	if report := p.ValidateData("IntegerBox", integer, validation.Limits{}); validation.StateName(report.State()) != "valid" {
		t.Fatalf("%+v", report.Diagnostics())
	}
	if report := p.ValidateData("BooleanBox", boolean, validation.Limits{}); validation.StateName(report.State()) != "invalid" {
		t.Fatalf("%+v", report.Diagnostics())
	}
}
...
func TestReadShowProperties(t *testing.T) {
	...
	property := func(n int64, den uint16, units []uint16, items []int16, choice uint8) bool {
		rational, err := value.Integer(n).Divide(value.Integer(int64(den) + 1))
		if err != nil {
			return false
		}
		list := make([]value.Data, len(items))
		for i, item := range items {
			list[i] = integerPayload(int64(item))
		}
		var optional value.Data
		switch choice % 3 {
		case 0:
			optional, _ = value.Variant("Nothing", nil)
		case 1:
			optional, _ = value.Variant("Just", []value.Data{value.Data{}})
		case 2:
			present, _ := value.Variant("NonNull", []value.Data{value.OfNumber(rational)})
			optional, _ = value.Variant("Just", []value.Data{present})
		}
		...
		shown, err := ShowDataWithoutValidation(raw, validation.Limits{})
		if err != nil {
			return false
		}
...
func TestReadRejectsUninhabitableCodecsAndChecksAnnotationRules(t *testing.T) {
	for _, source := range []string{
		...
		if _, err := Compile(source); err == nil {
			t.Fatalf("invalid codec/type annotation accepted: %s", source)
		}
	}
}
...
func TestShowRejectsUnrepresentableNamesAndInvalidBypassRead(t *testing.T) {
	for _, name := range []string{"bad-name", "where", "a.b"} {
		...
		if _, err := ShowDataWithoutValidation(raw, validation.Limits{}); err == nil {
			t.Fatalf("unrepresentable field %s shown", name)
		}
	}
	for _, name := range []string{"lowercase", "True", "Bad Name"} {
		raw, _ := value.Variant(name, nil)
		if _, err := ShowDataWithoutValidation(raw, validation.Limits{}); err == nil {
			t.Fatalf("unrepresentable constructor %s shown", name)
		}
	}
	p := validationProgram(t, "type Positive = Int where it > 0")
	shown, err := ShowDataWithoutValidation(integerPayload(-1), validation.Limits{})
	if err != nil {
		t.Fatal(err)
	}
	_, report := p.ReadData("Positive", shown, validation.Limits{})
	if validation.StateName(report.State()) != "invalid" {
		t.Fatal("bypass-created invalid value accepted by validating read")
	}
}

func TestCanonicalValueConformanceFixtures(t *testing.T) {
	input, err := os.ReadFile("testdata/canonical-values.json")
	if err != nil {
		t.Fatal(err)
	}
	var cases []struct {
	...
	for _, tc := range cases {
		t.Run(tc.Name, func(t *testing.T) {
			p := validationProgram(t, tc.Schema)
			text, _ := value.TextFromUTF8(tc.Input)
			...
			actual, err := shown.UTF8()
			if err != nil || actual != tc.Canonical {
				t.Fatalf("got %q, want %q", actual, tc.Canonical)
			}
			reread, report := p.ReadData("T", shown, validation.Limits{})
			equal, err := data.EqualWith(reread, func(uint64) error { return nil })
			if err != nil || !equal || validation.StateName(report.State()) != "valid" {
				t.Fatalf("canonical round trip: %+v", report.Diagnostics())
			}
		})
	}
}
...
func FuzzTypedReadShow(f *testing.F) {
	for _, seed := range []string{"[]", "[1,-2,3]", "[1/3]", "(Just 1)", "[1 + 2]", "[1e99999999]"} {
		f.Add(seed)
	}
	p, err := Compile("type T = [Real]")
	if err != nil {
		f.Fatal(err)
	}
	f.Fuzz(func(t *testing.T, input string) {
		...
		text, err := value.TextFromUTF8(input)
		if err != nil {
			return
		}
		...
		equal, err := data.EqualWith(reread, func(uint64) error { return nil })
		if err != nil || !equal || validation.StateName(again.State()) != "valid" {
			t.Fatal("valid read/show round trip failed")
		}
	})
}
