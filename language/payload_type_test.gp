...
func FuzzPayloadType(f *testing.F){
    for _,source:=range []string{"Box Age","Tree (Int where it > 0)","[Maybe Age]","Map String (Maybe Age)","JSON","Map String JSON","JSON where it /= JSONNull","{value :: Int}","Int where positive it","Int -> Int","Box a"}{f.Add(source)}
