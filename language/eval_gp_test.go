// Code generated by goplus from eval_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func execution(t *testing.T, source string, limits validation.Limits) (*evaluator, evalValue, *evalFailure) {
	...
	result, failure := e.attempt(&Expr{Form: Variable{Name: "entry"}}, nil)
...
func TestEvaluatorExplainedFailures(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if strings.Contains(failure.Error(), "secret-payload") {
				t.Fatal("payload leaked into failure")
			}
			if e.depth != 0 {
				t.Fatalf("evaluation stack not unwound: %d", e.depth)
			}
		})
	}
}
	...
	for _, mode := range []string{"satisfiesAll", "satisfiesOnlyOneOf", "satisfiesOneOf", "satisfiesAtLeastOneOf"} {
		for size := 0; size <= 4; size++ {
			...
			for encoded := 0; encoded < count; encoded++ {
				selected := []string{}
				yes, no, unknown, n := 0, 0, 0, encoded
				for i := 0; i < size; i++ {
					digit := n % 3
					n /= 3
					selected = append(selected, names[digit])
					switch digit {
					case 0:
						no++
					case 1:
				...
				if want == "unknown" {
					if failure == nil || failure.code != "evaluation.divide" {
						t.Fatalf("%s: %v", source, failure)
					}
				} else {
					if failure != nil || e.show(result, Span{}) != want {
						t.Fatalf("%s: %v; want %s", source, failure, want)
					}
				}
			}
		}
...
func TestEvaluatorCollectionUnknowns(t *testing.T) {
	...
	for _, tc := range []struct {
		expr string
		want string
	}{{"any predicate [0, 1]", "True"}, {"all predicate [0, -1]", "False"}, {"all predicate []", "True"}, {"any predicate []", "False"}} {
...
func TestEvaluatorDeterministicCostAndThreshold(t *testing.T) {
	...
	if used := e.meter.Used(); used != 14 {
		t.Fatalf("step accounting changed: %d", used)
	}
	n, _ := number(result, Span{})
	if n.Show() != "46" {
		t.Fatal("wrong sum")
	}
	for cap := uint64(1); cap <= 20; cap++ {
		_, _, failure := execution(t, source, validation.Limits{Clause: cap})
		if (failure == nil) != (cap >= 14) {
			t.Fatalf("cap %d: %v", cap, failure)
		}
	}
}
...
func TestEvaluatorMapFoldLaws(t *testing.T) {
	...
	property := func(input []int16) bool {
		...
		items := make([]evalValue, len(input))
		want := int64(0)
		for i, n := range input {
			items[i] = numberValue(value.Integer(int64(n)), "Int")
			want += int64(n) * 2
		}
		original := evalValue{form: evalList{items: items}}
		...
		n, _ := number(sum, Span{})
		if n.Show() != fmt.Sprint(want) {
			return false
		}
		for i, item := range items {
			n, _ := number(item, Span{})
			if n.Show() != fmt.Sprint(input[i]) {
				return false
			}
		}
		return true
...
func TestEvaluatorDataTransferAndFixedWidth(t *testing.T) {
	...
	null := value.Data{}
	optional, _ := value.Variant("Just", []value.Data{null})
	...
	same, err := raw.EqualWith(converted, func(uint64) error { return nil })
	if err != nil || !same {
		t.Fatal("payload changed during transfer")
	}
	expr := &Expr{Form: Binary{Operator: "+", Left: &Expr{Form: Variable{Name: "a"}}, Right: &Expr{Form: Variable{Name: "b"}}}}
	env := map[string]evalValue{"a": numberValue(value.Integer(127), "Int8"), "b": numberValue(value.Integer(1), "Int8")}
	_, failure := e.attempt(expr, env)
	if failure == nil || failure.code != "evaluation.overflow" {
		t.Fatalf("overflow: %v", failure)
	}
	n, _ := number(env["a"], Span{})
	if n.Show() != "127" {
		t.Fatal("overflow modified operand")
	}
}

func FuzzEvaluatorDeterminism(f *testing.F) {
	for _, seed := range []string{"1 + 2", "False && 1 / 0 > 0.0", "length \"x\"", "[1, 2]", "1e999999999999999999999"} {
		f.Add(seed)
	}
	f.Fuzz(func(t *testing.T, expr string) {
		...
		evaluate := func() (string, string, uint64) {
			...
			result, failure := e.attempt(&Expr{Form: Variable{Name: "entry"}}, nil)
			if failure != nil {
				return "", failure.Error(), e.meter.Used()
			}
			text := textOf(result, Span{})
			return text.Show(), "", e.meter.Used()
		}
		a, ea, ca := evaluate()
		b, eb, cb := evaluate()
		if a != b || ea != eb || ca != cb {
			t.Fatal("nondeterministic result or cost")
		}
	})
}
