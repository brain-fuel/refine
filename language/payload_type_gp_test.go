...
func TestCheckedPayloadTypes(t *testing.T) {
	...
	for _, tc := range []struct{ typ, text, state string }{
		...
		t.Run(tc.typ+"/"+tc.text, func(t *testing.T) {
			target, err := program.PayloadType(tc.typ)
			if err != nil {
				t.Fatal(err)
			}
			text, _ := value.TextFromUTF8(tc.text)
			data, report := target.ReadData(text, validation.Limits{})
			if validation.StateName(report.State()) != tc.state {
				t.Fatalf("%s: %+v", tc.typ, report.Diagnostics())
			}
			if tc.state == "valid" {
				if got := target.ValidateData(data, validation.Limits{}); validation.StateName(got.State()) != "valid" {
				...
				_, again := target.ReadData(shown, validation.Limits{})
				if validation.StateName(again.State()) != "valid" {
					t.Fatal(again.Diagnostics())
				}
			} else if name, ok := data.Constructor(); !ok || name != "Null" {
				t.Fatal("failed read exposed a candidate")
			}
			again, err := program.PayloadType(target.Formatted())
			if err != nil || again.Formatted() != target.Formatted() {
				t.Fatalf("unstable format: %v", err)
			}
		})
	}
}
...
func TestPayloadTypeRejectionAndIsolation(t *testing.T) {
	...
	for _, source := range []string{"", "a", "Box", "Box a", "Int Age", "Box Int String", "Unknown", "Int -> Int", "FunctionAlias", "HiddenFunction", "Tree FunctionAlias", "Phantom (Int -> Int)", "Int where it", "Int where missing it", "Int where it > 0 @message 1", "Int\ntype Injected = String", "Int; import \"network\"", "Int where (case Nothing of { Just x -> True })", strings.Repeat("[", 520) + "Int" + strings.Repeat("]", 520)} {
		if target, err := program.PayloadType(source); err == nil || target != nil {
			t.Fatalf("unchecked target accepted: %q, %v", source, err)
		}
	}
	if target, err := (*Program)(nil).PayloadType("Int"); err == nil || target != nil {
		t.Fatal("nil program accepted")
	}
	text, _ := value.TextFromUTF8("1")
	for _, target := range []*PayloadType{nil, {}} {
		if validation.StateName(target.ValidateData(value.OfNumber(value.Integer(1)), validation.Limits{}).State()) != "invalid" {
			t.Fatal("zero target validated")
		}
		if _, report := target.ReadData(text, validation.Limits{}); validation.StateName(report.State()) != "invalid" {
	...
	target.Syntax().Form = NamedType{Name: "String"}
	snapshot := target.CheckedSyntax()
	snapshot.Type.Form = NamedType{Name: "String"}
	snapshot.Module.Syntax.Types[0].Name = "Changed"
	clear(snapshot.Module.Inferred)
	if target.Source() != "Box Age" || program.Source() != payloadTypeContract {
		t.Fatal("snapshot changed source")
	}
	text, _ = value.TextFromUTF8("{value = -1}")
	if _, report := target.ReadData(text, validation.Limits{}); validation.StateName(report.State()) != "invalid" {
		t.Fatal("snapshot changed validation")
	}
	...
	otherTarget, err := other.PayloadType("Box Age")
	if err != nil {
		t.Fatal(err)
	}
	if _, report := otherTarget.ReadData(text, validation.Limits{}); validation.StateName(report.State()) != "valid" {
	...
	for i := 0; i < 8; i++ {
		...
		go func() {
			...
			for j := 0; j < 100; j++ {
				if _, report := target.ReadData(text, validation.Limits{}); validation.StateName(report.State()) != "invalid" {
					t.Error("concurrent target changed")
				}
			}
		}()
	}
...
func TestPayloadTypeNamedBudgetParity(t *testing.T) {
	program := validationProgram(t, payloadTypeContract)
	target, err := program.PayloadType("Age")
	if err != nil {
		t.Fatal(err)
	}
	same := func(a, b validation.Report) {
		left, _ := json.Marshal(a)
		right, _ := json.Marshal(b)
		if string(left) != string(right) {
			t.Fatalf("target budget mismatch\n%s\n%s", left, right)
		}
	}
	for _, n := range []int64{-1, 0, 1} {
		for steps := uint64(1); steps < 300; steps++ {
			limits := validation.Limits{Total: steps, Clause: steps}
			data := value.OfNumber(value.Integer(n))
			text, _ := value.TextFromUTF8(value.Integer(n).Show())
			...
			_, a := target.ReadData(text, limits)
			_, b := program.ReadData("Age", text, limits)
			same(a, b)
		}
	}
	if err := quick.Check(func(n int64) bool {
		data := value.OfNumber(value.Integer(n))
		normal := target.ValidateData(data, validation.Limits{})
		bypass := target.ValidateDataWithoutRefinements(data, validation.Limits{})
...
func FuzzPayloadType(f *testing.F) {
	for _, source := range []string{"Box Age", "Tree (Int where it > 0)", "[Maybe Age]", "Map String (Maybe Age)", "JSON", "Map String JSON", "JSON where it /= JSONNull", "{value :: Int}", "Int where positive it", "Int -> Int", "Box a"} {
		f.Add(source)
	}
	...
	f.Fuzz(func(t *testing.T, source string) {
		...
		target, err := program.PayloadType(source)
		if err != nil {
			if target != nil {
				t.Fatal("partial target escaped")
			}
			return
		}
		formatted := target.Formatted()
		again, err := program.PayloadType(formatted)
		if err != nil || again.Formatted() != formatted {
			t.Fatalf("target round trip: %v", err)
		}
		data := value.OfNumber(value.Integer(1))
		limit := validation.Limits{Total: 500, Clause: 100}
		left, _ := json.Marshal(target.ValidateData(data, limit))
		right, _ := json.Marshal(target.ValidateData(data, limit))
		if string(left) != string(right) {
			t.Fatal("nondeterministic target validation")
		}
	})
}
