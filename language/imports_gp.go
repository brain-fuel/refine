...
func (b *SourceBundle) Entry() string {
	if b == nil {
		return ""
	}
	return b.entry
}
func (b *SourceBundle) Program() *Program {
	if b == nil {
		return nil
	}
	return b.program
}
func (b *SourceBundle) Files() []SourceFile {
	if b == nil {
		return nil
	}
	out := append([]SourceFile(nil), b.files...)
	for i := range out {
		out[i].Imports = append([]string(nil), out[i].Imports...)
	}
	return out
}
func (b *SourceBundle) Sources() map[string]string {
	out := map[string]string{}
	if b != nil {
		for _, file := range b.files {
			out[file.ID] = file.Source
		}
	}
	return out
}
...
func sourceID(name string) bool {
	if name == "" || name == "." || !utf8.ValidString(name) || path.IsAbs(name) || path.Clean(name) != name || strings.HasPrefix(name, "../") || strings.ContainsAny(name, "\\:\x00") {
		return false
	}
	for _, r := range name {
...
func CompileSources(entry string, sources map[string]string) (*SourceBundle, error) {
	if !sourceID(entry) {
		return nil, &ImportError{Code: "language.import_path", SourceID: entry, Message: "entry must be a canonical relative source ID"}
	}
	if len(sources) > 10000 {
		return nil, &ImportError{Code: "language.import_limit", SourceID: entry, Message: "at most 10,000 source files may be supplied"}
	}
	r := sourceResolver{sources: make(map[string]string, len(sources)), modules: map[string]*Module{}, state: map[string]uint8{}}
	for id, source := range sources {
		if !sourceID(id) {
			return nil, &ImportError{Code: "language.import_path", SourceID: id, Message: "source IDs must be canonical relative paths"}
		}
		r.sources[id] = source
	}
	...
	for _, file := range r.files {
		...
		bind := func(table map[string]string, name string) error {
			if owner, exists := table[name]; exists {
				return &ImportError{Code: "language.import_collision", SourceID: file.ID, Message: "declaration " + name + " conflicts with " + owner}
			}
			table[name] = file.ID
			return nil
		}
		for _, decl := range module.Types {
			if err := bind(types, decl.Name); err != nil {
				return nil, err
			}
			for _, variant := range decl.Variants {
				if err := bind(terms, variant.Name); err != nil {
					return nil, err
				}
			}
		}
		for _, fn := range module.Functions {
			if err := bind(terms, fn.Name); err != nil {
				return nil, err
			}
		}
		clean := *module
	...
	if err != nil {
		...
		if detail, ok := err.(*Error); ok {
			for _, part := range segments {
				if detail.At.Start.Offset >= part.start && detail.At.Start.Offset < part.end {
					owner = part.id
					break
				}
			}
...
func (r *sourceResolver) visit(id string, depth int) error {
	if depth > 256 {
		return &ImportError{Code: "language.import_limit", SourceID: id, Message: "import nesting exceeds 256"}
	}
	if r.state[id] == 2 {
		return nil
	}
	if r.state[id] == 1 {
		return &ImportError{Code: "language.import_cycle", SourceID: id, Message: "cyclic module imports are not supported; put mutually recursive declarations in one module"}
	}
	source, ok := r.sources[id]
	if !ok {
		return &ImportError{Code: "language.import_missing", SourceID: id, Message: "import is not present in the supplied offline source map"}
	}
	if len(source) > (16<<20)-r.bytes {
		return &ImportError{Code: "language.import_limit", SourceID: id, Message: "reachable source files exceed 16 MiB"}
	}
	r.bytes += len(source)
	module, err := Parse(source)
	if err != nil {
		return &ImportError{Code: "language.import_parse", SourceID: id, Message: err.Error(), Cause: err}
	}
	r.modules[id] = module
	r.state[id] = 1
	imported := []string{}
	for _, entry := range module.Imports {
		...
		if relative == "" || path.IsAbs(relative) || strings.ContainsAny(relative, "\\:\x00") {
			return &ImportError{Code: "language.import_path", SourceID: id, Message: "imports must use relative paths, not URLs or host filesystem paths"}
		}
		resolved := path.Join(path.Dir(id), relative)
		if !sourceID(resolved) {
			return &ImportError{Code: "language.import_path", SourceID: id, Message: "import escapes the supplied source root or has an invalid ID"}
		}
		imported = append(imported, resolved)
		if err := r.visit(resolved, depth+1); err != nil {
			return err
		}
	}
	r.state[id] = 2
