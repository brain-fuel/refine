...
type CheckedPayloadType struct {
...
func (p *Program) PayloadType(source string) (target *PayloadType, failure error) {
	...
	if p == nil || p.module == nil {
		return nil, &Error{Code: "language.type", Message: "a compiled program is required"}
	}
	typ, err := ParseTypeExpression(source)
	if err != nil {
		return nil, err
	}
	...
	module, err := Parse(p.module.Source)
	if err != nil {
...
func (t *PayloadType) Source() string {
	if t == nil {
		return ""
	}
	return t.source
}
func (t *PayloadType) Formatted() string {
	if t == nil || t.typ == nil {
		return ""
	}
	return FormatType(t.typ)
}
func (t *PayloadType) Syntax() *Type {
	if t == nil || t.typ == nil {
		return nil
	}
	typ, err := ParseTypeExpression(t.source)
	if err != nil {
		panic("checked payload type stopped parsing")
	}
	return typ
}
func (t *PayloadType) CheckedSyntax() CheckedPayloadType {
	if t == nil || t.typ == nil {
		return CheckedPayloadType{}
	}
	copy, err := t.program.PayloadType(t.source)
	if err != nil {
		panic("checked payload type stopped compiling")
	}
...
func invalidPayloadType() validation.Report {
	return validation.Collect([]validation.Check{validation.Violated{Detail: validation.Diagnostic{Code: "validation.root", Paths: []string{""}, Message: "Choose a checked payload type."}}})
}
func (t *PayloadType) ValidateData(data value.Data, caller validation.Limits) validation.Report {
	return t.validateData(data, caller, false)
}
func (t *PayloadType) ValidateDataWithoutRefinements(data value.Data, caller validation.Limits) validation.Report {
	return t.validateData(data, caller, true)
}
func (t *PayloadType) validateData(data value.Data, caller validation.Limits, withoutRefinements bool) validation.Report {
	...
	v.structure = newEvaluator(t.program.module, v.budget.BeginStructure())
	v.run(t.typ, data)
