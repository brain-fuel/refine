// Code generated by goplus from capability.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func supportedCapability(name string) bool {
	switch name {
	case "Eq", "Show", "Read", "Num", "Integral", "Ord":
		return true
	}
	return false
}
...
func (c *checker) solveCapabilities() {
	...
	for _, fn := range c.module.Functions {
		...
		byVariable := make(map[string]map[string]Span)
		effective[fn.Name] = byVariable
		for name, term := range variables {
			owners[term.variable] = capabilityOwner{function: fn.Name, variable: name}
		}
		seen := make(map[string]bool)
		for _, constraint := range fn.Constraints {
			if !supportedCapability(constraint.Capability) {
				typeError(constraint.At, "unknown capability "+constraint.Capability+"; expected Eq, Show, Read, Num, Integral, or Ord")
			}
			if _, ok := variables[constraint.Variable]; !ok {
				typeError(constraint.At, "capability variable "+constraint.Variable+" does not occur in the function signature")
			}
			key := constraint.Capability + "\x00" + constraint.Variable
			if seen[key] {
				typeError(constraint.At, "duplicate capability constraint")
			}
			seen[key] = true
	...
	variables := 0
	for _, scope := range c.functionVariables {
		variables += len(scope)
	}
	limit := variables*6 + 1
	for pass := 0; changed && pass < limit; pass++ {
		...
		for _, call := range c.capabilityCalls {
			for variable, classes := range effective[call.callee] {
				instantiated, ok := call.bindings[variable]
				if !ok {
					continue
				}
				for class := range classes {
					if c.requireCapability(class, instantiated, call.at, effective, owners, make(map[string][]*term)) {
						changed = true
					}
				}
			}
	...
	for _, fn := range c.module.Functions {
		...
		for variable, classes := range effective[fn.Name] {
			for class, at := range classes {
				items = append(items, CapabilityConstraint{Capability: class, Variable: variable, At: at})
			}
		}
		sort.Slice(items, func(i, j int) bool {
			if items[i].Variable != items[j].Variable {
				return items[i].Variable < items[j].Variable
			}
			return order[items[i].Capability] < order[items[j].Capability]
		})
...
func addCapability(effective map[string]map[string]map[string]Span, function, variable, class string, at Span) bool {
	variables := effective[function]
	if variables == nil {
		variables = make(map[string]map[string]Span)
		effective[function] = variables
	}
	classes := variables[variable]
	if classes == nil {
		classes = make(map[string]Span)
...
func capabilityFailure(class string, at Span) {
	...
	case "Show":
		typeError(at, "functions do not support canonical display")
	case "Read":
		typeError(at, "functions are not readable payload values")
	case "Num":
		typeError(at, "numeric operations require a numeric type")
	case "Integral":
		typeError(at, "remainder requires an integer type")
	case "Ord":
		typeError(at, "ordering requires a numeric, String, or Timestamp type")
	}
}
...
func (c *checker) requireCapability(class string, t *term, at Span, effective map[string]map[string]map[string]Span, owners map[int]capabilityOwner, visiting map[string][]*term) bool {
	t = c.prune(t)
	if t.variable != 0 {
		if binding, ok := owners[t.variable]; ok {
			return addCapability(effective, binding.function, binding.variable, class, at)
		}
		if t.rigid {
			...
			case "Integral":
				typeError(at, "integer type cannot be inferred; add an annotation")
			case "Ord":
				typeError(at, "ordered type cannot be inferred; add an annotation")
			}
		}
		switch class {
		case "Read":
			typeError(at, "read target type cannot be inferred; add an annotation")
		case "Eq":
			typeError(at, "equality operand type cannot be inferred; add a concrete annotation")
		case "Show":
			typeError(at, "show operand type cannot be inferred; add an annotation")
		case "Num":
			typeError(at, "numeric type cannot be inferred; add an annotation")
		case "Integral":
			typeError(at, "integer type cannot be inferred; add an annotation")
	...
	if primitive(t.name) {
		...
		if class == "Integral" && !integralCapabilityPrimitive(t.name) {
			capabilityFailure(class, at)
		}
		if class == "Ord" && !numericCapabilityPrimitive(t.name) && t.name != "String" && t.name != "Timestamp" {
			capabilityFailure(class, at)
		}
		return false
	}
	...
	if class == "Num" || class == "Integral" || class == "Ord" {
		...
		if !declared || declaration.Body == nil {
			capabilityFailure(class, at)
			return false
		}
		if previous, seen := visiting[t.name]; seen {
			if sameCapabilityTerms(c, previous, t.args) {
				return false
			}
		...
		visiting[t.name] = append([]*term(nil), t.args...)
	...
	case "{}":
		for _, field := range t.fields {
			if c.requireCapability(class, field.typ, at, effective, owners, visiting) {
				changed = true
			}
		}
		return changed
...
func sameCapabilityTerms(c *checker, left, right []*term) bool {
	...
	for i := range left {
		a, b := c.prune(left[i]), c.prune(right[i])
		if a.variable != b.variable || a.name != b.name || len(a.args) != len(b.args) || len(a.fields) != len(b.fields) {
			return false
		}
		if !sameCapabilityTerms(c, a.args, b.args) {
			return false
		}
		for j := range a.fields {
			if a.fields[j].name != b.fields[j].name || !sameCapabilityTerms(c, []*term{a.fields[j].typ}, []*term{b.fields[j].typ}) {
				return false
			}
		}
	}
...
func integralCapabilityPrimitive(name string) bool {
	return name != "Real" && name != "Float32" && name != "Float64" && numericCapabilityPrimitive(name)
}
