// Code generated by goplus from float_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func TestExplicitFiniteFloatConversions(t *testing.T) {
	for _, tc := range []struct{ name, mode, typ string }{{"toFloat32", "exact", "Float32"}, {"toFloat64", "exact", "Float64"}, {"roundToFloat32", "round", "Float32"}, {"roundToFloat64", "round", "Float64"}, {"fromFloat32", "from", "Float32"}, {"fromFloat64", "from", "Float64"}} {
		conversion, ok := FixedFloatConversion(tc.name)
		if !ok || conversion.Mode != tc.mode || conversion.Type != tc.typ {
			t.Fatalf("conversion %s = %+v, %v", tc.name, conversion, ok)
		}
	}
	for _, name := range []string{"toFloat16", "toFloat032", "toFloat32x", "roundFloat32", "wrapFloat32", "fromFloat128", "Float32"} {
		if _, ok := FixedFloatConversion(name); ok {
			t.Fatalf("invalid conversion name accepted: %s", name)
		}
	}
	...
	for _, source := range []string{
		...
		if _, err := Compile(source); err == nil {
			t.Fatalf("implicit or unchecked conversion accepted: %s", source)
		}
	}
	e, result, failure := execution(t, "toFloat32 :: Int -> Int\ntoFloat32 n = n + 1\nentry :: Int\nentry = toFloat32 2", validation.Limits{})
	if failure != nil || e.show(result, Span{}) != "3" {
		t.Fatal("user conversion name did not shadow built-in", failure)
	}
}
...
func TestFiniteFloatPayloadReadShowAndArithmetic(t *testing.T) {
	...
	for _, tc := range []struct{ root, text, state string }{
		...
		t.Run(tc.root+"/"+tc.text, func(t *testing.T) {
			input, _ := value.TextFromUTF8(tc.text)
			data, report := program.ReadData(tc.root, input, validation.Limits{})
			if got := validation.StateName(report.State()); got != tc.state {
				t.Fatalf("got %s: %+v", got, report.Diagnostics())
			}
			if tc.state == "valid" {
				...
				raw, err := shown.UTF8()
				if err != nil || raw != tc.text {
					t.Fatalf("show = %q, %v", raw, err)
				}
			}
			if tc.state == "indeterminate" {
				found := false
				for _, diagnostic := range report.Diagnostics() {
					if strings.Contains(diagnostic.Message, "evaluation.precision") {
						found = true
					}
				}
				if !found {
					t.Fatal("missing exact arithmetic precision diagnostic")
				}
			}
		})
	}
	...
	if e.show(result, Span{}) != "(Ok 1)" {
		t.Fatal("Float32 did not satisfy Num")
	}
	if _, err := Compile("bad :: Integral a => a -> a\nbad n = n % n\nentry :: Result String Float32\nentry = case toFloat32 1.0 of { Ok n -> Ok (bad n); Err e -> Err e }"); err == nil || !strings.Contains(err.Error(), "integer type") {
		t.Fatalf("Float32 Integral accepted: %v", err)
	}
}
