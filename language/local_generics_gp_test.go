...
`
	evaluator, result, failure := execution(t, source, validation.Limits{})
	if failure != nil {
		t.Fatal(failure)
	}
	if actual := evaluator.show(result, Span{}); actual != "[(Ok 4), (Ok 7)]" {
		t.Fatal(actual)
	}
	bad := []string{"copy :: a -> a\ncopy x = let y :: b = x in y", "copy :: a -> a\ncopy x = let y :: a = 1 in y", "first :: a -> a\nfirst x = x\nsecond :: Int -> Int\nsecond x = let y :: a = x in y"}
	for _, text := range bad {
		if _, err := Compile(text); err == nil {
			t.Fatal("undeclared, mismatched, or leaked generic annotation accepted")
		}
	}
}
