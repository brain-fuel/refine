// Code generated by goplus from parser_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func TestExpressionPrecedence(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.source, func(t *testing.T) {
			...
			if actual := FormatExpression(e); actual != tc.formatted {
				t.Errorf("got %s, want %s", actual, tc.formatted)
			}
			again := parsedExpression(t, FormatExpression(e))
			if FormatExpression(again) != tc.formatted {
				t.Fatal("formatter is not stable")
			}
		})
	}
}
...
func TestAgreedContractSyntax(t *testing.T) {
	...
	module, err := Parse(string(data))
	if err != nil {
		t.Fatal(err)
	}
	if module.Package != "com.example.contracts" || len(module.Types) != 8 || len(module.Functions) != 7 {
		t.Fatalf("declaration counts: package=%s, types=%d, functions=%d", module.Package, len(module.Types), len(module.Functions))
	}
	formatted := Format(module)
	again, err := Parse(formatted)
	if err != nil {
		t.Fatalf("formatted module does not parse: %v\n%s", err, formatted)
	}
	...
	case NamedType:
		t.Fatal("missing child refinement")
	case ListType:
		t.Fatal("unexpected child type")
	case AppliedType:
		t.Fatal("unexpected child type")
	case ArrowType:
		t.Fatal("unexpected child type")
	...
	case NamedType:
		t.Fatal("missing percentage refinement")
	case ListType:
		t.Fatal("unexpected percentage type")
	case AppliedType:
		t.Fatal("unexpected percentage type")
	case ArrowType:
		t.Fatal("unexpected percentage type")
...
func TestWhereAnnotationsAndComments(t *testing.T) {
	...
	if !strings.Contains(Format(module), "@steps 123") {
		t.Fatal("budget annotation lost")
	}
	imported, err := Parse("import \"../common.refine\"\n" + source)
	if err != nil || len(imported.Imports) != 1 || imported.Imports[0].Path != "../common.refine" {
		t.Fatal("import not parsed", err)
	}
}
...
func TestMalformedSyntax(t *testing.T) {
	...
	for _, source := range cases {
		...
		if !errors.As(err, &problem) || problem.Code != "language.syntax" {
			t.Errorf("expected syntax error for %q, got %v", source, err)
		}
	}
	_, err := Parse("type X = Int\n\ntype Y = @\n")
	var problem *Error
	if !errors.As(err, &problem) || problem.At.Start.Line != 3 || problem.At.Start.Column != 10 {
		t.Errorf("source position = %v", err)
	}
}
...
func FuzzParseFormat(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, source string) {
		...
		if err != nil {
			t.Fatalf("formatter produced invalid source: %v\n%s", err, formatted)
		}
		if Format(next) != formatted {
			t.Fatal("format/parse is not stable")
		}
	})
}
...
func TestSyntaxResourceLimits(t *testing.T) {
	for _, source := range []string{
		...
		_, err := Parse(source)
		var problem *Error
		if !errors.As(err, &problem) || problem.Code != "language.limit" {
			t.Errorf("missing resource diagnostic: %v", err)
		}
	}
}
