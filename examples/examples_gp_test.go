...
func TestWorkedExamples(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.root, func(t *testing.T) {
			...
			if _, err := explain.Generate(program); err != nil {
				t.Fatal(err)
			}
			for _, sample := range []struct {
				raw  string
				want string
			}{{test.valid, "valid"}, {test.invalid, "invalid"}} {
				text, err := value.TextFromUTF8(sample.raw)
				if err != nil {
					t.Fatal(err)
				}
				_, report := program.ReadData(test.root, text, validation.Limits{})
				if validation.StateName(report.State()) != sample.want {
					t.Fatalf("want %s got %+v", sample.want, report)
				}
			}
		})
	}
...
func TestEvolutionExample(t *testing.T) {
	oldSource, err := os.ReadFile("evolution/v1.0.0.refine")
	if err != nil {
		t.Fatal(err)
	}
	newSource, err := os.ReadFile("evolution/SNAPSHOT.refine")
	if err != nil {
		t.Fatal(err)
	}
	old, err := language.Compile(string(oldSource))
	if err != nil {
		t.Fatal(err)
	}
	next, err := language.Compile(string(newSource))
	if err != nil {
		t.Fatal(err)
	}
	result, err := analysis.Compare(old, "Age", next, "Age", validation.Limits{})
