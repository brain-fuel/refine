// Code generated by goplus from engine.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func (f *Failure) Error() string {
	if f == nil {
		return "ECMA-262 regular expression failure"
	}
	return "ECMA-262 regular expression " + string(f.Kind) + " failure during " + string(f.Phase)
}
...
func failure(kind FailureKind, phase Phase, cause error) error {
	return &Failure{Kind: kind, Phase: phase, cause: cause}
}
...
func DefaultInitLimits() InitLimits {
	return InitLimits{MaxDuration: HardInitializationDuration, MaxPolls: HardInitializationPolls}
}
func checkedInitLimits(input InitLimits) (InitLimits, error) {
	if input.MaxDuration == 0 {
		input.MaxDuration = HardInitializationDuration
	}
	if input.MaxPolls == 0 {
		input.MaxPolls = HardInitializationPolls
	}
	if input.MaxDuration < 0 || input.MaxPolls == 0 {
		return input, errors.New("ECMA-262 initialization limits must be positive")
	}
	if input.MaxDuration > HardInitializationDuration || input.MaxPolls > HardInitializationPolls {
		return input, errors.New("ECMA-262 initialization limits may only tighten hard limits")
	}
	return input, nil
}
...
type Engine struct {
	...
	initialization InitLimits
	idle           chan *guestSession
...
func New(ctx context.Context, limits InitLimits) (*Engine, error) {
	if ctx == nil {
		return nil, errors.New("ECMA-262 engine requires a context")
	}
	if err := ctx.Err(); err != nil {
		return nil, failure(ResourceFailure, InitializationPhase, err)
	}
	checked, err := checkedInitLimits(limits)
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		runtime.Close(initCtx)
		return nil, failure(InternalFailure, InitializationPhase, err)
	}
	compiled, err := runtime.CompileModule(initCtx, artifact)
	if err != nil {
	...
	if err := verifyCompiled(compiled); err != nil {
		runtime.Close(initCtx)
		return nil, failure(InternalFailure, InitializationPhase, err)
	}
...
func verifyArtifact(input []byte) error {
	sum := sha256.Sum256(input)
	actual := hex.EncodeToString(sum[:])
	if actual != ArtifactSHA256 {
		return fmt.Errorf("checked ECMA-262 artifact digest mismatch: %s", actual)
	}
	return nil
}
...
func verifyCompiled(module wazero.CompiledModule) error {
	...
	if len(imports) != 1 {
		return fmt.Errorf("ECMA-262 artifact imports %d functions", len(imports))
	}
	namespace, name, ok := imports[0].Import()
	if !ok || namespace != "refine" || name != "should_interrupt" || !sameTypes(imports[0].ParamTypes(), api.ValueTypeI32) || !sameTypes(imports[0].ResultTypes(), api.ValueTypeI32) {
		return errors.New("ECMA-262 artifact import inventory changed")
	}
	if len(module.ImportedMemories()) != 0 {
		return errors.New("ECMA-262 artifact imports memory")
	}
	...
	if len(functions) != len(expected) {
		return fmt.Errorf("ECMA-262 artifact exports %d functions", len(functions))
	}
	for name, want := range expected {
		actual, ok := functions[name]
		if !ok || !sameTypes(actual.ParamTypes(), want.params...) || !sameTypes(actual.ResultTypes(), want.results...) {
			return fmt.Errorf("ECMA-262 artifact export %s changed", name)
		}
	}
	memories := module.ExportedMemories()
	if len(memories) != 1 {
		return errors.New("ECMA-262 artifact memory inventory changed")
	}
	memory, ok := memories["memory"]
	if !ok {
		return errors.New("ECMA-262 artifact does not export memory")
	}
	maximum, bounded := memory.Max()
	if !bounded || maximum != HardMemoryPages {
		return fmt.Errorf("ECMA-262 artifact memory maximum is %d pages (bounded=%t)", maximum, bounded)
	}
	if memory.Min() > maximum {
		return errors.New("ECMA-262 artifact memory minimum exceeds its maximum")
	}
	return nil
}
...
func sameTypes(actual []api.ValueType, want ...api.ValueType) bool {
...
func interruptHost(ctx context.Context, phase uint32) uint32 {
	request, ok := ctx.Value(requestContextKey{}).(*Request)
	if !ok || request == nil {
		return 1
	}
	return request.shouldInterrupt(phase)
}
