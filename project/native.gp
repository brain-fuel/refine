...
func addNativeResources(files map[string][]byte,base string,p *native.Project,formats []native.Format)error{
    ...
    for _,format:=range formats{for _,mode:=range []native.ExportMode{native.Ordinary,native.Refined}{
        ...
        for i,resource:=range exported.Resources(){name:=path.Join(prefix+"-resources",fmt.Sprintf("%04d.json",i));if resource.URI==exported.Root().Resource{name=prefix+".json"};if err=addFile(files,path.Join(base,name),[]byte(resource.Source));err!=nil{return err};mappings=append(mappings,mappedResource{URI:resource.URI,Path:name})}
        manifest:=struct{Format native.Format `json:"format"`;Version string `json:"version"`;Root native.ResourceSelector `json:"root"`;Metadata native.WireMetadata `json:"metadata"`;Resources []mappedResource `json:"resources"`;NativeConstraintSources []native.Resource `json:"nativeConstraintSources"`}{format,exported.Version(),exported.Root(),exported.Metadata(),mappings,exported.NativeConstraintSources()}
