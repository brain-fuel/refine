...
func Generate(input GenerateInput)(Bundle,error){
    ...
    all:=map[string][]byte{};seen:=map[string]bool{};testClasses:=[]string{};runtimeResources:=javaResourceSet{}
    ...
    for _,contract:=range contracts{
        ...
        if !contract.NoCodegen{
            ...
            if contract.NativeProject!=nil{if err=runtimeResources.addNativeJSON(contract.NativeProject);err!=nil{return Bundle{},err}}
            ...
            if operationsOnly{configured,configureErr:=nativeProjectWithNamespace(contract.NativeProject,namespace);if configureErr!=nil{return Bundle{},configureErr};tests,err=java.GenerateProjectOpenAPIPropertyTests(configured,namespace,class,openAPIOperationsClass,properties)}else{
                ...
                if jsonWire&&contract.NativeProject!=nil{tests,err=java.GenerateProjectPropertyTests(contract.NativeProject,namespace,class,properties)}else{tests,err=java.GeneratePropertyTests(contract.Program,namespace,class,properties)}
    ...
    if len(contracts)>0{if err:=emitDistributionNotices(all,layout.ResourceDir);err!=nil{return Bundle{},err}}
    if err:=runtimeResources.emit(all,layout.ResourceDir);err!=nil{return Bundle{},err}
