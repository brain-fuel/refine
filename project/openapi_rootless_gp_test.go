...
func TestGenerateOperationsOnlyOpenAPIRejectsMissingPropertyAssembly(t *testing.T) {
	...
	for _, disabled := range []bool{false, true} {
		...
		if err == nil || !strings.Contains(err.Error(), "operation-aware generated property tests") || len(output.Files) != 0 {
			t.Fatalf("rootless project assembly silently skipped its missing boundary: no-codegen=%v err=%v files=%d", disabled, err, len(output.Files))
		}
	}
}
