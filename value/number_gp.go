// Code generated by goplus from number.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func ParseNumber(text string) (Number, error) {
	if !numberSyntax.MatchString(text) {
		return Number{}, errors.New("invalid exact number")
	}
...
func (n Number) rat() *big.Rat {
	...
	if !ok {
		panic("corrupt internal exact number")
	}
	return r
}
...
func (n Number) Add(other Number) Number {
	return numberFromRat(new(big.Rat).Add(n.rat(), other.rat()))
}
func (n Number) Subtract(other Number) Number {
	return numberFromRat(new(big.Rat).Sub(n.rat(), other.rat()))
}
...
func (n Number) FixedWidth(bits uint, signed bool) (Number, error) {
	if bits == 0 || bits > 65536 {
		return Number{}, errors.New("integer width must be between 1 and 65536 bits")
	}
	r := n.rat()
	if !r.IsInt() {
		return Number{}, errors.New("expected an integer")
	}
	...
	if r.Num().Cmp(lower) < 0 || r.Num().Cmp(upper) >= 0 {
		return Number{}, errors.New("fixed-width integer overflow")
	}
	return n, nil
}
...
func (n Number) Wrap(bits uint, signed bool) (Number, error) {
	...
	if signed && v.Cmp(new(big.Int).Rsh(new(big.Int).Set(modulus), 1)) >= 0 {
		v.Sub(v, modulus)
	}
...
func (n Number) Decimal() (string, error) {
	...
	a, b := 0, 0
	rem := new(big.Int)
	for rem.Mod(denom, two).Sign() == 0 {
		denom.Quo(denom, two)
		a++
	}
	for rem.Mod(denom, five).Sign() == 0 {
		denom.Quo(denom, five)
		b++
	}
	if denom.Cmp(big.NewInt(1)) != 0 {
		return "", errors.New("number has no exact finite decimal encoding")
	}
	places := max(a, b)
