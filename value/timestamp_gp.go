...
func timestampDigits(s string) (int, bool) {
	n := 0
	for _, c := range s {
		if c < '0' || c > '9' {
			return 0, false
...
func ParseTimestamp(raw string) (Timestamp, error) {
	invalid := func() (Timestamp, error) {
		return Timestamp{}, timestampError("timestamp.syntax", "expected a valid RFC 3339 timestamp")
	}
	if len(raw) < 20 || raw[4] != '-' || raw[7] != '-' || raw[10] != 'T' && raw[10] != 't' || raw[13] != ':' || raw[16] != ':' {
		return invalid()
	}
	year, a := timestampDigits(raw[0:4])
	month, b := timestampDigits(raw[5:7])
	day, c := timestampDigits(raw[8:10])
	hour, d := timestampDigits(raw[11:13])
	minute, f := timestampDigits(raw[14:16])
	second, g := timestampDigits(raw[17:19])
	if !a || !b || !c || !d || !f || !g || month < 1 || month > 12 || day < 1 || day > monthDays(year, month) || hour > 23 || minute > 59 || second > 60 {
		return invalid()
	}
	index := 19
	fraction := Number{}
	if raw[index] == '.' {
		...
		var err error
		fraction, err = ParseNumber("0." + raw[start:index])
		if err != nil {
			return invalid()
		}
	}
	...
	if zone != "Z" && zone != "z" {
		...
		h, ok := timestampDigits(zone[1:3])
		m, ok2 := timestampDigits(zone[4:6])
		if !ok || !ok2 || h > 23 || m > 59 {
			return invalid()
		}
		offset = h*60 + m
	...
	if leap {
		...
		if !leapDates[date] {
			if date > "2026-12-31" {
				return Timestamp{}, timestampError("timestamp.unknown_leap", "leap-second occurrence is outside the pinned IERS table")
			}
...
func (t Timestamp) CivilSecondsUntil(other Timestamp) (Number, error) {
	if t.leap || other.leap {
		return Number{}, timestampError("timestamp.civil_leap", "civil-coordinate subtraction cannot represent a leap-labelled endpoint")
	}
...
func (t Timestamp) SISecondsUntil(other Timestamp) (Number, error) {
	...
	coordinate := func(x Timestamp) Number {
		...
		count := int64(0)
		for boundary := range leapDates {
			if strings.Compare(boundary, date) < 0 {
				count++
			}
		}
		if x.leap {
