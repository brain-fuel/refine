// Code generated by goplus from number_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func TestExactNumberCases(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.input, func(t *testing.T) {
			...
			if n.Show() != tc.show {
				t.Errorf("show = %s; want %s", n.Show(), tc.show)
			}
			...
			if err != nil || decimal != tc.decimal {
				t.Fatalf("decimal = %s, %v; want %s", decimal, err, tc.decimal)
			}
		})
	}
...
func TestExactArithmeticLaws(t *testing.T) {
	property := func(a, b, c int64) bool {
		...
		original := x.Show()
		sum := x.Add(y)
		if sum.Subtract(y).Compare(x) != 0 || sum.Compare(y.Add(x)) != 0 {
			return false
		}
		if x.Multiply(y.Add(z)).Compare(x.Multiply(y).Add(x.Multiply(z))) != 0 {
			return false
		}
		if x.Show() != original {
			return false
		...
		if err != nil || shown.Compare(sum) != 0 {
			return false
		}
		if b != 0 {
			q, err := x.Divide(y)
			if err != nil || q.Multiply(y).Compare(x) != 0 {
				return false
			}
		}
		return true
...
func TestFixedWidthAndExplicitWrapping(t *testing.T) {
	...
	for _, tc := range cases {
		...
		if (err == nil) != tc.valid {
			t.Errorf("%s fits %d signed=%v: %v", tc.input, tc.bits, tc.signed, err)
		}
		w, err := n.Wrap(tc.bits, tc.signed)
		if err != nil || w.Show() != tc.wrapped {
			t.Errorf("wrap %s: %s, %v", tc.input, w.Show(), err)
		}
	}
	if _, err := Integer(1).Divide(Integer(0)); err == nil {
		t.Fatal("division by zero accepted")
	}
	if _, err := mustNumber(t, "1/2").FixedWidth(8, true); err == nil {
		t.Fatal("fraction coerced to integer")
	}
	if _, err := mustNumber(t, "1/2").Wrap(8, true); err == nil {
		t.Fatal("fraction wrapped")
	}
	if (Number{}).Show() != "0" {
