// Code generated by goplus from rounding_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func TestExplicitRounding(t *testing.T) {
	for _, test := range []struct {
		...
		n, err := ParseNumber(test.raw)
		if err != nil {
			t.Fatal(err)
		}
		actual := []string{n.Truncate().Show(), n.Floor().Show(), n.Ceiling().Show(), n.RoundHalfEven().Show()}
		expected := []string{test.trunc, test.floor, test.ceil, test.even}
		for i := range actual {
			if actual[i] != expected[i] {
				t.Errorf("%s mode%d got %s want%s", test.raw, i, actual[i], expected[i])
			}
		}
		if n.Show() != test.raw {
			t.Fatal("rounding mutated raw value")
		}
	}
}
func TestRoundingLaws(t *testing.T) {
	property := func(n int32, divisor uint16) bool {
		...
		x, _ := ParseNumber(fmt.Sprintf("%d/%d", n, denominator))
		lo, hi, nearest := x.Floor(), x.Ceiling(), x.RoundHalfEven()
		distance := nearest.Subtract(x)
		if distance.Sign() < 0 {
			distance = distance.Negate()
		}
		half, _ := ParseNumber("1/2")
