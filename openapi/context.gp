...
type Schema struct {Version string `json:"version"`;Operations []OperationBinding `json:"operations"`;Native *NativeBindings `json:"native,omitempty"`}
...
type NativeBindings struct {Version string `json:"version"`;Operations []NativeOperationBinding `json:"operations"`}
type NativeOperationBinding struct {OperationID string `json:"operationId"`;Parameters []NativeParameterBinding `json:"parameters,omitempty"`;RequestBody *NativeMediaBinding `json:"requestBody,omitempty"`;Responses []NativeResponseBinding `json:"responses"`}
type NativeParameterBinding struct {In string `json:"in"`;Name string `json:"name"`;FieldPath []string `json:"fieldPath,omitempty"`}
type NativeMediaBinding struct {MediaType string `json:"mediaType,omitempty"`}
type NativeResponseBinding struct {Status string `json:"status"`;Headers []NativeHeaderBinding `json:"headers,omitempty"`;Body *NativeMediaBinding `json:"body,omitempty"`}
type NativeHeaderBinding struct {Name string `json:"name"`;FieldPath []string `json:"fieldPath,omitempty"`}
...
func copySchema(schema Schema)Schema{out:=Schema{Version:schema.Version,Operations:make([]OperationBinding,len(schema.Operations)),Native:copyNativeBindings(schema.Native)};for i,item:=range schema.Operations{out.Operations[i]=item;out.Operations[i].Responses=append([]ResponseBinding(nil),item.Responses...)};return out}
func copyNativeBindings(input *NativeBindings)*NativeBindings{if input==nil{return nil};out:=&NativeBindings{Version:input.Version,Operations:make([]NativeOperationBinding,len(input.Operations))};for i,item:=range input.Operations{out.Operations[i]=item;out.Operations[i].Parameters=make([]NativeParameterBinding,len(item.Parameters));for j,parameter:=range item.Parameters{out.Operations[i].Parameters[j]=parameter;out.Operations[i].Parameters[j].FieldPath=append([]string(nil),parameter.FieldPath...)};if item.RequestBody!=nil{body:=*item.RequestBody;out.Operations[i].RequestBody=&body};out.Operations[i].Responses=make([]NativeResponseBinding,len(item.Responses));for j,response:=range item.Responses{out.Operations[i].Responses[j]=response;out.Operations[i].Responses[j].Headers=make([]NativeHeaderBinding,len(response.Headers));for k,header:=range response.Headers{out.Operations[i].Responses[j].Headers[k]=header;out.Operations[i].Responses[j].Headers[k].FieldPath=append([]string(nil),header.FieldPath...)};if response.Body!=nil{body:=*response.Body;out.Operations[i].Responses[j].Body=&body}}};return out}
...
func canonicalRecordFields(program *language.Program,name string,names []string)(map[string]*language.Type,error){
    ...
    declarations:=map[string]language.TypeDecl{};for _,decl:=range program.Syntax().Types{declarations[decl.Name]=decl};decl,ok:=declarations[name];if !ok||len(decl.Parameters)!=0{return nil,fmt.Errorf("must name a declared closed type")};fields,err:=canonicalRecordBody(decl.Body,declarations,map[string]bool{name:true},0);if err!=nil{return nil,err};if len(fields)!=len(names){return nil,fmt.Errorf("must have exactly fields %s",strings.Join(names,", "))};for _,field:=range names{if fields[field]==nil{return nil,fmt.Errorf("must have exactly fields %s",strings.Join(names,", "))}};return fields,nil
}
func canonicalRecordBody(body *language.Type,declarations map[string]language.TypeDecl,seen map[string]bool,depth int)(map[string]*language.Type,error){if body==nil||depth>128{return nil,fmt.Errorf("record type resolution limit exceeded")};match body.Form{case language.RefinedType(base,_):return canonicalRecordBody(base,declarations,seen,depth+1);case language.NamedType(alias):if seen[alias]{return nil,fmt.Errorf("cyclic alias")};seen[alias]=true;next,found:=declarations[alias];if !found||len(next.Parameters)!=0{return nil,fmt.Errorf("unknown alias %s",alias)};return canonicalRecordBody(next.Body,declarations,seen,depth+1);case language.AppliedType(_,_):name,args,ok:=openAPIAppliedType(body);if !ok{return nil,fmt.Errorf("must resolve to a record")};key:=name+"\x00"+language.FormatType(body);if seen[key]{return nil,fmt.Errorf("cyclic alias")};next,found:=declarations[name];if !found||next.Body==nil||len(next.Parameters)!=len(args){return nil,fmt.Errorf("must resolve to a record")};seen[key]=true;bindings:=map[string]*language.Type{};for i,param:=range next.Parameters{bindings[param]=args[i]};closed,err:=language.SubstituteType(next.Body,bindings);if err!=nil{return nil,fmt.Errorf("generic record specialization failed: %w",err)};return canonicalRecordBody(closed,declarations,seen,depth+1);case language.RecordType(fields):out:=map[string]*language.Type{};for _,field:=range fields{out[field.Name]=field.Type};return out,nil;case _:return nil,fmt.Errorf("must resolve to a record")}}
func openAPIAppliedType(typ *language.Type)(string,[]*language.Type,bool){root:=typ;args:=[]*language.Type{};for{match root.Form{case language.AppliedType(fn,arg):args=append([]*language.Type{arg},args...);root=fn;case language.NamedType(name):return name,args,true;case _:return "",nil,false}}}
