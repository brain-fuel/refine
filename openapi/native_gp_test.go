...
func TestNativeBindingsAreImmutableInRefineOnlyContract(t *testing.T) {
	...
	snapshot.Native.Operations[0].Parameters[0].FieldPath[0] = "mutated"
	snapshot.Native.Operations[0].Responses[0].Headers[0].FieldPath[0] = "mutated"
	again := contract.Schema()
	if again.Native.Operations[0].Parameters[0].FieldPath[0] != "n" || again.Native.Operations[0].Responses[0].Headers[0].FieldPath[0] != "n" {
		t.Fatal("pure Refine contract exposed mutable native metadata")
	}
}
