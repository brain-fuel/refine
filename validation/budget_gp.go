...
func tighter(a, b uint64) uint64 {
	if b != 0 && b < a {
		return b
	}
	return a
...
func NewBudget(schema Limits, caller Limits) *Budget {
	...
	if total == 0 {
		total = DefaultTotalSteps
	}
	if clause == 0 {
		clause = DefaultClauseSteps
	}
...
func (m *Meter) Step(cost uint64) error {
	...
	for scope := m; scope != nil; scope = scope.enclosing {
		if scope.exhausted || cost > scope.remaining {
			permitted = false
			break
		}
	}
