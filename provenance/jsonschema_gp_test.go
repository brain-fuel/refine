// Code generated by goplus from jsonschema_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func discover(t *testing.T, source string) *JSONSchema {
	t.Helper()
	s, err := DiscoverJSONSchema([]byte(source), schemajson.Limits{})
	if err != nil {
		t.Fatal(err)
	}
	return s
}
func find(t *testing.T, s *JSONSchema, path string) Constraint {
	t.Helper()
	...
	t.Fatalf("missing constraint %s", path)
	return Constraint{}
}
...
func TestNativeBoundCanonicalForms(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.typ+"/"+tc.key+"/"+tc.bound, func(t *testing.T) {
			...
			c := cs[0]
			if c.Predicate != tc.predicate || c.Native != tc.bound || c.Pointer != "/"+tc.key {
				t.Fatalf("%+v", c)
			}
			recovered, err := s.RecoverNative(c.Name, s.ConstraintSource())
			if err != nil || recovered != tc.bound {
				t.Fatalf("native recovery: %s %v", recovered, err)
			}
			if s.Original() != raw {
				t.Fatal("lost native source bytes")
			}
		})
	}
}
...
func TestPerConstraintDeviationIsolation(t *testing.T) {
	...
	s := discover(t, raw)
	minimum, maximum := find(t, s, "/minimum"), find(t, s, "/maximum")
	if len(s.Constraints()) != 2 {
		t.Fatal("annotation was mistaken for a schema")
	}
	original := s.ConstraintSource()
	edited := strings.Replace(original, minimum.Predicate, "(it > 0)", 1)
	findings, err := s.AuditSource(edited)
	if err != nil {
		t.Fatal(err)
	}
	if StatusName(findings[0].Status) != "changed" || StatusName(findings[1].Status) != "unchanged" {
		t.Fatalf("%+v", findings)
	}
	if _, err := s.RecoverNative(minimum.Name, edited); err == nil || err.(*Error).Code != "native.bijection_changed" {
		t.Fatal("edited minimum claimed original bijection")
	}
	got, err := s.RecoverNative(maximum.Name, edited)
	if err != nil || got != "100" {
		t.Fatal("editing minimum invalidated maximum")
	}
	if s.Original() != raw {
		t.Fatal("audit changed original")
	}
	...
	if _, err := s.RecoverNative(maximum.Name, equivalent); err == nil {
		t.Fatal("arbitrary equivalence treated as canonical")
	}
	...
	findings, err = s.AuditSource(extra)
	if err != nil {
		t.Fatal(err)
	}
	for _, finding := range findings {
		if StatusName(finding.Status) != "unchanged" {
			t.Fatal("additional refinement broke unrelated native guarantee")
		}
	}
	// The normal formatter is layout-only and does not cause a deviation.
	parsed, err := language.Parse(extra)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := s.RecoverNative(minimum.Name, language.Format(parsed)); err != nil {
		t.Fatal(err)
	}
	for _, source := range []string{"", strings.Replace(original, "type "+minimum.Name+" = Int where "+minimum.Predicate+"\n", "", 1)} {
		findings, err := s.AuditSource(source)
		if err != nil {
			t.Fatal(err)
		}
		if StatusName(findings[0].Status) != "removed" {
			t.Fatal("removed declaration retained its guarantee")
		}
	}
	if _, err := s.AuditSource(strings.Replace(original, minimum.Predicate, "123", 1)); err == nil {
		t.Fatal("non-Boolean edited rule accepted")
	}
	if _, err := s.RecoverNative("unknown", original); err == nil {
		t.Fatal("forged origin recovered")
	}
}
...
func TestSchemaPositionTraversalAndNativeRetention(t *testing.T) {
	...
	if c.SchemaPointer != "/$defs/a~1b~0c" {
		t.Fatal("lost escaped schema context")
	}
	...
	for _, raw := range []string{`true`, `false`, `{"minimum":0}`, `{"type":["integer","null"],"minimum":0}`, `{"type":"string","minimum":0}`, `{"$ref":"external.json"}`} {
		if s := discover(t, raw); len(s.Constraints()) != 0 || s.Original() != raw {
			t.Fatal("invented numeric type or resolved external reference")
		}
	}
	for _, tc := range []struct {
		raw  string
...
func TestBuiltinShadowingInvalidatesOnlyDependentOrigins(t *testing.T) {
	...
	if StatusName(findings[0].Status) != "unchanged" || StatusName(findings[1].Status) != "changed" {
		t.Fatal("shadowed native helper kept a false guarantee")
	}
	if _, err := s.RecoverNative(multiple.Name, source); err == nil {
		t.Fatal("recovered under changed predicate binding")
	}
	if _, err := s.RecoverNative(minimum.Name, source); err != nil {
		t.Fatal("shadowing unrelated helper invalidated minimum")
	}
	findings[1].Constraint.Builtins[0] = "forged"
	cs := s.Constraints()
	cs[1].Builtins[0] = "forged"
	if s.Constraints()[1].Builtins[0] != "isInteger" {
		t.Fatal("mutable binding metadata escaped")
	}
}

func TestConstraintSourceIsolationAndIdentity(t *testing.T) {
	raw := []byte(`{"type":"integer","minimum":1,"maximum":10}`)
	s, err := DiscoverJSONSchema(raw, schemajson.Limits{})
	if err != nil {
		t.Fatal(err)
	}
	raw[0] = 'x'
	original := s.ConstraintSource()
	cs := s.Constraints()
	cs[0].Predicate = "False"
	cs[0].Native = "999"
	if s.ConstraintSource() != original || s.Constraints()[0].Native != "1" {
		t.Fatal("mutable provenance escaped")
	}
	changed := discover(t, `{"type":"integer","minimum":2,"maximum":10}`)
	for i, c := range s.Constraints() {
		other := changed.Constraints()[i]
		if c.Name != other.Name {
			t.Fatal("identity depends on edited native value")
		}
		if (c.Fingerprint == other.Fingerprint) != (c.Keyword == "maximum") {
			t.Fatal("fingerprint granularity is not per constraint")
		}
	}
	...
	for i := 0; i < 8; i++ {
		...
		go func() {
			defer group.Done()
			for j := 0; j < 10; j++ {
				if _, err := s.AuditSource(original); err != nil {
					t.Error(err)
				}
			}
		}()
...
func nativeOracle(t *testing.T, raw string) *oracle.Schema {
	t.Helper()
	doc, err := oracle.UnmarshalJSON(strings.NewReader(raw))
	if err != nil {
		t.Fatal(err)
	}
	compiler := oracle.NewCompiler()
	compiler.DefaultDraft(oracle.Draft2020)
	if err := compiler.AddResource("https://refine.invalid/schema", doc); err != nil {
		t.Fatal(err)
	}
	schema, err := compiler.Compile("https://refine.invalid/schema")
	if err != nil {
		t.Fatal(err)
	}
	return schema
...
func TestNativeBoundSemanticProperties(t *testing.T) {
	property := func(bound int16, scale uint8, n int16, kind uint8) bool {
		...
		if keyword == "multipleOf" {
			rawBound = strings.TrimPrefix(rawBound, "-")
		}
		raw := fmt.Sprintf(`{"type":%q,%q:%s}`, typ, keyword, rawBound)
		native := nativeOracle(t, raw)
		s := discover(t, raw)
		program, err := language.Compile(s.ConstraintSource())
		if err != nil {
			t.Fatal(err)
		}
		payload := fmt.Sprint(n)
		if typ == "number" {
			payload += ".125"
		}
		number, err := value.ParseNumber(payload)
		if err != nil {
			t.Fatal(err)
		}
		...
		if state == "indeterminate" || expected != (state == "valid") {
			t.Fatalf("native %s payload %s: %s vs %v", raw, payload, state, expected)
		}
...
func TestExactFractionalMultiples(t *testing.T) {
	for _, tc := range []struct {
		...
		native := nativeOracle(t, raw)
		s := discover(t, raw)
		p, err := language.Compile(s.ConstraintSource())
		if err != nil {
			t.Fatal(err)
		}
		n, err := value.ParseNumber(tc.payload)
		if err != nil {
			t.Fatal(err)
		}
		report := p.ValidateData(s.Constraints()[0].Name, value.OfNumber(n), validation.Limits{})
		state := validation.StateName(report.State())
		if state == "indeterminate" || (state == "valid") != tc.valid || (native.Validate(json.Number(tc.payload)) == nil) != tc.valid {
			t.Fatalf("%s payload %s: %+v", raw, tc.payload, report.Diagnostics())
		}
	}
}
...
func FuzzNativeConstraintRoundTrip(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, raw string) {
		...
		s, err := DiscoverJSONSchema([]byte(raw), schemajson.Limits{Depth: 30, Nodes: 1000})
		if err != nil {
			return
		}
		if s.Original() != raw {
			t.Fatal("native source not preserved")
		}
		source := s.ConstraintSource()
		findings, err := s.AuditSource(source)
		if err != nil {
		...
		again, err := DiscoverJSONSchema([]byte(s.Original()), schemajson.Limits{Depth: 30, Nodes: 1000})
		if err != nil || again.ConstraintSource() != source {
		...
		for _, finding := range findings {
			...
			back, err := s.RecoverNative(finding.Constraint.Name, source)
			if err != nil || back != finding.Constraint.Native {
				t.Fatal("canonical-to-native recovery failed")
			}
		}
	})
}
