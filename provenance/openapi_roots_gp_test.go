...
func TestOpenAPISchemaRootIndexUsesEveryDocumentContext(t *testing.T) {
	...
	roots := indexOpenAPIRoots(t, []OpenAPIResource{entry, shared, secondary32, secondary31})
	want := []OpenAPISchemaRoot{{Resource: secondary31.URI, Pointer: "/components/schemas/Own", Dialect: jsonSchema202012}, {Resource: shared.URI, Pointer: "/response/content/multipart~1mixed/itemSchema", Dialect: openAPIBase}}
	if !reflect.DeepEqual(roots, want) {
		t.Fatalf("document-local version/dialect indexing changed: %+v", roots)
	}
}
...
func TestOpenAPISchemaRootIndexWrapperDocumentAndFragmentContexts(t *testing.T) {
	...
	roots := indexOpenAPIRoots(t, []OpenAPIResource{entry, target})
	want := []OpenAPISchemaRoot{{Resource: target.URI, Pointer: "/components/responses/R/content/multipart~1mixed/schema", Dialect: openAPIBase}}
	if !reflect.DeepEqual(roots, want) {
		t.Fatalf("wrapper target did not use its document context: %+v", roots)
	}
	...
	roots = indexOpenAPIRoots(t, []OpenAPIResource{base, draft, shared})
	want = []OpenAPISchemaRoot{{Resource: shared.URI, Pointer: "/response/content/application~1json/schema", Dialect: jsonSchema202012}, {Resource: shared.URI, Pointer: "/response/content/application~1json/schema", Dialect: openAPIBase}}
	if !reflect.DeepEqual(roots, want) {
		t.Fatalf("untyped fragment contexts were collapsed: %+v", roots)
	}
}

func TestOpenAPISchemaRootIndexDocumentContextWorkIsAggregate(t *testing.T) {
	first := OpenAPIResource{URI: openAPIEntry, Source: []byte(`{"openapi":"3.1.2"}`), Syntax: OpenAPIJSON, Role: OpenAPIDocument}
	second := OpenAPIResource{URI: "https://example.test/second.json", Source: []byte(`{"openapi":"3.1.2"}`), Syntax: OpenAPIJSON, Role: OpenAPIDocument}
	if _, err := IndexOpenAPISchemaRoots([]OpenAPIResource{first, second}, OpenAPIOptions{EntryResource: openAPIEntry, Limits: schemajson.Limits{Nodes: 6}}); err == nil || !strings.Contains(err.Error(), "document-context work limit") {
		t.Fatalf("document context work budget reset per resource: %v", err)
	}
}
