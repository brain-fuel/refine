// Code generated by goplus from run.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func Run(args []string, input io.Reader, output, errorOutput io.Writer) int {
	...
	switch command {
	case "explain", "validate", "validate-native", "satisfiable", "compare-payload", "check-schema":
		return workflow(args, input, output, errorOutput)
	}
	if command != "typecheck" && command != "fmt" && command != "inspect-json" {
		fmt.Fprintln(errorOutput, "unknown command; use refine help")
		return 2
	}
	jsonMode, path := false, ""
	for _, arg := range args[1:] {
		if arg == "--json" {
			if jsonMode || command == "fmt" {
				fmt.Fprintln(errorOutput, "--json is not valid here")
				return 2
			}
			jsonMode = true
		} else if path == "" {
			...
			fmt.Fprintln(errorOutput, "expected exactly one input path or -")
			return 2
		}
	}
	if path == "" {
		fmt.Fprintln(errorOutput, "expected an input path or -")
		return 2
	}
	...
	case "typecheck":
		...
		if err == nil {
			result.Summary = program.Summary()
		}
	case "fmt":
		var module *language.Module
		module, err = language.Parse(string(source))
		if err == nil {
			if _, failure := io.WriteString(output, language.Format(module)); failure != nil {
				return 2
			}
			return 0
		}
	case "inspect-json":
		...
		if err == nil {
			result.Summary = "JSON " + schemajson.KindName(document.Root().Kind()) + "; syntax and duplicate-key checks passed"
		}
	}
	code := 0
	if err != nil {
		...
		if errors.As(err, &languageError) {
			...
			if languageError.Code == "language.limit" {
	...
	if jsonMode {
		...
		if _, err := fmt.Fprintln(output, result.Summary); err != nil {
			return 2
		}
	} else {
		if _, failure := fmt.Fprintln(errorOutput, err); failure != nil {
			return 2
		}
	}
	return code
