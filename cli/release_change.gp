...
func releaseSourcePackages(entry *releaseSchemaEntry,catalog releaseCatalog)(string,error){
    type item struct{ID,Package string;Imports []string};items:=[]item{};files:=[]language.SourceFile{};root:=""
    canonicalID:=func(id string)string{if source:=catalog.entries[id];source!=nil{return "family:"+source.family};return "file:"+id}
    if entry.bundle!=nil{files=entry.bundle.Files();root=entry.sourcePath}else if entry.nativeProject!=nil{files=entry.nativeProject.LanguageFiles();root=entry.nativeProject.LanguageEntry()}
    for _,file:=range files{module,err:=language.Parse(file.Source);if err!=nil{return "",err};imports:=make([]string,len(file.Imports));for i,id:=range file.Imports{imports[i]=canonicalID(id)};items=append(items,item{canonicalID(file.ID),module.Package,imports})}
    sort.Slice(items,func(i,j int)bool{return items[i].ID<items[j].ID});encoded,err:=json.Marshal(struct{Entry string;Files []item}{canonicalID(root),items});return string(encoded),err
}
