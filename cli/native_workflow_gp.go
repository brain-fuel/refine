// Code generated by goplus from native_workflow.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func nativeCommand(args []string, input io.Reader, output, errorOutput io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(errorOutput, "expected native ingest, source, update, original, or validate-payload")
		return 2
	}
	action := args[0]
	...
	switch action {
	case "ingest":
		flags.StringVar(&resource, "resource", resource, "absolute URI identifying the input; never fetched")
		flags.StringVar(&pointer, "pointer", "", "selected root JSON Pointer")
		flags.StringVar(&rootType, "type", rootType, "editable root type name")
		flags.StringVar(&resourceFile, "resources", "", "explicit JSON array of offline native resources")
	...
	default:
		fmt.Fprintln(errorOutput, "unknown native command")
		return 2
	}
	...
	paths := flags.Args()
	expected := 1
	if action == "ingest" || action == "update" || action == "validate-payload" {
		expected = 2
	}
	if len(paths) != expected {
		fmt.Fprintln(errorOutput, "incorrect native command arguments; use refine help")
		return 2
	}
	inputPaths := paths
	if action == "ingest" {
	...
	if resourceFile != "" {
		inputPaths = append(append([]string(nil), inputPaths...), resourceFile)
	}
	stdin := 0
	...
	for _, name := range inputPaths {
		data, err := load(name, input)
		if err != nil {
			fmt.Fprintln(errorOutput, "cannot read native workflow input:", err)
			return 2
		}
		if len(data) > 16<<20 {
			fmt.Fprintln(errorOutput, "native workflow input exceeds 16 MiB")
			return 1
		}
		loaded[name] = data
	}
	failure := func(err error) int {
		code := "native.workflow"
		var detail *native.Error
		if errors.As(err, &detail) {
			code = detail.Code
		}
		message := "Native workflow failed; input values and native-oracle details are omitted."
		if action == "validate-payload" && jsonMode {
			result := report{Phase: "native.validate-payload", State: "invalid", Diagnostics: []diagnostic{{Code: code, Message: message}}}
			if code == "native.enforcement" || code == "native.limit" {
			...
			if _, err := fmt.Fprintf(errorOutput, "%s: %s\n", code, message); err != nil {
				return 2
			}
		}
		return 1
	}
	var project *native.Project
	var err error
	if action == "ingest" {
		...
		if resourceFile == "" {
			project, err = native.IngestProject(native.Format(paths[0]), loaded[paths[1]], options)
		} else {
			raw := loaded[resourceFile]
			if _, err = schemajson.Parse(raw, schemajson.Limits{}); err != nil {
				return failure(err)
			}
			var resources []native.Resource
			...
			for _, dependency := range resources {
				if dependency.URI == resource {
					return failure(fmt.Errorf("root resource must not be duplicated in dependencies"))
				}
			}
			resources = append(resources, native.Resource{URI: resource, Source: string(loaded[paths[1]])})
			project, err = native.IngestProjectResources(native.Format(paths[0]), resources, options)
		}
	} else {
		project, err = native.ParseBundle(loaded[paths[0]])
	}
	if err != nil {
		return failure(err)
	}
	...
	switch action {
	case "ingest":
		artifact, err = project.Bundle()
	case "source":
		artifact = []byte(project.EditableSource())
	case "original":
		artifact = project.NativeDocument().ExportOriginal()
	case "update":
		project, err = project.WithEditedSource(string(loaded[paths[1]]))
		if err == nil {
			artifact, err = project.Bundle()
		}
	case "validate-payload":
		if !nativeOnly {
			...
			for _, detail := range checked.Diagnostics() {
		...
		if jsonMode {
			if err := json.NewEncoder(output).Encode(report{Phase: "native.validate-payload", State: "valid", Summary: summary, Diagnostics: []diagnostic{}, Result: struct {
				NativeOnly bool `json:"nativeOnly"`
			}{true}}); err != nil {
				return 2
			}
	...
	if _, err := output.Write(artifact); err != nil {
		return 2
	}
	return 0
}
