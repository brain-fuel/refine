// Code generated by goplus from model_anonymous_records_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
`
	...
	files, err := GenerateJSONSerde(program, "example.anonymous", "Contract", "DocumentJSON", JSONSerdeOptions{Root: "Document", Integers: JSONIntegerNumber})
	if err != nil {
		t.Fatal(err)
	}
	expected := map[string]bool{"DocumentShippingRecord.java": false, "DocumentShippingRecordGeoRecord.java": false, "DocumentItemsItemRecord.java": false, "EnvelopePayloadRecord.java": false}
	for _, file := range files {
		if _, ok := expected[filepath.Base(file.Path)]; ok {
			expected[filepath.Base(file.Path)] = true
		}
	}
	for name, found := range expected {
		if !found {
			t.Fatalf("missing synthesized nested model %s", name)
		}
	}
	...
	harness := filepath.Join(dir, "AnonymousRecordGate.java")
	if err := os.WriteFile(harness, []byte(anonymousRecordHarnessJava), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("anonymous record javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-cp", classes+string(os.PathListSeparator)+classpath, "AnonymousRecordGate").CombinedOutput(); err != nil {
		t.Fatalf("anonymous record runtime: %v\n%s", err, output)
	}
}
