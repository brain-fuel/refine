// Code generated by goplus from properties_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func jetCheckClasspath(t *testing.T) string {
	...
	if dir == "" {
		if os.Getenv("REFINE_REQUIRE_JAVA") == "1" {
			t.Fatal("REFINE_JETCHECK_DIR is required for the Java release gate")
		}
	...
	for _, jar := range jars {
		...
		if fmt.Sprintf("%x", sha256.Sum256(data)) != jar.digest {
			t.Fatalf("checksum mismatch: %s", jar.name)
		}
...
func TestGeneratedJavaJetCheck(t *testing.T) {
	...
	if err := os.WriteFile(harness, []byte(propertiesJava), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("javac properties: %v\n%s", err, output)
	}
	cp := classes + string(os.PathListSeparator) + dependencies
	if output, err := exec.Command(vm, "-cp", cp, "Properties").CombinedOutput(); err != nil {
		t.Fatalf("jetCheck (including replay instructions): %v\n%s", err, output)
	} else {
		t.Log(string(output))
	}
}
