// Code generated by goplus from model_inheritance.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func (m *modelEmitter) modelRoot(name string) string {
	...
	for m.parents[name] != "" {
		if seen[name] {
			unsupported(m.declarations[name].At, "cyclic nominal model hierarchy")
		}
		seen[name] = true
		name = m.parents[name]
	}
...
func (m *modelEmitter) useFrame(frame modelFrame) {
	m.parameters = frame.types
	m.witnesses = frame.witnesses
}
func (m *modelEmitter) modelFrames(decl language.TypeDecl) []modelFrame {
	m.genericContext(decl)
	types, _, arguments := m.genericParts(decl)
	result := []modelFrame{{decl, m.parameters, m.witnesses, arguments, types}}
	for m.parents[decl.Name] != "" {
		_, args := applied(unrefined(decl.Body))
		parent := m.declarations[m.parents[decl.Name]]
		frame := modelFrame{declaration: parent, types: map[string]string{}, witnesses: map[string]string{}}
		for i, arg := range args {
			typ, witness := m.javaType(arg), m.witness(arg)
			frame.types[parent.Parameters[i]] = typ
			frame.witnesses[parent.Parameters[i]] = witness
			frame.arguments = append(frame.arguments, witness)
			frame.typeArguments = append(frame.typeArguments, typ)
		}
