// Code generated by goplus from fixed_float_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func TestGeneratedFiniteFloatModelsAndRuntimeParity(t *testing.T) {
	...
	program, err := language.Compile(fixedFloatContract)
	if err != nil {
		t.Fatal(err)
	}
	files, err := GenerateModels(program, "example.fixedfloat", "Contract")
	if err != nil {
		t.Fatal(err)
	}
	root := t.TempDir()
	...
	harness := filepath.Join(root, "FixedFloatHarness.java")
	if err := os.WriteFile(harness, []byte(fixedFloatHarnessJava), 0600); err != nil {
		t.Fatal(err)
	}
	...
	vectors := []vector{}
	for _, tc := range []struct{ name, raw string }{{"F32", "1/2"}, {"F32", "1/3"}, {"F32", "16777217"}, {"Nonnegative32", "-1/2"}, {"Sum32", "340282346638528859811704183484516925440"}, {"F64", "9007199254740992"}, {"F64", "9007199254740993"}} {
		text, _ := value.TextFromUTF8(tc.raw)
		_, report := program.ReadData(tc.name, text, validation.Limits{})
		vectors = append(vectors, vector{fmt.Sprintf("%s\t%s", tc.name, readUnits(text)), reportLine(report)})
	}
	var input strings.Builder
	for _, v := range vectors {
		input.WriteString(v.input + "\n")
	}
	command := exec.Command(vm, "-cp", classes, "example.fixedfloat.FixedFloatHarness")
	...
	if len(lines) != len(vectors) {
		t.Fatalf("got %d reports, want %d", len(lines), len(vectors))
	}
