// Code generated by goplus from native_regex.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func nativeRegexParts(enabled bool, className string) nativeRegexJavaParts {
	if !enabled {
		return nativeRegexJavaParts{fieldsAndConstructors: fmt.Sprintf("    private final Limits limits;private final tools.jackson.databind.ObjectMapper instanceMapper;private final com.networknt.schema.Schema schema;\n    public %s(){this(Limits.defaults());}public %s(Limits limits){this.limits=java.util.Objects.requireNonNull(limits);this.instanceMapper=mapper(limits);this.schema=compile();}", className, className), validationOpen: "var result=", compileSignature: "()"}
	}
	return nativeRegexJavaParts{
		hardLimits:            ",HARD_MAX_REGEX_PATTERN_UNITS=16384,HARD_MAX_REGEX_INPUT_UNITS=1<<20,HARD_MAX_REGEX_MILLIS=1000,HARD_MAX_REGEX_INITIALIZATION_MILLIS=10000;private static final long HARD_MAX_REGEX_EVALUATIONS=10000,HARD_MAX_REGEX_WORK_UNITS=8L<<20",
...
const nativeRegexRuntimeJava = `
    ...
    private static final class RegexRequest implements AutoCloseable{
        ...
        private final RegexLimits limits;private final java.util.concurrent.atomic.AtomicBoolean timedOut=new java.util.concurrent.atomic.AtomicBoolean();private final java.util.concurrent.atomic.AtomicBoolean initializationTimedOut=new java.util.concurrent.atomic.AtomicBoolean();private final java.util.Map<String,org.graalvm.polyglot.Value> matchers=new java.util.HashMap<>();private long evaluations,work,deadline;private org.graalvm.polyglot.Context context;private org.graalvm.polyglot.Value factory;private Thread watchdog;private volatile boolean closed,initializationComplete;
        private RegexRequest(RegexLimits limits){this.limits=limits;}
        private static RegexRequest open(RegexLimits limits){if(ACTIVE.get()!=null)throw new NativeValidationException(Code.ENFORCEMENT,"Native regex validation cannot be re-entered.");var request=new RegexRequest(limits);ACTIVE.set(request);try{request.initialize();return request;}catch(RuntimeException failure){request.abortInitialization();throw failure;}}
        ...
        private void initialize(){long initializationDeadline=System.nanoTime()+java.util.concurrent.TimeUnit.MILLISECONDS.toNanos(HARD_MAX_REGEX_INITIALIZATION_MILLIS);var hostAccess=org.graalvm.polyglot.HostAccess.newBuilder(org.graalvm.polyglot.HostAccess.NONE).allowMutableTargetMappings().build();var created=org.graalvm.polyglot.Context.newBuilder("js").engine(ENGINE).sandbox(org.graalvm.polyglot.SandboxPolicy.CONSTRAINED).allowHostAccess(hostAccess).allowHostClassLookup(name->false).allowNativeAccess(false).allowCreateThread(false).allowCreateProcess(false).allowIO(org.graalvm.polyglot.io.IOAccess.NONE).allowEnvironmentAccess(org.graalvm.polyglot.EnvironmentAccess.NONE).allowPolyglotAccess(org.graalvm.polyglot.PolyglotAccess.NONE).allowValueSharing(false).in(java.io.InputStream.nullInputStream()).out(java.io.OutputStream.nullOutputStream()).err(java.io.OutputStream.nullOutputStream()).option("js.ecmascript-version","2020").option("js.allow-eval","false").build();this.context=created;if(System.nanoTime()-initializationDeadline>=0)throw initializationFailure();var thread=Thread.ofVirtual().name("refine-native-regex-initialization-watchdog").inheritInheritableThreadLocals(false).unstarted(()->watchInitialization(created,initializationDeadline));thread.setContextClassLoader(null);this.watchdog=thread;thread.start();try{factory=created.eval(MATCHER);if(!factory.execute("(?:)").execute("").asBoolean())throw new NativeValidationException(Code.ENFORCEMENT,"Native ECMA-262 regex initialization produced an invalid matcher.");if(initializationTimedOut.get()||System.nanoTime()-initializationDeadline>=0)throw initializationFailure();}catch(NativeValidationException failure){throw failure;}catch(org.graalvm.polyglot.PolyglotException failure){if(initializationTimedOut.get()||failure.isCancelled()||failure.isInterrupted()||failure.isResourceExhausted())throw initializationFailure();throw new NativeValidationException(Code.ENFORCEMENT,"Native ECMA-262 regex initialization could not complete.");}catch(RuntimeException failure){if(initializationTimedOut.get())throw initializationFailure();throw new NativeValidationException(Code.ENFORCEMENT,"Native ECMA-262 regex initialization could not complete.");}finally{initializationComplete=true;thread.interrupt();watchdog=null;}deadline=System.nanoTime()+java.util.concurrent.TimeUnit.MILLISECONDS.toNanos(limits.maxMillis());thread=Thread.ofVirtual().name("refine-native-regex-watchdog").inheritInheritableThreadLocals(false).unstarted(()->watch(created));thread.setContextClassLoader(null);watchdog=thread;thread.start();}
        private void watchInitialization(org.graalvm.polyglot.Context active,long initializationDeadline){for(;;){long remaining=initializationDeadline-System.nanoTime();if(remaining<=0)break;java.util.concurrent.locks.LockSupport.parkNanos(remaining);if(Thread.interrupted())return;}if(closed||initializationComplete)return;initializationTimedOut.set(true);try{active.close(true);}catch(RuntimeException ignored){}}
        ...
        private static NativeValidationException initializationFailure(){return new NativeValidationException(Code.ENFORCEMENT,"Native ECMA-262 regex initialization exceeded its deployment limit.");}
        private void abortInitialization(){closed=true;ACTIVE.remove();if(watchdog!=null)watchdog.interrupt();if(context!=null)try{context.close(true);}catch(RuntimeException ignored){}matchers.clear();factory=null;}
        @Override public void close(){if(closed)return;closed=true;ACTIVE.remove();if(watchdog!=null)watchdog.interrupt();RuntimeException cleanup=null;if(context!=null&&!timedOut.get())try{context.close(true);}catch(RuntimeException failure){cleanup=failure;}matchers.clear();factory=null;if(cleanup!=null)throw new NativeValidationException(Code.ENFORCEMENT,"Native ECMA-262 regex cleanup could not complete.");}
    }
`
