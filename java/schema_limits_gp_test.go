...
func TestGeneratedSchemaLimitsApplyToValidationAndRead(t *testing.T) {
	...
	files, err := GenerateValidatorWithTypes(program, "example.schemalimits", "Contract", map[string]string{"Override": "Override"})
	if err != nil {
		t.Fatal(err)
	}
	...
	harness := filepath.Join(dir, "SchemaLimitsHarness.java")
	if err := os.WriteFile(harness, []byte(schemaLimitsHarnessJava), 0600); err != nil {
		t.Fatal(err)
	}
	...
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("schema limits javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-Xss256k", "-cp", classes, "SchemaLimitsHarness").CombinedOutput(); err != nil {
		t.Fatalf("schema limits Java: %v\n%s", err, output)
	}
}
