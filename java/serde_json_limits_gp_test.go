// Code generated by goplus from serde_json_limits_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func TestGeneratedJSONCodecResourceLimits(t *testing.T) {
	...
	program, err := language.Compile("type Root = { value :: Int }\n")
	if err != nil {
		t.Fatal(err)
	}
	...
	files, err := GenerateJSONSerde(program, "example.jsonlimits", "Contract", "RootModule", options)
	if err != nil {
		t.Fatal(err)
	}
	dir := t.TempDir()
	...
	harness := filepath.Join(dir, "JSONLimits.java")
	if err := os.WriteFile(harness, []byte(jsonLimitsHarnessJava), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("bounded JSON javac: %v\n%s", err, output)
	}
	command := exec.Command(vm, "-Xss256k", "-Xmx32m", "-cp", classes+string(os.PathListSeparator)+classpath, "JSONLimits")
	if output, err := command.CombinedOutput(); err != nil {
		t.Fatalf("bounded JSON runtime: %v\n%s", err, output)
	}
}
...
func TestJSONCodecGenerationLimitsFailClosed(t *testing.T) {
	...
	for _, limits := range cases {
		files, err := GenerateJSONSerde(program, "example", "Contract", "RootModule", JSONSerdeOptions{Root: "Root", CodecLimits: limits})
		if err == nil || files != nil {
			t.Fatalf("unsafe codec limits accepted: %+v", limits)
		}
	}
}
