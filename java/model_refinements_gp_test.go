// Code generated by goplus from model_refinements_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func TestInlineRefinementModels(t *testing.T) {
	...
	program, err := language.Compile(inlineModelContract)
	if err != nil {
		t.Fatal(err)
	}
	files, err := GenerateModels(program, "example.inline", "Contract")
	if err != nil {
		t.Fatal(err)
	}
	dir := t.TempDir()
	...
	harness := filepath.Join(dir, "InlineModels.java")
	if err := os.WriteFile(harness, []byte(inlineModelHarnessJava), 0644); err != nil {
		t.Fatal(err)
	}
	...
	for index, spec := range specs {
		target, err := program.PayloadType(spec)
		if err != nil {
			t.Fatal(err)
		}
		for _, n := range []int64{-1, 0, 1, 10} {
			for limit := uint64(0); limit < 300; limit++ {
				payload := testNumber(fmt.Sprint(n))
				if index == 5 {
					payload = testRecord(value.DataField{Name: "sample", Value: testNumber("1")}, value.DataField{Name: "text", Value: testText(fmt.Sprint(n))})
				}
	...
	defer cancel()
	command := exec.CommandContext(ctx, vm, "-Xss256k", "-cp", classes+string(os.PathListSeparator)+dependencies, "InlineModels")
	command.Stdin = strings.NewReader(input.String())
	var stderr bytes.Buffer
	command.Stderr = &stderr
	output, err := command.Output()
	if err != nil {
		t.Fatalf("inline models: %v\n%s", err, stderr.String())
	}
...
func TestInlineWitnessScale(t *testing.T) {
	...
	for i := 0; i < 1100; i++ {
		...
		fmt.Fprintf(&source, " f%d :: Box (a where show it /= \"\")", i)
	}
	source.WriteString(" }\ntype Deep a = { box :: Box (a where ")
	for i := 0; i < 200; i++ {
		source.WriteString("True && (")
	}
	source.WriteString("show it /= \"\"")
	source.WriteString(strings.Repeat(")", 200))
	source.WriteString(") }\n")
	...
	for i := 0; i < 1100; i++ {
		fmt.Fprintf(&harness, "d.setF%d(box);\n", i)
	}
	harness.WriteString("});\n")
	for i := 0; i < 1100; i++ {
		fmt.Fprintf(&harness, "if(wide.f%d().validate().state()!=Validation.State.VALID)throw new AssertionError();\n", i)
	}
	...
	target := filepath.Join(dir, "InlineScale.java")
	...
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("inline scale compile: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-Xss256k", "-cp", classes, "InlineScale").CombinedOutput(); err != nil {
		t.Fatalf("inline scale execution: %v\n%s", err, output)
	}
}
