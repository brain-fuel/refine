// Code generated by goplus from model_unions_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func TestGeneratedUnionModels(t *testing.T) {
	...
	program, err := language.Compile(unionModelContract)
	if err != nil {
		t.Fatal(err)
	}
	files, err := GenerateModels(program, "example.unions", "Contract")
	if err != nil {
		t.Fatal(err)
	}
	root := t.TempDir()
	...
	if err := os.WriteFile(proof, []byte(unionProofJava), 0644); err != nil {
		t.Fatal(err)
	}
	sources = append(sources, proof)
	harness := filepath.Join(root, "UnionConformance.java")
	if err := os.WriteFile(harness, []byte(unionModelHarnessJava), 0644); err != nil {
		t.Fatal(err)
	}
	...
	add := func(mode, name, constructor string, a, b int64, total, clause uint64) {
		av, bv := value.OfNumber(value.Integer(a)), value.OfNumber(value.Integer(b))
		data := testVariant(constructor)
		switch constructor {
		case "Cash", "Leaf":
			data = testVariant(constructor, av)
		case "Split":
			data = testVariant(constructor, av, bv)
		case "Card":
			data = testVariant(constructor, testRecord(value.DataField{Name: "name", Value: testText("Account")}, value.DataField{Name: "amount", Value: av}))
		case "Branch":
	...
	for _, name := range []string{"Payment", "Paid", "Large", "Bounded", "Ordered", "Unknown"} {
		for _, constructor := range []string{"Cash", "Card", "Split", "Free", "Missing"} {
			for _, n := range []int64{-1, 0, 1, 9, 10, 99, 100, 101} {
				for _, mode := range []string{"validate", "bypass", "read"} {
					add(mode, name, constructor, n, n+1, 0, 0)
				}
			}
		}
	}
	for _, name := range []string{"Tree", "PositiveTree"} {
		for _, constructor := range []string{"Leaf", "Branch", "Missing"} {
	...
	for _, name := range []string{"Payment", "Paid", "Large", "Unknown", "Ordered"} {
		for _, constructor := range []string{"Cash", "Split", "Free"} {
			for n := uint64(1); n < 400; n++ {
				add("validate", name, constructor, 1, 2, n, 0)
				add("validate", name, constructor, 0, 0, 0, n)
				add("read", name, constructor, 1, 2, n, 0)
			}
		}
	}
	...
	defer cancel()
	command := exec.CommandContext(ctx, vm, "-Xss256k", "-cp", classes+string(os.PathListSeparator)+dependencies, "UnionConformance")
	command.Stdin = strings.NewReader(input.String())
	var stderr bytes.Buffer
	command.Stderr = &stderr
	output, err := command.Output()
	if err != nil {
		t.Fatalf("Java union models: %v\n%s", err, stderr.String())
	}
	...
	for _, source := range []string{
		...
		target := filepath.Join(root, "Wrong.java")
		...
		if err == nil || !(strings.Contains(string(output), "incompatible types") || strings.Contains(string(output), "not allowed to extend sealed")) {
			t.Fatalf("nominal/sealed negative compilation: %v\n%s", err, output)
		}
	}
