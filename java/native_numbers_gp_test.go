...
func TestGeneratedNativeNumberWireDefaultsRemainExact(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		source, pointer, namespace string
	}{
		...
		project, err := native.IngestProject(tc.format, []byte(tc.source), native.ProjectOptions{Root: native.ResourceSelector{Pointer: tc.pointer, TypeName: "Amount"}, Metadata: native.WireMetadata{PublicationNamespace: tc.namespace}})
		if err != nil {
			t.Fatal(err)
		}
		generated, err := GenerateProjectJSONSerde(project, "Contract", "AmountModule")
		if err != nil {
			t.Fatal(err)
		}
	...
	harness := filepath.Join(dir, "NativeNumbers.java")
	if err := os.WriteFile(harness, []byte(nativeNumbersJava), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("native numbers javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-Xss256k", "-cp", classes+string(os.PathListSeparator)+classpath, "NativeNumbers").CombinedOutput(); err != nil {
		t.Fatalf("native numbers runtime: %v\n%s", err, output)
	}
}
