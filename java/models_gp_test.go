// Code generated by goplus from models_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func TestGeneratedSemanticModels(t *testing.T) {
	...
	program, err := language.Compile(modelContract)
	if err != nil {
		t.Fatal(err)
	}
	files, err := GenerateModels(program, "example.models", "Contract")
	if err != nil {
		t.Fatal(err)
	}
	dir := t.TempDir()
...
public final class ProofChecks {
	...
	if err := os.WriteFile(proof, []byte(proofSource), 0644); err != nil {
		t.Fatal(err)
	}
	...
	minimum := uint64(0)
	for limit := uint64(1); limit < 1000; limit++ {
		if validation.StateName(program.ValidateData("Booking", candidate, validation.Limits{Total: limit}).State()) == "valid" {
			minimum = limit
			break
		}
	}
	if minimum == 0 {
		t.Fatal("no valid Go booking budget")
	}
	harnessSource := strings.ReplaceAll(modelHarnessJava, "@BOOKING_BUDGET@", fmt.Sprint(minimum))
	harness := filepath.Join(dir, "Models.java")
	...
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("javac models: %v\n%s", err, output)
	}
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()
	if output, err := exec.CommandContext(ctx, vm, "-cp", classes+string(os.PathListSeparator)+dependencies, "Models").CombinedOutput(); err != nil {
		t.Fatalf("Java models: %v\n%s", err, output)
	} else {
		t.Log(string(output))
	}
	// An unrelated nominal wrapper with the same base type must not substitute.
	negative := filepath.Join(dir, "Wrong.java")
	if err := os.WriteFile(negative, []byte(`import example.models.*; class Wrong { AccountId id = new OtherId("x"); }`), 0644); err != nil {
		t.Fatal(err)
	}
	if output, err := exec.Command(compiler, "--release", "25", "-cp", classes, negative).CombinedOutput(); err == nil || !strings.Contains(string(output), "incompatible types") {
		t.Fatalf("unrelated nominal types became interchangeable: %v %s", err, output)
	}
}
...
func TestModelGenerationBoundaries(t *testing.T) {
	nested, err := language.Compile("type Nested = { value :: { inner :: Int } }")
	if err != nil {
		t.Fatal(err)
	}
	nestedFiles, err := GenerateModels(nested, "example", "Contract")
	if err != nil {
		t.Fatal(err)
	}
	foundParent, foundChild := false, false
	for _, file := range nestedFiles {
		if strings.HasSuffix(file.Path, "/Nested.java") {
			foundParent = strings.Contains(file.Source, "NestedValueRecord value()")
		}
		if strings.HasSuffix(file.Path, "/NestedValueRecord.java") {
			foundChild = strings.Contains(file.Source, "class NestedValueRecord") && strings.Contains(file.Source, "BigInteger inner()")
		}
	}
	if !foundParent || !foundChild {
		t.Fatal("nested anonymous record models were not generated with typed accessors")
	}
	for _, source := range []string{"type Unsupported = { callback :: Int -> Int }", "type Contract = Int", "type ModelSupport = Int", "type ModelType = Int", "type Id = Int\ntype ID = Int", "type DATA = Int", "type FooΣ = Int\ntype Fooς = Int"} {
		...
		files, err := GenerateModels(program, "example", "Contract")
		if _, ok := err.(*GenerationError); !ok || files != nil {
			t.Fatalf("unsupported model did not fail atomically: %s: %v", source, err)
		}
	}
	if files, err := GenerateModels(nil, "", "Contract"); files != nil || err == nil {
	...
	first, err := GenerateModels(program, "example.models", "Contract")
	if err != nil {
		t.Fatal(err)
	}
	copy := program.Syntax()
	copy.Types[0].Name = "Changed"
	second, err := GenerateModels(program, "example.models", "Contract")
	if err != nil {
		t.Fatal(err)
	}
	if len(first) != len(second) {
		t.Fatal("unstable file count")
	}
	seen := map[string]bool{}
	for i, file := range first {
		if file != second[i] || seen[file.Path] {
			t.Fatal("nondeterministic or colliding output")
		}
		seen[file.Path] = true
	}
}
...
func TestModelPackageLayouts(t *testing.T) {
	...
	for _, namespace := range []string{"", "δοκιμή.映像", "record.var"} {
		t.Run(namespace, func(t *testing.T) {
			files, err := GenerateModels(program, namespace, "Contract")
			if err != nil {
				t.Fatal(err)
			}
			dir := t.TempDir()
			...
			if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
				t.Fatalf("model package: %v %s", err, output)
			}
		})
	}
}
...
func FuzzModelGeneration(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, source string) {
		...
		first, err := GenerateModels(program, "example", "Contract")
		second, again := GenerateModels(program, "example", "Contract")
		if err != nil {
			if again == nil || first != nil || second != nil {
				t.Fatal("partial or nondeterministic failed model generation")
			}
			return
		}
		if again != nil || len(first) != len(second) {
			t.Fatal("unstable model source set")
		}
		seen := map[string]bool{}
		for i, file := range first {
			if file != second[i] || seen[file.Path] {
				t.Fatal("nondeterministic or colliding model source")
			}
			seen[file.Path] = true
		}
	})
...
func BenchmarkGenerateModels(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		if _, err := GenerateModels(program, "example", "Contract"); err != nil {
			b.Fatal(err)
		}
	}
}
