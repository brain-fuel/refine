...
func GenerateProjectPropertyTests(project *native.Project,namespace,contractName string,options PropertyTestOptions)([]File,error){
    ...
    if options.NativeJSONValidator!=""{
        for _,probe:=range []string{`null`,`false`,`true`,`0`,`1`,`-1`,`""`,`"__refine_probe__"`,`[]`,`{}`,`[null]`}{
            failure:=project.ValidateJSON([]byte(probe));if failure==nil{continue};var invalid *native.Error
            if errors.As(failure,&invalid)&&invalid.Code=="native.payload"{options.nativeInvalidJSON=append(options.nativeInvalidJSON,probe)}else{return nil,fmt.Errorf("native rejection probe could not be classified: %w",failure)}
        }
    }
