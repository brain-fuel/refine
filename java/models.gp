...
func (m *modelEmitter) shape(name string)*language.Type{
    ...
    for{
        ...
        decl:=m.declarations[name];if decl.Body==nil{unsupported(decl.At,"Java model alternatives remain required; validator generation already supports them")}
...
func GenerateModels(program *language.Program,namespace,contractName string)(files []File,failure error){
    ...
    for _,decl:=range m.module.Types{
        if decl.Body==nil{unsupported(decl.At,"tagged-union model emission remains required")}
    ...
    for _,decl:=range m.module.Types{root:=decl.Name;for m.parents[root]!=""{root=m.parents[root]};shape:=m.shape(root);match shape.Form{case language.RecordType(_):m.recordFields(root,shape);case _:}}
    ...
    support:=[]struct{name string;body string}{{"ModelSupport",fmt.Sprintf(modelSupportJava,strings.Join(parents,","),contractName,contractName)},{"ModelMaybe",modelMaybeJava},{"ModelNullable",modelNullableJava},{"ModelResult",modelResultJava}}
    for _,item:=range support{files=append(files,File{Path:path.Join(prefix,item.name+".java"),Source:header+item.body})}
    for _,decl:=range m.module.Types{files=append(files,File{Path:path.Join(prefix,decl.Name+".java"),Source:header+m.model(decl)})}
    return files,nil
}
