...
func GeneratePropertyTests(program *language.Program,namespace,contractName string,options PropertyTestOptions)([]File,error){
    ...
    candidateFields:="";candidateChecks:=[]string{}
    ...
    if options.AvroSerde!=""{if err:=javaClassName(options.AvroSerde);err!=nil{return nil,err};candidateChecks=append(candidateChecks,"AVRO.acceptsNativeCandidate(data)")}
    candidateExpression:="true";if len(candidateChecks)>0{candidateExpression=strings.Join(candidateChecks," && ")};nativeCandidate:=candidateFields+"    private static boolean nativeCandidate(Data data) { return "+candidateExpression+"; }\n"
