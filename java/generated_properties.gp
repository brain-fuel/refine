...
type propertyRule struct { target string; code string; predicate string; enclosing string; paths []string; offset int; automatic bool }
...
func propertyAffectedPaths(predicate *language.Expr,enclosing string)[]string{relative:=language.AffectedPaths(predicate,"");if len(relative)==0||len(relative)>1000000{return []string{enclosing}};limit:=1<<20;if len(enclosing)>0&&len(relative)>limit/len(enclosing){return []string{enclosing}};used:=len(enclosing)*len(relative);for _,item:=range relative{if len(item)>limit-used{return []string{enclosing}};used+=len(item)};out:=make([]string,len(relative));for i,item:=range relative{out[i]=enclosing+item};return out}
func (e *propertyEmitter) rules(target,path string,t *language.Type)([]propertyRule,error){return e.rulesAt(target,path,path,t)}
func (e *propertyEmitter) rulesAt(target,codePath,diagnosticPath string,t *language.Type)([]propertyRule,error){
    ...
    case language.RefinedType(base,rules):
        nested,err:=e.rulesAt(target,codePath,diagnosticPath,base);if err!=nil{return nil,err};result=append(result,nested...);for _,rule:=range rules{result=append(result,propertyRule{target:target,code:generatedRuleCode(codePath,rule),predicate:language.FormatExpression(rule.Predicate),enclosing:diagnosticPath,paths:propertyAffectedPaths(rule.Predicate,diagnosticPath),offset:rule.At.Start.Offset,automatic:rule.Code==""})}
    case language.RecordType(fields):for _,field:=range fields{escaped:="/"+strings.ReplaceAll(strings.ReplaceAll(field.Name,"~","~0"),"/","~1");nested,err:=e.rulesAt(target,codePath+escaped,diagnosticPath+escaped,field.Type);if err!=nil{return nil,err};result=append(result,nested...)}
    case language.ListType(element):nested,err:=e.rulesAt(target,codePath+"/0",diagnosticPath+"/"+OpenAPIPropertyElementPathSegment,element);if err!=nil{return nil,err};result=append(result,nested...)
    case language.NamedType(name):if decl,ok:=e.declarations[name];ok{if e.visiting[name]{return result,nil};e.visiting[name]=true;if decl.Body!=nil{nested,err:=e.rulesAt(target,codePath,diagnosticPath,decl.Body);if err!=nil{return nil,err};result=append(result,nested...)}else{for _,variant:=range decl.Variants{for index,argument:=range variant.Arguments{suffix:=fmt.Sprintf("/%d",index);nested,err:=e.rulesAt(target,codePath+suffix,diagnosticPath+suffix,argument);if err!=nil{return nil,err};result=append(result,nested...)}}};delete(e.visiting,name)}
    case language.AppliedType(_,_):name,args:=applied(t);if name=="Maybe"||name=="Nullable"{if len(args)!=1{return nil,fmt.Errorf("%s requires one payload type",name)};return e.rulesAt(target,codePath,diagnosticPath,args[0])};if name=="Map"{if len(args)!=2{return nil,fmt.Errorf("Map requires String keys and one value type")};return e.rulesAt(target,codePath+"/0",diagnosticPath+"/"+OpenAPIPropertyElementPathSegment,args[1])};if name=="Result"{if len(args)!=2{return nil,fmt.Errorf("Result requires two payload types")};for _,argument:=range args{nested,err:=e.rulesAt(target,codePath+"/0",diagnosticPath+"/0",argument);if err!=nil{return nil,err};result=append(result,nested...)};return result,nil};decl,ok:=e.declarations[name];if !ok||len(args)!=len(decl.Parameters){return nil,fmt.Errorf("unsupported applied payload type %s",name)};if e.visiting[name]{return result,nil};e.visiting[name]=true;defer delete(e.visiting,name);bindings:=map[string]*language.Type{};for i,param:=range decl.Parameters{bindings[param]=args[i]};if decl.Body==nil{for _,variant:=range decl.Variants{for index,argument:=range variant.Arguments{closed,err:=language.SubstituteType(argument,bindings);if err!=nil{return nil,err};suffix:=fmt.Sprintf("/%d",index);nested,err:=e.rulesAt(target,codePath+suffix,diagnosticPath+suffix,closed);if err!=nil{return nil,err};result=append(result,nested...)}}}else{closed,err:=language.SubstituteType(decl.Body,bindings);if err!=nil{return nil,err};return e.rulesAt(target,codePath,diagnosticPath,closed)}
