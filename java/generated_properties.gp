...
type PropertyExample struct { Target string; Value value.Data; Expected PropertyOutcome; DiagnosticCodes []string }
type PropertyReplay struct { Target string; Kind ReplayKind; DiagnosticCode string; SerializedData string }
type PropertyTestOptions struct { Targets []PropertyTarget; CaseCount int; AttemptBudget int; Seed int64; Examples []PropertyExample; Replays []PropertyReplay }
...
func GeneratePropertyTests(program *language.Program,namespace,contractName string,options PropertyTestOptions)([]File,error){
    ...
    emitter:=&propertyEmitter{declarations:declarations,visiting:map[string]bool{}};var methods strings.Builder;known:=map[string]bool{};usedReplays:=map[string]bool{}
    for i,target:=range targets{decl,ok:=declarations[target.Name];if !ok||len(decl.Parameters)>0||decl.Body==nil{return nil,fmt.Errorf("property target %s must be a closed non-union declaration",target.Name)};if known[target.Name]{return nil,fmt.Errorf("duplicate property target %s",target.Name)};known[target.Name]=true;raw,err:=emitter.generator(decl.Body);if err!=nil{return nil,fmt.Errorf("property target %s: %w",target.Name,err)};rules,err:=emitter.rules(target.Name,"",decl.Body);if err!=nil{return nil,err};validKey:=replayKey(target.Name,ReplayValid,"");validReplay:=replays[validKey];if validReplay!=""{usedReplays[validKey]=true};factoryDecl,receiver:="",target.Name+".";if targetUsesFactory(decl.Body,declarations,map[string]bool{}){factoryDecl="var factory=new "+target.Name+".Factory(); ";receiver="factory."};fmt.Fprintf(&methods,"    private static boolean validBoundary%d(Data data) {\n        try { %svar model=%sfromData(data); if (!model.rawData().equals(data) || model.validate().state()!=Validation.State.VALID) return false; var shown=model.showWithoutValidation(); var read=%sread(shown); return read.validate().state()==Validation.State.VALID && read.showWithoutValidation().equals(shown); } catch (ValidationException failure) { return false; }\n    }\n",i,factoryDecl,receiver,receiver);fmt.Fprintf(&methods,"    private static boolean invalidBoundary%d(Data data,String code) {\n        %svar bypass=%sfromDataWithoutValidation(data); if (!bypass.rawData().equals(data) || !targeted(bypass.validate(),code)) return false; try { %sfromData(data); return false; } catch (ValidationException failure) { if (!targeted(failure.outcome(),code)) return false; } try { %sread(bypass.showWithoutValidation()); return false; } catch (ValidationException failure) { return targeted(failure.outcome(),code); }\n    }\n",i,factoryDecl,receiver,receiver,receiver);fmt.Fprintf(&methods,"    private static void target%d() {\n        var raw = %s;\n        var valid = requiring(raw, d -> %s.validate(%s,d).state() == Validation.State.VALID, %d, %s);\n        check(valid, %sGeneratedProperties::validBoundary%d, %s);\n",i,raw,contractName,javaQuote(target.Name),attempts,javaQuote("valid "+target.Name),contractName,i,javaQuote(validReplay));for j,rule:=range rules{key:=replayKey(target.Name,ReplayInvalid,rule.code);replay:=replays[key];if replay!=""{usedReplays[key]=true};fmt.Fprintf(&methods,"        var invalid%d = requiring(raw, d -> targeted(%s.validate(%s,d),%s), %d, %s);\n        check(invalid%d, d -> invalidBoundary%d(d,%s), %s);\n",j,contractName,javaQuote(target.Name),javaQuote(rule.code),attempts,javaQuote("invalid "+target.Name+" "+rule.code),j,i,javaQuote(rule.code),javaQuote(replay))};methods.WriteString("    }\n")}
    for key:=range replays{if !usedReplays[key]{return nil,fmt.Errorf("replay does not match a generated target property")}}
    var examples strings.Builder;for i,example:=range options.Examples{if !known[example.Target]{return nil,fmt.Errorf("example %d targets unknown property target %s",i,example.Target)};data,err:=exampleDataJava(example.Value);if err!=nil{return nil,err};state:="VALID";if example.Expected==ExampleInvalid{state="INVALID"}else if example.Expected==ExampleIndeterminate{state="INDETERMINATE"}else if example.Expected!=ExampleValid{return nil,fmt.Errorf("example %d has invalid expected outcome",i)};fmt.Fprintf(&examples,"        var example%d = %s.validate(%s,%s); if (example%d.state() != Validation.State.%s) throw new AssertionError(\"embedded example %d outcome\");\n",i,contractName,javaQuote(example.Target),data,i,state,i);for _,code:=range example.DiagnosticCodes{fmt.Fprintf(&examples,"        if (example%d.diagnostics().stream().noneMatch(d -> d.code().equals(%s))) throw new AssertionError(\"embedded example %d diagnostic\");\n",i,javaQuote(code),i)}}
...
`,class,cases,options.Seed,methods.String(),strings.Join(calls," "),examples.String())
