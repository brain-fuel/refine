// Code generated by goplus from property_integers.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func fixedIntegerPropertyGenerator(name string) (string, error) {
	...
	if err != nil || width == 0 || width > 65536 {
		return "", fmt.Errorf("fixed integer property width must be 1 through 65536")
	}
	count := new(big.Int).Lsh(big.NewInt(1), uint(width))
	minimum := new(big.Int)
	if !strings.HasPrefix(name, "UInt") {
		minimum.Neg(new(big.Int).Rsh(new(big.Int).Set(count), 1))
	}
