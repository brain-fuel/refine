// Code generated by goplus from serde_json_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func jacksonClasspath(t *testing.T) string {
	dir := os.Getenv("REFINE_JACKSON_DIR")
	if dir == "" {
		if os.Getenv("REFINE_REQUIRE_JAVA") == "1" {
			t.Fatal("REFINE_JACKSON_DIR is required by the Java release gate")
		}
		t.Skip("set REFINE_JACKSON_DIR to pinned Jackson 3 test jars")
	}
	...
	for _, jar := range jars {
		...
		if err != nil {
			t.Fatalf("Jackson test dependency: %v", err)
		}
		sum := sha256.Sum256(data)
		if hex.EncodeToString(sum[:]) != jar.sum {
			t.Fatalf("Jackson test dependency %s has an unexpected SHA-256", jar.name)
		}
...
`)
	...
	files, err := GenerateJSONSerde(program, "example.jsonserde", "Contract", "SampleJacksonModule", JSONSerdeOptions{Root: "Sample"})
	if err != nil {
		t.Fatal(err)
	}
	dir := t.TempDir()
	...
	harness := filepath.Join(dir, "JSONSerde.java")
	if err := os.WriteFile(harness, []byte(jsonSerdeHarnessJava), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("Jackson 3 javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-cp", classes+string(os.PathListSeparator)+classpath, "JSONSerde").CombinedOutput(); err != nil {
		t.Fatalf("Jackson 3 runtime: %v\n%s", err, output)
	}
}
...
func TestJacksonExactRealPolicies(t *testing.T) {
	compiler, vm := javaTools(t)
	classpath := jacksonClasspath(t)
	program, err := language.Compile("type Exact = Real\n")
	if err != nil {
		t.Fatal(err)
	}
	decimal, err := GenerateJSONSerde(program, "example.jsonnumbers", "Contract", "DecimalModule", JSONSerdeOptions{Root: "Exact", Reals: JSONRealExactDecimal})
	if err != nil {
		t.Fatal(err)
	}
	rational, err := GenerateJSONSerde(program, "example.jsonnumbers", "Contract", "RationalModule", JSONSerdeOptions{Root: "Exact", Reals: JSONRealRationalString})
	if err != nil {
		t.Fatal(err)
	}
	files := decimal
	for _, file := range rational {
		if strings.HasSuffix(file.Path, "RationalModule.java") {
	...
	harness := filepath.Join(dir, "JSONNumbers.java")
	if err := os.WriteFile(harness, []byte(jsonNumbersJava), 0644); err != nil {
		t.Fatal(err)
	}
	sources = append(sources, harness)
	classes := filepath.Join(dir, "classes")
	args := append([]string{"--release", "25", "-Xlint:all", "-Werror", "-cp", classpath, "-d", classes}, sources...)
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("exact JSON javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-cp", classes+string(os.PathListSeparator)+classpath, "JSONNumbers").CombinedOutput(); err != nil {
		t.Fatalf("exact JSON runtime: %v\n%s", err, output)
	}
}
...
func TestJSONSerdeRequiresExplicitWirePolicies(t *testing.T) {
	...
	for _, tc := range cases {
		...
		files, err := GenerateJSONSerde(program, "example", "Contract", "Module", JSONSerdeOptions{Root: tc.root})
...
func TestJacksonClosedGenericRoot(t *testing.T) {
	compiler, vm := javaTools(t)
	classpath := jacksonClasspath(t)
	program, err := language.Compile("type Box a = { value :: a }\ntype Age = Int where it >= 0\ntype AgeBox = Box Age\n")
	if err != nil {
		t.Fatal(err)
	}
	files, err := GenerateJSONSerde(program, "example.jsongeneric", "Contract", "AgeBoxModule", JSONSerdeOptions{Root: "AgeBox"})
	if err != nil {
		t.Fatal(err)
	}
	dir := t.TempDir()
	...
	harness := filepath.Join(dir, "JSONGeneric.java")
	source := `import example.jsongeneric.*; public final class JSONGeneric { public static void main(String[] a)throws Exception{var mapper=tools.jackson.databind.json.JsonMapper.builder().addModule(new AgeBoxModule()).build();var box=mapper.readValue("{\"value\":21}",AgeBox.class);if(!box.value().value().equals(java.math.BigInteger.valueOf(21)))throw new AssertionError();if(!mapper.writeValueAsString(box).equals("{\"value\":21}"))throw new AssertionError();}}`
	if err := os.WriteFile(harness, []byte(source), 0644); err != nil {
		t.Fatal(err)
	}
	sources = append(sources, harness)
	classes := filepath.Join(dir, "classes")
	args := append([]string{"--release", "25", "-Xlint:all", "-Werror", "-cp", classpath, "-d", classes}, sources...)
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("generic JSON javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-cp", classes+string(os.PathListSeparator)+classpath, "JSONGeneric").CombinedOutput(); err != nil {
		t.Fatalf("generic JSON runtime: %v\n%s", err, output)
	}
}
...
`)
	...
	files, err := GenerateJSONSerde(program, "example.jsonnested", "Contract", "RootModule", JSONSerdeOptions{Root: "Root"})
	if err != nil {
		t.Fatal(err)
	}
	preserved, err := GenerateJSONSerde(program, "example.jsonnested", "Contract", "PreserveModule", JSONSerdeOptions{Root: "Root", PreserveExtraFieldsFor: map[string]bool{"Node": true, "Box": true}})
	if err != nil {
		t.Fatal(err)
	}
	for _, file := range preserved {
		if strings.HasSuffix(file.Path, "PreserveModule.java") {
			files = append(files, file)
		}
	}
	module := ""
	...
	sources := []string{}
	for _, file := range files {
		if strings.HasSuffix(file.Path, "RootModule.java") {
			module = file.Source
		}
		target := filepath.Join(dir, filepath.FromSlash(file.Path))
		if err := os.MkdirAll(filepath.Dir(target), 0755); err != nil {
	...
	if !strings.Contains(module, "Node") || !strings.Contains(module, "Box<Int>") {
		t.Fatal("nested specialization descriptors were not emitted")
	}
	harness := filepath.Join(dir, "JSONNested.java")
	if err := os.WriteFile(harness, []byte(jsonNestedJava), 0644); err != nil {
		t.Fatal(err)
	}
	sources = append(sources, harness)
	classes := filepath.Join(dir, "classes")
	args := append([]string{"--release", "25", "-Xlint:all", "-Werror", "-cp", classpath, "-d", classes}, sources...)
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("nested JSON javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-cp", classes+string(os.PathListSeparator)+classpath, "JSONNested").CombinedOutput(); err != nil {
		t.Fatalf("nested JSON runtime: %v\n%s", err, output)
	}
}
...
`)
	...
	options := JSONSerdeOptions{Root: "Root", Discriminators: map[string]JSONDiscriminator{
	...
	files, err := GenerateJSONSerde(program, "example.jsonunion", "Contract", "RootModule", options)
	if err != nil {
		t.Fatal(err)
	}
	dir := t.TempDir()
	...
	harness := filepath.Join(dir, "JSONUnion.java")
	if err := os.WriteFile(harness, []byte(jsonUnionJava), 0644); err != nil {
		t.Fatal(err)
	}
	sources = append(sources, harness)
	classes := filepath.Join(dir, "classes")
	args := append([]string{"--release", "25", "-Xlint:all", "-Werror", "-cp", classpath, "-d", classes}, sources...)
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("union JSON javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-cp", classes+string(os.PathListSeparator)+classpath, "JSONUnion").CombinedOutput(); err != nil {
		t.Fatalf("union JSON runtime: %v\n%s", err, output)
	}
	bad := options
	bad.Discriminators = map[string]JSONDiscriminator{"Choice": {Field: "kind", Values: map[string]string{"None": "same", "Number": "same", "Pair": "pair"}, Arguments: map[string][]string{"None": {}, "Number": {"value"}, "Pair": {"label", "count"}}}, "Result": options.Discriminators["Result"]}
	if generated, err := GenerateJSONSerde(program, "example", "Contract", "BadModule", bad); err == nil || generated != nil {
		t.Fatal("duplicate discriminator values accepted")
	}
}
