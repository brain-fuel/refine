// Code generated by goplus from serde_json_extra_fields_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func TestGeneratedJSONExtraFieldPoliciesArePerOccurrence(t *testing.T) {
	...
	program, err := language.Compile(jsonExtraFieldContract)
	if err != nil {
		t.Fatal(err)
	}
	files, err := GenerateJSONSerde(program, "example.extras", "Contract", "DiscardModule", JSONSerdeOptions{Root: "Root"})
	if err != nil {
		t.Fatal(err)
	}
	modules := []struct {
	...
	for _, item := range modules {
		generated, generateErr := GenerateJSONSerde(program, "example.extras", "Contract", item.name, item.options)
		if generateErr != nil {
			t.Fatal(generateErr)
		}
		found := false
		for _, file := range generated {
			if strings.HasSuffix(file.Path, "/"+item.name+".java") {
				files = append(files, file)
				found = true
			}
		}
		if !found {
			t.Fatal("JSON module source is absent", item.name)
		}
	}
	same := JSONSerdeOptions{Root: "IntBox", PreserveExtraFields: true, PreserveExtraFieldsFor: map[string]bool{"Box": true}}
	if generated, sameErr := GenerateJSONSerde(program, "example.same", "Contract", "SameModule", same); sameErr != nil || len(generated) == 0 {
		t.Fatalf("equal alias-chain policies did not coalesce: %v", sameErr)
	}
	...
	for _, options := range guards {
		if generated, guardErr := GenerateJSONSerde(program, "example.bad", "Contract", "BadModule", options); guardErr == nil || generated != nil {
			t.Fatalf("invalid per-occurrence policy emitted output: %+v", options)
		}
	}
	...
	if generated, guardErr := GenerateJSONSerde(scalar, "example.bad", "Contract", "BadModule", JSONSerdeOptions{Root: "Scalar", RejectExtraFields: true}); guardErr == nil || generated != nil {
		t.Fatal("non-record reject policy emitted output")
	}
	metadataOptions, metadataErr := JSONSerdeOptionsFromMetadata("IntBox", native.WireMetadata{ExtraFields: map[string]native.ExtraFieldMode{"IntBox": native.RejectExtraFields}})
	if metadataErr != nil || !metadataOptions.RejectExtraFields || !metadataOptions.RejectExtraFieldsFor["IntBox"] {
		t.Fatalf("reject metadata was not retained: %+v %v", metadataOptions, metadataErr)
	}
	dir := t.TempDir()
	sources := []string{}
	...
	harness := filepath.Join(dir, "ExtraFieldPolicies.java")
	if err := os.WriteFile(harness, []byte(jsonExtraFieldHarnessJava), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("extra-field javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-cp", classes+string(os.PathListSeparator)+classpath, "ExtraFieldPolicies").CombinedOutput(); err != nil {
		t.Fatalf("extra-field runtime: %v\n%s", err, output)
	}
}
