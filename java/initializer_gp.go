// Code generated by goplus from initializer.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
type initializer struct {
	...
	signatures   []*language.Type
	signatureIDs map[*language.Type]int
}
...
func (e *initializer) node(typ, source string) string {
	index := len(e.nodes)
	e.nodes = append(e.nodes, initializerNode{typ, source})
...
func (e *initializer) literal(text string) string {
	...
	if len(text) <= 8192 {
		return javaQuote(text)
	}
	pieces := []string{}
	start := 0
	for offset := range text {
		if offset-start >= 8192 {
			pieces = append(pieces, e.node("String", javaQuote(text[start:offset])))
			start = offset
		}
	}
...
func (e *initializer) strings(items []string) string {
	...
	for i, item := range items {
		values[i] = e.literal(item)
	}
	return e.list("String", values)
}
func (e *initializer) list(typ string, items []string) string {
	if len(items) <= initializerChunk {
		return e.node("java.util.List<"+typ+">", javaList(items))
	}
	parts := []string{}
	for start := 0; start < len(items); start += initializerChunk {
		end := start + initializerChunk
		if end > len(items) {
			end = len(items)
		}
		parts = append(parts, e.list(typ, items[start:end]))
	}
...
func (e *initializer) signature(t *language.Type) string {
	...
	if e.signatureIDs == nil {
		e.signatureIDs = make(map[*language.Type]int)
	}
	index, found := e.signatureIDs[t]
	if !found {
		index = len(e.signatures)
		e.signatureIDs[t] = index
		e.signatures = append(e.signatures, t)
	}
...
func (e *initializer) source(root, signatures string) string {
	...
	for start := 0; start < len(e.nodes); start += initializerChunk {
		...
		for i := start; i < end; i++ {
			node := e.nodes[i]
			fmt.Fprintf(&out, "    static final %s n%d = %s;\n", node.typ, i, node.source)
		}
		out.WriteString("}\n")
	}
	for start := 0; start < chunks; start += initializerLoader {
		...
		end := start + initializerLoader
		if end > chunks {
			end = chunks
		}
		for i := start; i < end; i++ {
			fmt.Fprintf(&out, "        %sChunk%d.init();\n", e.prefix, i)
		}
		out.WriteString("    }\n}\n")
	}
