// Code generated by goplus from regex_parse.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func buildRegexClassesJava() string {
	...
	for _, name := range ordered {
		...
		for _, fold := range []bool{false, true} {
			...
			node, err := syntax.Parse(source, syntax.Perl)
			...
			if node.Op == syntax.OpLiteral && len(node.Rune) == 1 {
				...
				if node.Flags&syntax.FoldCase != 0 {
					for f := unicode.SimpleFold(point); f != point; f = unicode.SimpleFold(f) {
				...
				for _, r := range points {
					node.Rune = append(node.Rune, rune(r), rune(r))
				}
			} else if node.Op == syntax.OpAnyChar {
				node.Rune = []rune{0, unicode.MaxRune}
			} else if node.Op == syntax.OpAnyCharNotNL {
				node.Rune = []rune{0, 9, 11, unicode.MaxRune}
			} else if node.Op != syntax.OpCharClass {
				panic(fmt.Sprintf("Unicode class %s has op %v", source, node.Op))
			}
			fmt.Fprintf(&data, "%s:%t:", canonical, fold)
			for i, r := range node.Rune {
				if i > 0 {
					data.WriteByte(',')
				}
				fmt.Fprint(&data, r)
	...
	chunks := []string{}
	for len(raw) > 8000 {
		chunks = append(chunks, javaQuote(raw[:8000]))
		raw = raw[8000:]
	}
	chunks = append(chunks, javaQuote(raw))
