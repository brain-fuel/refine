...
func TestGeneratedCodePointLengthRuntimeParityAndNativeStringLengthSerde(t *testing.T) {
	program, err := language.Compile(codePointLengthContract)
	if err != nil {
		t.Fatal(err)
	}
	shadow, err := language.Compile(codePointLengthShadowContract)
	if err != nil {
		t.Fatal(err)
	}
	files, err := GenerateValidator(program, "example.codepoints", "Contract")
	if err != nil {
		t.Fatal(err)
	}
	shadowFiles, err := GenerateValidator(shadow, "example.codepointshadow", "Contract")
	if err != nil {
		t.Fatal(err)
	}
	files = append(files, shadowFiles...)
	schema := `{"$schema":"https://json-schema.org/draft/2020-12/schema","type":"string","minLength":1.0,"maxLength":3}`
	project, err := native.IngestProject(native.JSONSchema, []byte(schema), native.ProjectOptions{Root: native.ResourceSelector{TypeName: "Text"}, Metadata: native.WireMetadata{PublicationNamespace: "example.codepointnative"}})
	if err != nil {
		t.Fatal(err)
	}
	minimum := ""
	for _, item := range project.NativeConstraints() {
		if item.Constraint.Keyword == "minLength" {
			minimum = item.Constraint.Predicate
		}
	}
	if minimum != "((codePointLength it) >= 1)" {
		t.Fatalf("unexpected minLength provenance: %q", minimum)
	}
	source := project.ResourceConstraintSource("urn:refine:root")
	editedMinimum := strings.Replace(minimum, ">= 1", ">= 2", 1)
	changed := strings.Replace(source, minimum, editedMinimum, 1)
	if editedMinimum == minimum || changed == source {
		t.Fatal("minLength edit was not applied")
	}
	project, err = project.WithEditedNativeConstraintSource("urn:refine:root", changed)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := project.ValidateJSON([]byte(`"😀"`)); err == nil {
		t.Fatal("edited native minLength was not enforced")
	}
	nativeFiles, err := GenerateProjectJSONSerde(project, "Contract", "TextModule")
	if err != nil {
		t.Fatal(err)
	}
	...
	harness := filepath.Join(dir, "CodePointLengthHarness.java")
	if err := os.WriteFile(harness, []byte(codePointLengthHarnessJava), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if output, err := exec.CommandContext(compileContext, compiler, args...).CombinedOutput(); err != nil {
		if compileContext.Err() != nil {
			t.Fatalf("codePointLength javac exceeded the process deadline: %v", compileContext.Err())
		}
		t.Fatalf("codePointLength javac: %v\n%s", err, output)
	}

	vectors := []vector{}
	add := func(contract, root string, data value.Data, limits validation.Limits) {
		owner := program
		if contract == "shadow" {
			owner = shadow
		}
		text, ok := data.Text()
		if !ok {
			t.Fatal("codePointLength vector is not text")
		}
		vectors = append(vectors, vector{fmt.Sprintf("%s\t%s\t%s\t%d\t%d", contract, root, unitsHex(text.Units()), limits.Total, limits.Clause), reportLine(owner.ValidateData(root, data, limits))})
	}
	semantic := []struct {
		root  string
		data  value.Data
	...
	for _, item := range semantic {
		report := program.ValidateData(item.root, item.data, validation.Limits{})
		if (validation.StateName(report.State()) == "valid") != item.valid || report.Incomplete() {
			t.Fatalf("bad semantic fixture %s: %+v", item.root, report.Diagnostics())
		}
		add("base", item.root, item.data, validation.Limits{})
	}
	for _, data := range []value.Data{codePointText('a'), codePointText(0xd83d, 0xde00), codePointText(0xd83d, 0xde00, 0xd83d, 0xde01), codePointText(0xd800)} {
		for limit := uint64(1); limit <= 20; limit++ {
			add("base", "One", data, validation.Limits{Total: limit})
			add("base", "One", data, validation.Limits{Clause: limit})
		}
	}
	add("shadow", "Shadowed", codePointText(0xd83d, 0xde00), validation.Limits{})
	add("shadow", "Shadowed", codePointText(0xd800), validation.Limits{})
	...
	if err != nil {
		if runContext.Err() != nil {
			t.Fatalf("codePointLength JVM exceeded the process deadline: %v", runContext.Err())
		}
		t.Fatalf("codePointLength JVM: %v\n%s", err, stderr.String())
	}
