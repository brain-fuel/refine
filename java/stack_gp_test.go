// Code generated by goplus from stack_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func TestGeneratedJavaStackSafety(t *testing.T) {
	...
	chain := func(depth int, last string) value.Data {
		...
		for i := 0; i < depth; i++ {
			result = testRecord(value.DataField{Name: "value", Value: testNumber("0")}, value.DataField{Name: "next", Value: testVariant("Just", result)})
		}
		return result
	}
	for _, kind := range []string{"equality", "expression"} {
		t.Run(kind, func(t *testing.T) {
			source := "type Chain = { value :: Int, next :: Maybe Chain }\ntype Check = { left :: Chain, right :: Chain } where it.left == it.right"
			if kind == "expression" {
				source = "type Check = Int where " + strings.Repeat("it + ", 159) + "it == it * 160"
			}
			program, err := language.Compile(source)
			if err != nil {
				t.Fatal(err)
			}
			files, err := GenerateValidator(program, "example.stack", "Contract")
			if err != nil {
				t.Fatal(err)
			}
			root := t.TempDir()
			...
			harness := filepath.Join(root, "StackConformance.java")
			if err := os.WriteFile(harness, []byte(stackHarnessJava), 0644); err != nil {
				t.Fatal(err)
			}
			...
			add := func(depth int, last string, total, clause uint64) {
				...
				if kind == "equality" {
					data = testRecord(value.DataField{Name: "left", Value: chain(depth, "0")}, value.DataField{Name: "right", Value: chain(depth, last)})
				}
				for _, bypass := range []bool{false, true} {
					limits := validation.Limits{Total: total, Clause: clause}
					report := program.ValidateData("Check", data, limits)
					if bypass {
						report = program.ValidateDataWithoutRefinements("Check", data, limits)
					}
					if total == 0 && clause == 0 && (kind == "expression" || depth <= 120) {
						expected := "valid"
						if kind == "equality" && last != "0" && !bypass {
							expected = "invalid"
						}
						if validation.StateName(report.State()) != expected || report.Incomplete() {
							t.Fatalf("in-policy case unexpectedly limited: %s depth %d: %s", kind, depth, reportLine(report))
						}
					}
			...
			if kind == "equality" {
				for _, depth := range []int{0, 1, 40, 80, 120, 160, 169, 170, 180, 260, 512} {
					for _, last := range []string{"0", "1"} {
						for _, limit := range []uint64{0, 1, 40, 500, 2000, 10000} {
							add(depth, last, limit, 0)
							add(depth, last, 0, limit)
						}
					}
				}
			} else {
				for _, number := range []string{"0", "1", "-123456789"} {
					for limit := uint64(1); limit <= 1500; limit++ {
						add(0, number, limit, 0)
						add(0, number, 0, limit)
					}
					add(0, number, 0, 0)
				}
			}
			...
			defer cancel()
			command := exec.CommandContext(ctx, vm, "-Xss256k", "-cp", classes, "StackConformance")
