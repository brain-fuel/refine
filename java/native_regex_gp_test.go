...
func graalJSClasspath(t *testing.T) string {
	t.Helper()
	dir := os.Getenv("REFINE_GRAALJS_DIR")
	if dir == "" {
		if os.Getenv("REFINE_REQUIRE_JAVA") == "1" {
			t.Fatal("REFINE_GRAALJS_DIR is required by the Java release gate")
		}
		t.Skip("set REFINE_GRAALJS_DIR to pinned GraalJS Community 25.0.1 jars")
	}
	...
	for _, jar := range jars {
		...
		if err != nil {
			t.Fatalf("GraalJS test dependency: %v", err)
		}
		sum := sha256.Sum256(data)
		if hex.EncodeToString(sum[:]) != jar.sum {
			t.Fatalf("GraalJS test dependency %s has an unexpected SHA-256", jar.name)
		}
...
func TestNativeRegexEmissionIsConditional(t *testing.T) {
	...
	if strings.Contains(files[0].Source, "org.graalvm") || strings.Contains(files[0].Source, "RegexLimits") || strings.Contains(files[0].Source, "regularExpressionFactory") {
		t.Fatal("a non-regex schema acquired a GraalJS runtime dependency")
	}
