// Code generated by goplus from serde_avro_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func avroClasspath(t *testing.T) string {
	dir := os.Getenv("REFINE_AVRO_DIR")
	if dir == "" {
		if os.Getenv("REFINE_REQUIRE_JAVA") == "1" {
			t.Fatal("REFINE_AVRO_DIR is required by the Java release gate")
		}
		t.Skip("set REFINE_AVRO_DIR to pinned Apache Avro 1.12 test jars")
	}
	...
	for _, jar := range jars {
		...
		if err != nil {
			t.Fatalf("Avro test dependency: %v", err)
		}
		sum := sha256.Sum256(data)
		if hex.EncodeToString(sum[:]) != jar.sum {
			t.Fatalf("Avro test dependency %s has an unexpected SHA-256", jar.name)
		}
...
func TestGeneratedAvroBinaryJSONResolutionAndValidation(t *testing.T) {
	...
	schema := `{"type":"record","name":"Payload","aliases":["Old"],"fields":[{"name":"renamed","type":"long","aliases":["oldName"]},{"name":"added","type":"long","default":5},{"name":"accepted","type":"boolean","aliases":["valid"],"default":true}]}`
	project, err := native.IngestProject(native.Avro, []byte(schema), native.ProjectOptions{ResourceID: "urn:avro:new", Root: native.ResourceSelector{TypeName: "PayloadRoot"}, Metadata: native.WireMetadata{PublicationNamespace: "example.avro"}})
	if err != nil {
		t.Fatal(err)
	}
	project, err = project.WithEditedSource("type Accepted = Bool where it\ntype PayloadRoot = {renamed :: Int64, added :: Int64, accepted :: Accepted}\n")
	if err != nil {
		t.Fatal(err)
	}
	files, err := GenerateProjectAvroSerde(project, "Contract", "PayloadAvroSerde")
	if err != nil {
		t.Fatal(err)
	}
	dir := t.TempDir()
	...
	harness := filepath.Join(dir, "AvroGate.java")
	if err := os.WriteFile(harness, []byte(avroHarnessJava), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("Avro 1.12 javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-cp", classes+string(os.PathListSeparator)+classpath, "AvroGate").CombinedOutput(); err != nil {
		t.Fatalf("Avro 1.12 runtime: %v\n%s", err, output)
	}
}

func TestAvroSerdeRejectsUnboundedOrUnimplementedShapes(t *testing.T) {
	schema := `{"type":"record","name":"Wire","fields":[{"name":"value","type":"long"}]}`
	base, err := native.IngestProject(native.Avro, []byte(schema), native.ProjectOptions{ResourceID: "urn:avro:gate", Root: native.ResourceSelector{TypeName: "Payload"}})
	if err != nil {
		t.Fatal(err)
	}
	cases := []struct{ name, source string }{{name: "arbitrary", source: "type Payload = Int\n"}, {name: "real", source: "type Payload = Real\n"}}
	for _, tc := range cases {
		project, err := base.WithEditedSource(tc.source)
		if err != nil {
			t.Fatalf("%s fixture: %v", tc.name, err)
		}
		if files, err := GenerateProjectAvroSerde(project, "Contract", "Serde"); err == nil || files != nil {
			t.Fatalf("%s shape was accepted", tc.name)
		}
	}
}

func TestAvroSchemaSourcesAreChunkedBelowJavaConstantLimit(t *testing.T) {
	expression := avroJavaString(strings.Repeat("x", 70000))
	if !strings.HasPrefix(expression, "join(") || strings.Contains(expression, strings.Repeat("x", 65000)) || strings.Count(expression, "\",\"") < 10 {
		t.Fatal("large Avro schema was not split into runtime string chunks")
	}
}

func TestAvroSerdeBoundsExpandingGenericSchemaPair(t *testing.T) {
	schema := `{"type":"record","name":"GrowWire","fields":[{"name":"value","type":"int"},{"name":"next","type":"GrowWire"}]}`
	project, err := native.IngestProject(native.Avro, []byte(schema), native.ProjectOptions{Root: native.ResourceSelector{TypeName: "Payload"}})
	if err != nil {
		t.Fatal(err)
	}
	project, err = project.WithEditedSource("type Grow a = {value :: Int32, next :: Grow [a]}\ntype Payload = Grow Int32\n")
	if err != nil {
		t.Fatal(err)
	}
	if files, err := GenerateProjectAvroSerde(project, "Contract", "Serde"); err == nil || files != nil || !strings.Contains(err.Error(), "specialization limit") {
		t.Fatalf("expanding generic schema pair was not bounded: %v", err)
	}
}
...
func TestGeneratedAvroClosedGenericProjectPipeline(t *testing.T) {
	compiler, vm := javaTools(t)
	classpath := avroClasspath(t)
	source := "type Box a = {value :: a}\ntype Pair a b = {left :: Box a, right :: Box b}\ntype Swap a b = Pair b a\ntype Node a = {value :: a, children :: [Node a]}\ntype Positive = Int32 where fromInt32 it > 0 @code \"positive\"\ntype Payload = {swapped :: Swap String Positive, tree :: Node Positive}\n"
	program, err := language.Compile(source)
	if err != nil {
		t.Fatal(err)
	}
	payload, err := program.PayloadType("Payload")
	if err != nil {
		t.Fatal(err)
	}
	lowered, err := native.LowerPayload(native.Avro, payload, native.LowerOptions{Mode: native.Refined, AllowDocumentedLoss: true})
	if err != nil {
		t.Fatal(err)
	}
	if strings.Count(lowered.String(), `"name": "Payload"`) != 1 {
		t.Fatalf("monomorphic root wire name changed: %s", lowered.String())
	}
	project, err := native.IngestProject(native.Avro, lowered.Bytes(), native.ProjectOptions{ResourceID: "urn:avro:generic-project", Root: native.ResourceSelector{TypeName: "Payload"}})
	if err != nil {
		t.Fatal(err)
	}
	...
	project, err = project.WithMetadata(native.WireMetadata{PublicationNamespace: "example.genericavro"})
	if err != nil {
		t.Fatal(err)
	}
	files, err := GenerateProjectAvroSerde(project, "Contract", "GenericAvroSerde")
	if err != nil {
		t.Fatal(err)
	}
	dir := t.TempDir()
	...
	harness := filepath.Join(dir, "GenericAvroGate.java")
	if err := os.WriteFile(harness, []byte(genericAvroHarnessJava), 0644); err != nil {
		t.Fatal(err)
	}
	sources = append(sources, harness)
	classes := filepath.Join(dir, "classes")
	args := append([]string{"--release", "25", "-encoding", "UTF-8", "-Xlint:all", "-Werror", "-cp", classpath, "-d", classes}, sources...)
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("closed generic Avro javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-cp", classes+string(os.PathListSeparator)+classpath, "GenericAvroGate").CombinedOutput(); err != nil {
		t.Fatalf("closed generic Avro runtime: %v\n%s", err, output)
	}
}
...
func TestGeneratedAvroArraysAndRecursiveRecordsAreBounded(t *testing.T) {
	compiler, vm := javaTools(t)
	classpath := avroClasspath(t)
	schema := `{"type":"record","name":"Node","fields":[{"name":"value","type":"long"},{"name":"children","type":{"type":"array","items":"Node"}}]}`
	project, err := native.IngestProject(native.Avro, []byte(schema), native.ProjectOptions{ResourceID: "urn:avro:recursive", Root: native.ResourceSelector{TypeName: "NodeRoot"}, Metadata: native.WireMetadata{PublicationNamespace: "example.recursive"}})
	if err != nil {
		t.Fatal(err)
	}
	files, err := GenerateProjectAvroSerde(project, "Contract", "NodeAvroSerde")
	if err != nil {
		t.Fatal(err)
	}
	dir := t.TempDir()
	...
	harness := filepath.Join(dir, "RecursiveAvroGate.java")
	if err := os.WriteFile(harness, []byte(recursiveAvroHarnessJava), 0644); err != nil {
		t.Fatal(err)
	}
	sources = append(sources, harness)
	classes := filepath.Join(dir, "classes")
	args := append([]string{"--release", "25", "-encoding", "UTF-8", "-Xlint:all", "-Werror", "-cp", classpath, "-d", classes}, sources...)
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("recursive Avro javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-cp", classes+string(os.PathListSeparator)+classpath, "RecursiveAvroGate").CombinedOutput(); err != nil {
		t.Fatalf("recursive Avro runtime: %v\n%s", err, output)
	}
}
...
func TestGeneratedAvroNamedUnionPreservesWireBranchIdentity(t *testing.T) {
	...
	classpath := avroClasspath(t)
	schema := `["long","string"]`
	project, err := native.IngestProject(native.Avro, []byte(schema), native.ProjectOptions{ResourceID: "urn:avro:choice", Root: native.ResourceSelector{TypeName: "Choice"}, Metadata: native.WireMetadata{PublicationNamespace: "example.choice"}})
	if err != nil {
		t.Fatal(err)
	}
	files, err := GenerateProjectAvroSerde(project, "Contract", "ChoiceAvroSerde")
	if err != nil {
		t.Fatal(err)
	}
	dir := t.TempDir()
	...
	harness := filepath.Join(dir, "UnionAvroGate.java")
	if err := os.WriteFile(harness, []byte(unionAvroHarnessJava), 0644); err != nil {
		t.Fatal(err)
	}
	sources = append(sources, harness)
	classes := filepath.Join(dir, "classes")
	args := append([]string{"--release", "25", "-encoding", "UTF-8", "-Xlint:all", "-Werror", "-cp", classpath, "-d", classes}, sources...)
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("union Avro javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-cp", classes+string(os.PathListSeparator)+classpath, "UnionAvroGate").CombinedOutput(); err != nil {
		t.Fatalf("union Avro runtime: %v\n%s", err, output)
	}
}
...
func TestGeneratedAvroEnumBytesAndFixed(t *testing.T) {
	compiler, vm := javaTools(t)
	classpath := avroClasspath(t)
	schema := `{"type":"record","name":"Blob","fields":[{"name":"color","type":{"type":"enum","name":"Color","symbols":["RED","Blue"]}},{"name":"bytes","type":"bytes"},{"name":"fixed","type":{"type":"fixed","name":"Hash","size":4}}]}`
	project, err := native.IngestProject(native.Avro, []byte(schema), native.ProjectOptions{ResourceID: "urn:avro:blob", Root: native.ResourceSelector{TypeName: "BlobRoot"}, Metadata: native.WireMetadata{PublicationNamespace: "example.blob"}})
	if err != nil {
		t.Fatal(err)
	}
	...
	harness := filepath.Join(dir, "BlobAvroGate.java")
	if err := os.WriteFile(harness, []byte(blobAvroHarnessJava), 0644); err != nil {
		t.Fatal(err)
	}
	sources = append(sources, harness)
	classes := filepath.Join(dir, "classes")
	args := append([]string{"--release", "25", "-encoding", "UTF-8", "-Xlint:all", "-Werror", "-cp", classpath, "-d", classes}, sources...)
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("enum/bytes Avro javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-cp", classes+string(os.PathListSeparator)+classpath, "BlobAvroGate").CombinedOutput(); err != nil {
		t.Fatalf("enum/bytes Avro runtime: %v\n%s", err, output)
	}
}
...
func TestGeneratedAvroStringsRejectMalformedWireAndJavaText(t *testing.T) {
	compiler, vm := javaTools(t)
	classpath := avroClasspath(t)
	project, err := native.IngestProject(native.Avro, []byte(`"string"`), native.ProjectOptions{ResourceID: "urn:avro:text", Root: native.ResourceSelector{TypeName: "TextRoot"}, Metadata: native.WireMetadata{PublicationNamespace: "example.text"}})
	if err != nil {
		t.Fatal(err)
	}
	files, err := GenerateProjectAvroSerde(project, "Contract", "TextAvroSerde")
	if err != nil {
		t.Fatal(err)
	}
	dir := t.TempDir()
	...
	harness := filepath.Join(dir, "TextAvroGate.java")
	if err := os.WriteFile(harness, []byte(textAvroHarnessJava), 0644); err != nil {
		t.Fatal(err)
	}
	sources = append(sources, harness)
	classes := filepath.Join(dir, "classes")
	args := append([]string{"--release", "25", "-encoding", "UTF-8", "-Xlint:all", "-Werror", "-cp", classpath, "-d", classes}, sources...)
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("text Avro javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-cp", classes+string(os.PathListSeparator)+classpath, "TextAvroGate").CombinedOutput(); err != nil {
		t.Fatalf("text Avro runtime: %v\n%s", err, output)
	}
}
...
func TestGeneratedAvroExplicitExactScalarPolicies(t *testing.T) {
	compiler, vm := javaTools(t)
	classpath := avroClasspath(t)
	schema := `{"type":"record","name":"Scalars","fields":[{"name":"big","type":"string"},{"name":"moment","type":"string"},{"name":"ratio","type":{"type":"record","name":"RatioWire","fields":[{"name":"numerator","type":"bytes"},{"name":"denominator","type":"bytes"}]}},{"name":"money","type":{"type":"bytes","logicalType":"decimal","precision":6,"scale":2}},{"name":"uuid","type":{"type":"string","logicalType":"uuid"}},{"name":"time","type":{"type":"int","logicalType":"time-millis"}}]}`
	project, err := native.IngestProject(native.Avro, []byte(schema), native.ProjectOptions{ResourceID: "urn:avro:scalars", Root: native.ResourceSelector{TypeName: "ScalarsRoot"}})
	if err != nil {
		t.Fatal(err)
	}
	project, err = project.WithEditedSource("type Big = Int\ntype Moment = Timestamp\ntype Ratio = Real\ntype Money = Real\ntype ScalarsRoot = {big :: Big, moment :: Moment, ratio :: Ratio, money :: Money, uuid :: String, time :: Int32}\n")
	if err != nil {
		t.Fatal(err)
	}
	project, err = project.WithMetadata(native.WireMetadata{PublicationNamespace: "example.scalars", Scalars: map[string]native.ScalarEncoding{"Big": {Kind: native.DecimalString}, "Moment": {Kind: native.TimestampString}, "Ratio": {Kind: native.RationalRecord}, "Money": {Kind: native.AvroBytesDecimal, Precision: 6, Scale: 2}}})
	if err != nil {
		t.Fatal(err)
	}
	files, err := GenerateProjectAvroSerde(project, "Contract", "ScalarsAvroSerde")
	if err != nil {
		t.Fatal(err)
	}
	dir := t.TempDir()
	...
	harness := filepath.Join(dir, "ScalarAvroGate.java")
	if err := os.WriteFile(harness, []byte(scalarAvroHarnessJava), 0644); err != nil {
		t.Fatal(err)
	}
	sources = append(sources, harness)
	classes := filepath.Join(dir, "classes")
	args := append([]string{"--release", "25", "-encoding", "UTF-8", "-Xlint:all", "-Werror", "-cp", classpath, "-d", classes}, sources...)
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("scalar Avro javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-cp", classes+string(os.PathListSeparator)+classpath, "ScalarAvroGate").CombinedOutput(); err != nil {
		t.Fatalf("scalar Avro runtime: %v\n%s", err, output)
	}
}
...
func TestGeneratedAvroFloatDoubleOnlyAcceptExactFiniteReals(t *testing.T) {
	compiler, vm := javaTools(t)
	classpath := avroClasspath(t)
	schema := `{"type":"record","name":"Floating","fields":[{"name":"single","type":"float"},{"name":"double_","type":"double"}]}`
	project, err := native.IngestProject(native.Avro, []byte(schema), native.ProjectOptions{ResourceID: "urn:avro:floating", Root: native.ResourceSelector{TypeName: "FloatingRoot"}, Metadata: native.WireMetadata{PublicationNamespace: "example.floating"}})
	if err != nil {
		t.Fatal(err)
	}
	files, err := GenerateProjectAvroSerde(project, "Contract", "FloatingAvroSerde")
	if err != nil {
		t.Fatal(err)
	}
	dir := t.TempDir()
	...
	harness := filepath.Join(dir, "FloatingAvroGate.java")
	if err := os.WriteFile(harness, []byte(floatingAvroHarnessJava), 0644); err != nil {
		t.Fatal(err)
	}
	sources = append(sources, harness)
	classes := filepath.Join(dir, "classes")
	args := append([]string{"--release", "25", "-encoding", "UTF-8", "-Xlint:all", "-Werror", "-cp", classpath, "-d", classes}, sources...)
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("floating Avro javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-cp", classes+string(os.PathListSeparator)+classpath, "FloatingAvroGate").CombinedOutput(); err != nil {
		t.Fatalf("floating Avro runtime: %v\n%s", err, output)
	}
}
