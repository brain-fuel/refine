// Code generated by goplus from models.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func integerType(name string) bool {
	...
	return err == nil && n > 0 && strconv.FormatUint(n, 10) == digits
}
func (m *modelEmitter) fresh() string {
	for {
		m.next++
		name := fmt.Sprintf("_refine%d", m.next)
		if !m.locals[name] {
			m.locals[name] = true
			return name
		}
...
func (m *modelEmitter) qualified(name string) string {
	if m.namespace == "" {
		return name
	}
...
func (m *modelEmitter) shape(name string) *language.Type {
	...
	for {
		if seen[name] {
			unsupported(m.declarations[name].At, "cyclic model alias has no structural representation")
		}
		seen[name] = true
...
func applied(t *language.Type) (string, []*language.Type) {
	...
	case language.NamedType:
		...
		return name, args
	default:
		unsupported(t.At, "unsupported model type application")
	}
	return "", nil
}
func (m *modelEmitter) javaType(t *language.Type) string {
	...
	case language.ListType:
		...
		return "java.util.List<" + m.javaType(element) + ">"
	case language.AppliedType:
		...
		for _, arg := range args {
			types = append(types, m.javaType(arg))
		}
...
func (m *modelEmitter) encode(t *language.Type, input, location string) string {
	...
	case language.ListType:
		...
		return "ModelSupport.list(" + input + ",(" + item + "," + where + ") -> " + m.encode(element, item, where) + "," + location + ")"
	case language.AppliedType:
		...
		for _, arg := range args {
			item, where := m.fresh(), m.fresh()
			pieces = append(pieces, "("+item+","+where+") -> "+m.encode(arg, item, where))
		}
...
func (m *modelEmitter) decode(t *language.Type, input string) string {
	...
	case language.ListType:
		...
		item := m.fresh()
		return "ModelSupport.list(" + input + "," + item + " -> " + m.decode(element, item) + ")"
	case language.AppliedType:
...
func fieldIdentifier(name string) string {
	if packageName(name) == nil && !strings.Contains(name, ".") {
		return name
	}
	var out strings.Builder
	for i, r := range name {
		valid := unicode.IsLetter(r) || unicode.In(r, unicode.Nl, unicode.Sc, unicode.Pc)
		if i > 0 {
			valid = valid || unicode.IsDigit(r) || unicode.In(r, unicode.Mn, unicode.Mc)
		}
		if valid {
			out.WriteRune(r)
		} else {
...
func (m *modelEmitter) recordFields(root string, t *language.Type) []modelField {
	...
	case language.RecordType:
		...
		for _, field := range fields {
			...
			used[member] = true
			m.locals[member] = true
			setter := "set" + upperFirst(member)
			for setters[setter] {
				setter += "_"
			}
			setters[setter] = true
			m.javaType(field.Type)
			result = append(result, modelField{name: field.Name, member: member, setter: setter, typ: field.Type})
		}
	default:
		panic("recordFields requires a record")
	}
	m.fields[root] = result
	return result
}
func (m *modelEmitter) rawRecord(fields []modelField) string {
	...
	for _, field := range fields {
		...
		values = append(values, "new Data.Field("+javaQuote(field.name)+","+m.encode(field.typ, field.member, javaQuote(location))+")")
	}
...
func (m *modelEmitter) model(decl language.TypeDecl) string {
	...
	fields := []modelField{}
	if record {
		fields = m.recordFields(root, shape)
	} else {
		m.javaType(shape)
	}
	modifier := "final"
	...
	if parent != "" {
		out.WriteString("        super(evidence);\n")
	...
	if record {
		for _, field := range fields {
			parameters = append(parameters, m.javaType(field.typ)+" "+field.member)
			arguments = append(arguments, field.member)
		}
		raw = m.rawRecord(fields)
	} else {
		parameters = append(parameters, m.javaType(shape)+" value")
	...
	if record {
		...
		if parent == "" {
			...
			for range fields {
				...
				fmt.Fprintf(&out, "        private boolean %s;\n", flag)
			}
			...
			for i, field := range fields {
				fmt.Fprintf(&out, "        public void %s(%s value) { this.%s = value; this.%s = true; }\n", field.setter, m.javaType(field.typ), field.member, changed[i])
			}
...
func GenerateModels(program *language.Program, namespace, contractName string) (files []File, failure error) {
	...
	files, failure = GenerateValidator(program, namespace, contractName)
	if failure != nil {
		return nil, failure
	}
	...
	for _, decl := range m.module.Types {
		if parent := m.parents[decl.Name]; parent != "" {
...
func prepareModelEmitter(program *language.Program, namespace, contractName string, files []File) *modelEmitter {
	...
	for _, decl := range m.module.Types {
		if err := javaClassName(decl.Name); err != nil {
			unsupported(decl.At, err.Error())
		}
		if decl.Name == contractName {
			unsupported(decl.At, "model name conflicts with the chosen contract class")
		}
		folded := sourceNameKey(decl.Name + ".java")
		if sourceNames[folded] {
