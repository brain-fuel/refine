// Code generated by goplus from local_generics_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
`
	...
	files, err := GenerateModels(program, "example.locals", "Contract")
	if err != nil {
		t.Fatal(err)
	}
	root := t.TempDir()
	...
	if err := os.WriteFile(harness, []byte(strings.ReplaceAll(conversionHarnessJava, "example.conversions", "example.locals")), 0600); err != nil {
		t.Fatal(err)
	}
	...
	for _, sample := range []struct {
		name string
		raw  string
	}{{"IntegerBox", "{value = 1}"}, {"BooleanBox", "{value = True}"}, {"Number", "4"}, {"Number", "-7"}, {"Message", "9007199254740993"}} {
		text, err := value.TextFromUTF8(sample.raw)
		if err != nil {
			t.Fatal(err)
		}
		for budget := uint64(0); budget < 350; budget++ {
			for _, limits := range []validation.Limits{{Total: budget}, {Clause: budget}} {
				_, report := program.ReadData(sample.name, text, limits)
				vectors = append(vectors, vector{fmt.Sprintf("%s\t%s\t%d\t%d", sample.name, readUnits(text), limits.Total, limits.Clause), reportLine(report)})
			}
		}
	}
