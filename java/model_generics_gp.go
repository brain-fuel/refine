...
func (m *modelEmitter) genericModel(decl language.TypeDecl) string {
	...
	shape := unrefined(decl.Body)
	fields := []modelField{}
	record := false
	switch any(shape.Form).(type) {
	case language.RecordType:
		record = true
		fields = m.recordFields(name, shape)
	default:
		m.javaType(shape)
	}
	...
	out.WriteString("    }\n")
	ctorParams, ctorArgs := []string{}, []string{}
	raw := ""
	if record {
		for _, field := range fields {
			ctorParams = append(ctorParams, m.javaType(field.typ)+" "+field.member)
			ctorArgs = append(ctorArgs, field.member)
		}
		raw = m.rawRecord(fields)
	} else {
		ctorParams = []string{m.javaType(shape) + " value"}
		ctorArgs = []string{"value"}
	...
	if record {
		...
		fmt.Fprintf(&out, "    public %s value() { return %s; }\n", m.javaType(shape), m.decode(shape, "$raw"))
	}
	out.WriteString("}\n")
	return out.String()
}
