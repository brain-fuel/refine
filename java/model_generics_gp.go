// Code generated by goplus from model_generics.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func (m *modelEmitter) genericModel(decl language.TypeDecl) string {
	...
	m.useFrame(root)
	shape := unrefined(root.declaration.Body)
	fields := []modelField{}
	record := false
	switch any(shape.Form).(type) {
	case language.RecordType:
		record = true
		fields = m.recordFields(root.declaration.Name, shape)
	default:
		m.javaType(shape)
	}
	fieldParams, fieldArgs := []string{}, []string{}
	raw := ""
	if record {
		for _, field := range fields {
			fieldParams = append(fieldParams, m.javaType(field.typ)+" "+field.member)
			fieldArgs = append(fieldArgs, field.member)
		}
		raw = m.rawRecord(fields)
	} else {
		fieldParams = []string{m.javaType(shape) + " value"}
		fieldArgs = []string{"value"}
		raw = m.encode(shape, "value", `""`)
	}
	m.useFrame(self)
	...
	parent := m.parents[name]
	modifier, inheritance, permits := "final", "", ""
	if len(m.children[name]) > 0 {
		modifier = "sealed"
		permits = " permits " + strings.Join(m.children[name], ", ")
	}
	if parent != "" {
		inheritance = " extends " + m.qualified(parent) + genericSuffix(frames[1].typeArguments)
	}
	...
	fmt.Fprintf(&out, "public %s class %s%s%s {\n", modifier, full, inheritance, permits)
	if parent == "" {
	...
	if parent != "" {
		...
		fmt.Fprintf(&out, "        this.$raw=evidence.dataFor(%s);\n", target)
	}
	...
	if parent == "" {
		...
		if record {
			...
			fmt.Fprintf(&out, "    public final %s value() { return %s; }\n", m.javaType(shape), m.decode(shape, "$raw"))
		}
		m.useFrame(self)
	}
	draftName := m.modelDraftName()
	draft := m.qualified(root.declaration.Name) + "." + draftName + genericSuffix(root.typeArguments)
	newDraft := func(raw string) string {
		return m.qualified(root.declaration.Name) + ".$newDraft(" + strings.Join(append(append([]string{}, root.arguments...), raw), ",") + ")"
	}
	if record {
	...
	for _, ending := range []string{"", "WithoutValidation"} {
		...
		if record {
			staticWrapper("create"+ending, full, []string{"java.util.function.Consumer<" + draft + "> initialize"}, []string{"initialize"})
			staticWrapper("create"+ending, full, []string{"java.util.function.Consumer<" + draft + "> initialize", "Budget.Limits caller"}, []string{"initialize", "caller"})
		}
	}
	if positional {
		staticWrapper("createWithoutValidation", full, fieldParams, fieldArgs)
		staticWrapper("createWithoutValidation", full, append(append([]string{}, fieldParams...), "Budget.Limits caller"), append(append([]string{}, fieldArgs...), "caller"))
	}
	staticWrapper("read", full, []string{"String text"}, []string{"text"})
	staticWrapper("read", full, []string{"String text", "Budget.Limits caller"}, []string{"text", "caller"})
	staticWrapper("validateData", "Validation.Outcome", []string{"Data raw"}, []string{"raw"})
	staticWrapper("validateData", "Validation.Outcome", []string{"Data raw", "Budget.Limits caller"}, []string{"raw", "caller"})
	...
	if parent == "" && record {
		out.WriteString(m.genericDraft(decl, fields))
	}
	out.WriteString("}\n")
	return out.String()
}
...
func (m *modelEmitter) genericDraft(decl language.TypeDecl, fields []modelField) string {
	...
	args := append(append([]string{}, arguments...), "baseline")
	...
	for i := 0; i < len(fields); i += 64 {
		fmt.Fprintf(&out, "            $fields%d(changes);\n", i/64)
	}
	...
	for start := 0; start < len(fields); start += 64 {
		...
		for i := start; i < min(start+64, len(fields)); i++ {
			field := fields[i]
			location := "/" + strings.ReplaceAll(strings.ReplaceAll(field.name, "~", "~0"), "/", "~1")
			fmt.Fprintf(&out, "            if ($changed%d) changes.put(%s,%s);\n", i, javaQuote(field.name), m.encode(field.typ, "this."+field.member, javaQuote(location)))
		}
		out.WriteString("        }\n")
	}
	out.WriteString("    }\n")
