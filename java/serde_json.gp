...
func GenerateProjectJSONSerde(project *native.Project,contractName,moduleName string)([]File,error){
    if project==nil{return nil,&GenerationError{Message:"a checked native project is required"}};program,err:=language.Compile(project.EditableSource());if err!=nil{return nil,err};metadata:=project.Metadata();root:=project.Root().TypeName;options:=JSONSerdeOptions{Root:root}
    options.PreserveExtraFieldsFor=map[string]bool{};for name,mode:=range metadata.ExtraFields{if mode==native.PreserveExtraFields{options.PreserveExtraFieldsFor[name]=true}};options.Discriminators=map[string]JSONDiscriminator{};for name,wire:=range metadata.Discriminators{values:=map[string]string{};arguments:=map[string][]string{};for constructor,value:=range wire.Values{values[constructor]=value};for constructor,names:=range wire.Arguments{arguments[constructor]=append([]string(nil),names...)};options.Discriminators[name]=JSONDiscriminator{Field:wire.Field,Values:values,Arguments:arguments}};if metadata.ExtraFields[root]==native.PreserveExtraFields{options.PreserveExtraFields=true};if scalar,ok:=metadata.Scalars[root];ok{switch scalar.Kind{case native.JSONNumber:options.Integers=JSONIntegerNumber;options.Reals=JSONRealExactDecimal;case native.DecimalString:options.Integers=JSONIntegerString;options.Reals=JSONRealRationalString;case native.TimestampString:options.Timestamps=JSONTimestampRFC3339String;default:return nil,&GenerationError{Message:"native scalar encoding is not supported by Jackson JSON serde"}}}
