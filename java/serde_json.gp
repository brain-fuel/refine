...
func JSONSerdeOptionsFromMetadata(root string,metadata native.WireMetadata)(JSONSerdeOptions,error){
    if root==""{return JSONSerdeOptions{},&GenerationError{Message:"JSON serde root is required"}};options:=JSONSerdeOptions{Root:root,PreserveExtraFieldsFor:map[string]bool{},Discriminators:map[string]JSONDiscriminator{},Scalars:map[string]native.ScalarEncoding{},NumericExpansion:metadata.NumericExpansion}
    for name,mode:=range metadata.ExtraFields{if mode==native.PreserveExtraFields{options.PreserveExtraFieldsFor[name]=true}else if mode!=native.DiscardExtraFields{return JSONSerdeOptions{},&GenerationError{Message:"unsupported native extra-field policy for "+name}}};for name,wire:=range metadata.Discriminators{values:=map[string]string{};arguments:=map[string][]string{};for constructor,value:=range wire.Values{values[constructor]=value};for constructor,names:=range wire.Arguments{arguments[constructor]=append([]string(nil),names...)};options.Discriminators[name]=JSONDiscriminator{Field:wire.Field,Values:values,Arguments:arguments}}
    if metadata.ExtraFields[root]==native.PreserveExtraFields{options.PreserveExtraFields=true};for name,scalar:=range metadata.Scalars{switch scalar.Kind{case native.JSONNumber:if name==root{options.Integers=JSONIntegerNumber};case native.DecimalString:if name==root{options.Integers=JSONIntegerString};case native.TimestampString:if name==root{options.Timestamps=JSONTimestampRFC3339String};case native.RationalRecord:default:return JSONSerdeOptions{},&GenerationError{Message:"native scalar encoding is not supported by Jackson JSON serde"}};options.Scalars[name]=scalar};return options,nil
}
...
func GenerateJSONSerde(program *language.Program,namespace,contractName,moduleName string,options JSONSerdeOptions)(files []File,failure error){
    ...
    root,ok:=decls[options.Root];if !ok{unsupported(language.Span{},"unknown JSON serde root "+options.Root)};if len(root.Parameters)>0{unsupported(root.At,"JSON serde roots must be closed; register an instantiated nominal declaration")};emitter:=&jsonShapeEmitter{decls:decls,options:options,definitions:map[string]string{},building:map[string]bool{}}
    shape:="";if root.Body==nil{shape=emitter.unionDecl(root.Name,root.Name,root.Variants,nil,0)}else{body:=jsonResolve(root.Body,decls,map[string]bool{});match body.Form{case language.RecordType(fields):shape=emitter.record(options.Root,options.Root,fields,0);case _:shape=emitter.shape(&language.Type{Form:language.NamedType(root.Name),At:root.At},0)}};definitionParts:=[]string{};for _,key:=range emitter.order{definitionParts=append(definitionParts,"java.util.Map.entry("+javaQuote(key)+","+emitter.definitions[key]+")")};definitions:="java.util.Map.ofEntries("+strings.Join(definitionParts,",")+")"
...
func (e *jsonShapeEmitter) preserve(name string)bool{return (name==e.options.Root&&e.options.PreserveExtraFields)||e.options.PreserveExtraFieldsFor[name]}
func (e *jsonShapeEmitter) reference(key string)string{return "new S(\"ref\",null,null,"+javaQuote(key)+",false,null)"}
func (e *jsonShapeEmitter) record(key,name string,fields []language.Field,depth int)string{if _,ok:=e.definitions[key];ok{return e.reference(key)};e.definitions[key]="";e.building[key]=true;e.order=append(e.order,key);parts:=[]string{};for _,field:=range fields{parts=append(parts,"new F("+javaQuote(field.Name)+","+e.shape(field.Type,depth+1)+")")};e.definitions[key]="new S(\"record\",null,java.util.List.of("+strings.Join(parts,",")+"),null,"+fmt.Sprint(e.preserve(name))+",null)";delete(e.building,key);return e.reference(key)}
...
func (e *jsonShapeEmitter) union(key,name string,variants []jsonVariantShape,depth int)string{if _,ok:=e.definitions[key];ok{return e.reference(key)};wire,ok:=e.options.Discriminators[name];if !ok||wire.Field==""{unsupported(language.Span{},"tagged-union JSON serde requires explicit discriminator metadata for "+name)};if len(wire.Values)!=len(variants)||len(wire.Arguments)!=len(variants){unsupported(language.Span{},"discriminator metadata must cover every constructor of "+name)};e.definitions[key]="";e.order=append(e.order,key);seenTags:=map[string]bool{};parts:=[]string{};for _,variant:=range variants{tag,ok:=wire.Values[variant.name];if !ok||tag==""||seenTags[tag]{unsupported(language.Span{},"missing or duplicate discriminator value for "+name+"."+variant.name)};seenTags[tag]=true;names,ok:=wire.Arguments[variant.name];if !ok||len(names)!=len(variant.arguments){unsupported(language.Span{},"constructor argument names have wrong arity for "+name+"."+variant.name)};seenNames:=map[string]bool{wire.Field:true};nameParts:=[]string{};shapeParts:=[]string{};for i,argument:=range variant.arguments{if names[i]==""||seenNames[names[i]]{unsupported(argument.At,"duplicate or empty union JSON member name")};seenNames[names[i]]=true;nameParts=append(nameParts,javaQuote(names[i]));shapeParts=append(shapeParts,e.shape(argument,depth+1))};parts=append(parts,"new V("+javaQuote(variant.name)+","+javaQuote(tag)+",java.util.List.of("+strings.Join(nameParts,",")+"),java.util.List.of("+strings.Join(shapeParts,",")+"))")};e.definitions[key]="new S(\"union\",null,null,null,false,new U("+javaQuote(wire.Field)+",java.util.List.of("+strings.Join(parts,",")+")))";return e.reference(key)}
func (e *jsonShapeEmitter) shape(t *language.Type,depth int)string{
    e.visit(t,depth);t=unrefined(t);match t.Form{
    case language.NamedType(name):
        if shape,ok:=e.namedScalar(name,t.At);ok{return shape}
        if name=="Float32"||name=="Float64"{return "new S(\"float-number\",null,null,null,false,null)"}
        if integerType(name){return "new S(\"integer\",null,null,null,false,null)"};switch name{case "String":return "new S(\"text\",null,null,null,false,null)";case "Bool":return "new S(\"bool\",null,null,null,false,null)";case "Real":if e.options.Reals==JSONRealUnspecified{unsupported(t.At,"exact Real JSON encoding policy is required")};return "new S(\"real\",null,null,null,false,null)";case "Timestamp":if e.options.Timestamps!=JSONTimestampRFC3339String{unsupported(t.At,"Timestamp JSON encoding policy is required")};return "new S(\"timestamp\",null,null,null,false,null)"}
        if name=="JSON"{return "new S(\"json\",null,null,null,false,null)"}
        decl,ok:=e.decls[name];if !ok{unsupported(t.At,"unknown JSON field type")};if len(decl.Parameters)>0{unsupported(t.At,"generic JSON field requires type arguments")};if decl.Body==nil{return e.unionDecl(name,name,decl.Variants,nil,depth)};base:=unrefined(decl.Body);match base.Form{case language.RecordType(fields):return e.record(name,name,fields,depth);case _:return e.shape(base,depth+1)}
    case language.RecordType(fields):return e.record("@anonymous:"+fmt.Sprint(t.At.Start.Offset),"",fields,depth)
    case language.ListType(element):return "new S(\"list\","+e.shape(element,depth+1)+",null,null,false,null)"
    case language.AppliedType(fn,arg):if fn==nil||arg==nil{unsupported(t.At,"invalid generic JSON field")};name,args:=applied(t);if name=="Maybe"&&len(args)==1{return "new S(\"maybe\","+e.shape(args[0],depth+1)+",null,null,false,null)"};if name=="Nullable"&&len(args)==1{return "new S(\"nullable\","+e.shape(args[0],depth+1)+",null,null,false,null)"};if name=="Map"&&len(args)==2{return "new S(\"map\","+e.shape(args[1],depth+1)+",null,null,false,null)"};key:=jsonTypeKey(t,0);if name=="Result"&&len(args)==2{return e.union(key,"Result",[]jsonVariantShape{{name:"Err",arguments:args[:1]},{name:"Ok",arguments:args[1:]}},depth)};decl,ok:=e.decls[name];if !ok{unsupported(t.At,"unknown generic JSON field")};if len(args)!=len(decl.Parameters){unsupported(t.At,"generic JSON type argument count mismatch")};bindings:=map[string]*language.Type{};for i,param:=range decl.Parameters{bindings[param]=args[i]};if decl.Body==nil{return e.unionDecl(key,name,decl.Variants,bindings,depth)};closed,err:=language.SubstituteType(decl.Body,bindings);if err!=nil{unsupported(t.At,err.Error())};base:=unrefined(closed);match base.Form{case language.RecordType(fields):return e.record(key,name,fields,depth);case _:return e.shape(base,depth+1)}
