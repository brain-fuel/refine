...
func GenerateProjectJSONSerde(project *native.Project,contractName,moduleName string)([]File,error){
    if project==nil{return nil,&GenerationError{Message:"a checked native project is required"}};program,err:=language.Compile(project.EditableSource());if err!=nil{return nil,err};metadata:=project.Metadata();root:=project.Root().TypeName;options,err:=JSONSerdeOptionsFromMetadata(root,metadata);if err!=nil{return nil,err}
    validatorName,err:=JSONNativeValidatorName(program,contractName,moduleName);if err!=nil{return nil,err};options.nativeValidator=validatorName
...
func GenerateJSONSerde(program *language.Program,namespace,contractName,moduleName string,options JSONSerdeOptions)(files []File,failure error){
    ...
    numericExpansion:=options.NumericExpansion;if numericExpansion==0{numericExpansion=65536};if numericExpansion<1||numericExpansion>1000000{unsupported(language.Span{},"JSON numeric expansion limit must be between 1 and 1000000")}
