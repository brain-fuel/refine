// Code generated by goplus from native_avro_json_oracle_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func TestNativeAvroJSONAgreesWithApacheEncoding(t *testing.T) {
	...
	dir := t.TempDir()
	source := filepath.Join(dir, "AvroJSONOracle.java")
	if err = os.WriteFile(source, []byte(avroJSONOracleHarness), 0600); err != nil {
		t.Fatal(err)
	}
	classes := filepath.Join(dir, "classes")
	if output, err := exec.Command(compiler, "--release", "25", "-encoding", "UTF-8", "-Xlint:all", "-Werror", "-cp", classpath, "-d", classes, source).CombinedOutput(); err != nil {
		t.Fatalf("Avro JSON oracle javac: %v\n%s", err, output)
	}
	command := exec.Command(vm, "-Xss256k", "-Xmx64m", "-cp", classes+string(os.PathListSeparator)+classpath, "AvroJSONOracle")
	command.Stdin = bytes.NewReader(input)
	var stderr bytes.Buffer
	command.Stderr = &stderr
	output, err := command.Output()
	if err != nil {
		t.Fatalf("Avro JSON oracle: %v\n%s", err, stderr.String())
	}
	lines := strings.Split(strings.TrimSpace(string(output)), "\n")
	if len(lines) != len(cases) {
		t.Fatalf("oracle produced %d lines for %d cases", len(lines), len(cases))
	}
	for i, tc := range cases {
		project, err := native.IngestProject(native.Avro, []byte(tc.Schema), native.ProjectOptions{Root: native.ResourceSelector{TypeName: "Datum"}})
		if err != nil {
			t.Fatal(err)
		}
		binary, err := base64.StdEncoding.DecodeString(lines[i])
		if err != nil {
			t.Fatal(err)
		}
		want, wantReport, err := project.DecodeAndValidateAvro(binary, native.AvroPayloadLimits{}, validation.Limits{})
		if err != nil || validation.StateName(wantReport.State()) != "valid" {
			t.Fatalf("case %d Apache binary: %v %+v", i, err, wantReport)
		}
		...
		if err != nil || validation.StateName(report.State()) != "valid" {
			t.Fatalf("case %d Go JSON: %v %+v", i, err, report)
		}
		equal, err := got.EqualWith(want, func(uint64) error { return nil })
		if err != nil || !equal {
			t.Fatalf("case %d Go/Apache exact data mismatch: %v", i, err)
		}
	}
}
