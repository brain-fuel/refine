// Code generated by goplus from codec_read_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func readUnits(text value.Text) string {
...
func TestGeneratedTypedRead(t *testing.T) {
	...
	program, err := language.Compile(readContract)
	if err != nil {
		t.Fatal(err)
	}
	files, err := GenerateValidator(program, "example.read", "Contract")
	if err != nil {
		t.Fatal(err)
	}
	root := t.TempDir()
	...
	harness := filepath.Join(root, "ReadConformance.java")
	if err := os.WriteFile(harness, []byte(readHarnessJava), 0644); err != nil {
		t.Fatal(err)
	}
	...
	add := func(mode, name string, text value.Text, total, clause uint64) {
		...
		canonical := ""
		if mode == "r" {
			data, result := program.ReadData(name, text, limits)
			report = result
			if validation.StateName(report.State()) == "valid" {
				...
				canonical = fmt.Sprintf("|%x", s)
			}
		} else {
			report = program.ValidateData(name, value.OfText(text), limits)
		}
	...
	for _, name := range []string{"Integer", "RealValue", "TextValue", "Flag", "Positive", "Unknown", "Both", "Small", "Optional", "Record", "IntegerTree", "IntegerBox", "BooleanBox", "Recursive", "missing", "Tree"} {
		for _, raw := range corpus {
			for _, limit := range []uint64{0, 1, 8, 32, 128, 512} {
				add("r", name, text(raw), limit, 0)
				if limit != 0 {
					add("r", name, text(raw), 0, limit)
				}
			}
		}
	}
	for _, name := range []string{"ReadInt", "ReadPositive", "ReadUnknown", "ReadBoth", "ReadBudgeted", "ReadBadMessage", "Higher", "Local", "Generic", "Anonymous", "Recovery"} {
		for _, raw := range corpus {
			add("v", name, text(raw), 0, 0)
		}
		for _, raw := range []string{"1", "-1", "bad", "1/0", "{value=1}", "[1,2]"} {
			for limit := uint64(1); limit < 180; limit++ {
				add("v", name, text(raw), limit, 0)
				add("v", name, text(raw), 0, limit)
			}
		}
	...
	for _, depth := range []int{0, 80, 255, 256, 510, 511, 512, 513, 600} {
		for _, raw := range []string{strings.Repeat("(", depth) + "1" + strings.Repeat(")", depth), strings.Repeat("[", depth) + "1" + strings.Repeat("]", depth), strings.Repeat("1+", depth) + "1", strings.Repeat("1+", depth) + "1)", "case x of { C " + strings.Repeat("_ ", depth) + "-> 1 }"} {
			add("r", "Integer", text(raw), 0, 0)
			add("v", "ReadInt", text(raw), 0, 0)
		}
	}
	syntax := readerSyntaxCorpus(t)
	for _, source := range syntax {
		vectors = append(vectors, vector{fmt.Sprintf("p\tInteger\t%s\t0\t0", readUnits(text(source))), parserShape(source)})
	}
	for _, resource := range []struct {
		mode   string
		source string
	}{{"pbytes", strings.Repeat(" ", 16*1024*1024+1)}, {"punicode", "--" + strings.Repeat("😀", 4*1024*1024)}, {"ptokens", strings.Repeat("1 ", 1000001)}} {
		expected := parserShape(resource.source)
		if expected != "parse:evaluation.budget" {
			t.Fatalf("Go parser resource fixture: %s: %s", resource.mode, expected)
		}
	...
	defer cancel()
	command := exec.CommandContext(ctx, vm, "-Xss256k", "-cp", classes+string(os.PathListSeparator)+dependencies, "ReadConformance")
	command.Stdin = strings.NewReader(input.String())
	var stderr bytes.Buffer
	command.Stderr = &stderr
	output, err := command.Output()
	if err != nil {
		t.Fatalf("Java read: %v\n%s", err, stderr.String())
	}
