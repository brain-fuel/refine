// Code generated by goplus from openapi_native_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func nativeOpenAPIProject(t *testing.T) *native.Project {
	t.Helper()
	resources := []native.Resource{{URI: "https://example.test/api.json", Source: nativeOpenAPIResource}, {URI: "https://example.test/schemas.json", Source: `{"$schema":"https://json-schema.org/draft/2020-12/schema","$defs":{"Input":{"type":"integer","minimum":1},"Output":{"type":"integer","minimum":2}}}`}}
	project, err := native.IngestProjectResources(native.OpenAPI, resources, native.ProjectOptions{Root: native.ResourceSelector{Resource: "https://example.test/api.json", Pointer: "/components/schemas/Root", TypeName: "Root"}})
	if err != nil {
		t.Fatal(err)
	}
	project, err = project.WithEditedSource(nativeOpenAPISource)
	if err != nil {
		t.Fatal(err)
	}
	...
	binding := refineopenapi.NativeOperationBinding{OperationID: "createItem", Parameters: []refineopenapi.NativeParameterBinding{{In: "header", Name: "X-Key", FieldPath: []string{"xKey"}}}, Responses: nativeResponses}
...
func TestGeneratedNativeOpenAPIContextComposesAllBoundaries(t *testing.T) {
	...
	harness := filepath.Join(dir, "NativeOpenAPIHarness.java")
	if err := os.WriteFile(harness, []byte(nativeOpenAPIHarness), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if output, err := exec.Command(compiler, args...).CombinedOutput(); err != nil {
		t.Fatalf("native OpenAPI javac: %v\n%s", err, output)
	}
	if output, err := exec.Command(vm, "-Xss256k", "-Xmx64m", "-cp", classes+string(os.PathListSeparator)+classpath, "NativeOpenAPIHarness").CombinedOutput(); err != nil {
		t.Fatalf("native OpenAPI runtime: %v\n%s", err, output)
	}
}
