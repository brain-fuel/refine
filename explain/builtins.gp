...
func builtinMeaning(name string)(string,bool){
    if conversion,ok:=language.FixedIntegerConversion(name);ok{
        ...
        if conversion.Mode=="from"{meaning="Convert the "+conversion.Type+" argument to an arbitrary-precision integer without changing its mathematical value."}
        if conversion.Mode=="wrap"{meaning=fmt.Sprintf("Explicitly reduce the arbitrary-precision integer modulo 2 to the power %d, then interpret the result as %s (subtract that modulus for the upper signed half). This produces a new predicate value and does not mutate the payload.",conversion.Bits,conversion.Type)}
    ...
    if conversion,ok:=language.FixedFloatConversion(name);ok{
        ...
        if conversion.Mode=="exact"{meaning="Convert the exact rational argument to finite "+conversion.Type+" only when it is already exactly representable; return Err instead of rounding or producing an infinity."}
        if conversion.Mode=="round"{meaning="Explicitly round the exact rational argument to the nearest finite "+conversion.Type+", breaking half-way ties toward an even significand; return Err when the result would be infinite."}
        return "Use this built-in operation when applied: "+meaning,true
    }
    meaning:=""
    switch name{
    ...
    case "lookup":meaning="Look up the exact UTF-16 string key in the immutable map without normalization; return Nothing when absent or Just with its value when present."
    case "member":meaning="Return whether the immutable map contains the exact UTF-16 string key, without case folding or Unicode normalization."
    case "keys":meaning="Return the map keys in deterministic exact UTF-16 order."
    case "values":meaning="Return the map values in deterministic exact UTF-16 key order."
    case "size":meaning="Return the number of entries in the map."
    case "insert":meaning="Return a new map with the exact key associated with the supplied value, replacing that key's prior value without mutating the input."
    case "delete":meaning="Return a new map without the exact supplied key, leaving the input unchanged."
    case "mapValues":meaning="Apply the function to each map value in deterministic exact UTF-16 key order and return a new map with unchanged keys."
    case "filterValues":meaning="Apply the predicate to each map value in deterministic exact UTF-16 key order and return a new map containing only known-true values; predicate errors remain indeterminate."
    case "allValues":meaning="Apply the predicate to map values in deterministic exact UTF-16 key order with the same three-outcome rules as all."
    case "anyValues":meaning="Apply the predicate to map values in deterministic exact UTF-16 key order with the same three-outcome rules as any."
