...
func documentation(t *testing.T, source string) Document {
	t.Helper()
	program, err := language.Compile(source)
	if err != nil {
		t.Fatal(err)
	}
	doc, err := Generate(program)
	if err != nil {
		t.Fatal(err)
	}
	return doc
...
`)
	rules := doc.Rules()
	if len(rules) != 4 {
		t.Fatalf("one entry per where, got %d", len(rules))
	}
	child := rules[1]
	if child.Code != "child.range" || child.Steps != 123 || child.MessageEntry == "" || !strings.Contains(child.Predicate, "&&") {
		t.Fatalf("lost clause metadata: %+v", child)
	}
	if rules[0].Location != "value / field age" {
		t.Fatal(rules[0])
	}
	markdown := doc.Markdown()
	for _, want := range []string{"Expected 0 <= age < 18; got", "Declared clause limit: 123", "without evaluating", "custom", "Custom-message failure", "does not claim", "one diagnostic unit"} {
	...
	rules[0].Predicate = "False"
	defs := doc.Definitions()
	defs[0].Name = "Changed"
	steps := doc.Instructions()
	steps[0].English = "Changed"
	if doc.Markdown() != markdown {
		t.Fatal("mutable exported document")
	}
}

func TestAllSyntaxAndRecursion(t *testing.T) {
	input, err := os.ReadFile("../language/testdata/contracts.refine")
	if err != nil {
		t.Fatal(err)
	}
	doc := documentation(t, string(input))
	text := doc.Markdown()
	for _, want := range []string{"isAdult", "sum", "nonEmpty", "nonempty list", "Recursive calls", "partial applications", "first match", "Bind that value", "alternative", "UTF-16", "absence is not null", "null is not absence"} {
	...
	before := doc.Markdown()
	equations[0].Patterns[0] = "mutated"
	if doc.Markdown() != before {
		t.Fatal("equation pattern mutation escaped")
	}
...
`)
	if len(forever.Instructions()) > 30 {
		t.Fatal("recursive function expanded instead of referenced")
	}
}
...
type T = Int where f it > 0`, "lexically bound value length", "number of list elements or UTF-16"},
	...
	for _, test := range cases {
		doc := documentation(t, test.source)
		text := doc.Markdown()
		if !strings.Contains(text, test.wanted) {
			t.Fatalf("missing %q in %s", test.wanted, text)
		}
...
`)
	text := doc.Markdown()
	for _, want := range []string{"without evaluating", "otherwise evaluate and return only", "division by zero", "prepend", "concatenate", "construct a new record", "select its field", "do not execute the numbered list eagerly"} {
		if !strings.Contains(text, want) {
			t.Errorf("missing %q", want)
		}
	}
...
func TestDeterministicAndConcurrent(t *testing.T) {
	...
	p, err := language.Compile("type T = Int where it > 0")
	if err != nil {
		t.Fatal(err)
	}
	base, err := Generate(p)
	...
	for range 16 {
		...
		go func() {
			...
			if err != nil || doc.Markdown() != want {
				t.Error("non-deterministic concurrent generation")
			}
		}()
	}
	wg.Wait()
...
func TestMarkdownLiteralSafety(t *testing.T) {
	...
	if !strings.Contains(text, "&lt;script&gt;") {
		t.Fatal("instruction prose did not escape HTML")
	}
...
func TestGenerationLimitsAndNil(t *testing.T) {
	...
	b = builder{next: 100000}
	func() {
		defer func() {
			if _, ok := recover().(limitError); !ok {
				t.Error("missing instruction limit")
			}
		}()
		b.expr("", &language.Expr{Form: language.BoolLiteral{Value: true}})
	}()
}
...
func TestSelectedPayloadDocumentation(t *testing.T) {
	...
	target, err := p.PayloadType(`Box (Int where it > 0 @message "Root argument must be positive")`)
	if err != nil {
		t.Fatal(err)
	}
	doc, err := GeneratePayload(target)
	if err != nil {
		t.Fatal(err)
	}
	if len(doc.Rules()) != 1 || doc.Rules()[0].Owner != "$payload" || !strings.Contains(doc.Markdown(), "Root argument must be positive") {
		t.Fatal("inline selected target lost")
	}
	if _, err := GeneratePayload(nil); err == nil {
...
func FuzzExplain(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, source string) {
		...
		other, err := Generate(program)
		if err != nil || doc.Markdown() != other.Markdown() {
			t.Fatal("nondeterministic explanation")
		}
		ids := map[string]bool{}
		for _, step := range doc.Instructions() {
			if ids[step.ID] || step.English == "" {
				t.Fatal("invalid instruction")
			}
			ids[step.ID] = true
		}
		for _, rule := range doc.Rules() {
			if !ids[rule.Entry] || (rule.MessageEntry != "" && !ids[rule.MessageEntry]) {
