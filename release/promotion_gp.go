// Code generated by goplus from promotion.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
const (
	ExactRelease ImportKind = iota + 1
	SnapshotImport
	VersionRangeImport
)
...
func cleanRelative(path string) (string, error) {
	if path == "" || filepath.IsAbs(path) {
		return "", fmt.Errorf("release.path: path must be relative")
	}
...
func resolvedRoot(root string) (string, error) {
	if root == "" {
		return "", fmt.Errorf("release.root: root is required")
	}
	...
	if !info.IsDir() || info.Mode()&os.ModeSymlink != 0 {
		return "", fmt.Errorf("release.root: root must be an existing non-symlink directory")
	}
	return absolute, nil
}
func checkNoSymlink(root, relative string, includeLeaf bool) error {
	...
	for i := 0; i < limit; i++ {
		...
		if info.Mode()&os.ModeSymlink != 0 {
			return fmt.Errorf("release.symlink: %s contains a symbolic link", relative)
		}
		if i < limit-1 && !info.IsDir() {
			return fmt.Errorf("release.path: parent of %s is not a directory", relative)
		}
	}
	return nil
}
	...
	for _, part := range parts {
		...
		if err == nil {
			if info.Mode()&os.ModeSymlink != 0 || !info.IsDir() {
				return created, fmt.Errorf("release.path: unsafe parent for %s", relative)
			}
			continue
		}
		if !errors.Is(err, os.ErrNotExist) {
...
func Promote(input PromotionInput) (PromotionResult, error) {
	root, err := resolvedRoot(input.Root)
	if err != nil {
		return PromotionResult{}, err
	}
	lock := filepath.Join(root, lockName)
	lockFile, err := os.OpenFile(lock, os.O_WRONLY|os.O_CREATE|os.O_EXCL, 0600)
	if err != nil {
	...
	for _, r := range input.Available {
		if !validFamily(r.Family) || !r.Version.Valid() || !r.Content.Valid() {
	...
	add := func(path string, content []byte) error {
		...
		if _, e = os.Lstat(filepath.Join(root, relative)); e == nil {
			return fmt.Errorf("release.immutable: destination already exists: %s", relative)
	...
	for _, family := range input.Families {
		if !validFamily(family.Family) || seenFamilies[family.Family] {
			return result, fmt.Errorf("release.family: invalid or duplicate family %q", family.Family)
		}
		...
		snapshotRel, e := cleanRelative(family.SnapshotPath)
		if e != nil {
			return result, e
		}
		if e = checkNoSymlink(root, snapshotRel, true); e != nil {
			return result, e
		}
		snapshotFull := filepath.Join(root, snapshotRel)
		snapshotInfo, e := os.Lstat(snapshotFull)
		if e != nil || !snapshotInfo.Mode().IsRegular() {
			return result, fmt.Errorf("release.snapshot: %s must be a regular file", family.Family)
		}
		snapshotBytes, e := os.ReadFile(snapshotFull)
		if e != nil {
			return result, fmt.Errorf("release.snapshot: %w", e)
		}
		if Digest(snapshotBytes) != family.SnapshotFileContent {
		...
		batch[key] = releaseDigest
		if e = add(family.ReleasePath, family.ReleaseContent); e != nil {
			return result, e
		}
		for _, artifact := range family.Generated {
			if e = add(artifact.Path, artifact.Content); e != nil {
				return result, e
			}
		}
		result.Released = append(result.Released, PublishedRelease{Family: family.Family, Version: family.Version, Content: releaseDigest})
		result.SnapshotPending[family.Family] = false
	}
	for _, family := range input.Families {
		for _, pin := range family.Imports {
			if pin.Kind != ExactRelease {
				return PromotionResult{}, fmt.Errorf("release.import: %s import of %s is not an exact released version", family.Family, pin.Family)
			}
			if !validFamily(pin.Family) || !pin.Version.Valid() || !pin.Content.Valid() {
				return PromotionResult{}, fmt.Errorf("release.import: invalid exact pin in %s", family.Family)
			}
			key := releaseKey(pin.Family, pin.Version)
			actual, ok := batch[key]
			if !ok {
				actual, ok = available[key]
			}
			if !ok || actual != pin.Content {
				return PromotionResult{}, fmt.Errorf("release.import: %s does not resolve %s %s to the pinned content", family.Family, pin.Family, pin.Version.String())
			}
		}
	}
	...
	if err = syncDir(transaction); err != nil {
		cleanup()
		return PromotionResult{}, err
	}
...
func Recover(rootPath string) error {
	root, err := resolvedRoot(rootPath)
	if err != nil {
		return err
	}
	matches, err := filepath.Glob(filepath.Join(root, transactionPrefix+"*"))
	if err != nil {
		return err
	}
	sort.Strings(matches)
	for _, transaction := range matches {
		info, e := os.Lstat(transaction)
		if e != nil {
			return e
		}
		if !info.IsDir() || info.Mode()&os.ModeSymlink != 0 {
			return fmt.Errorf("release.recovery: unsafe transaction path %s", filepath.Base(transaction))
		}
		journalPath := filepath.Join(transaction, "journal.json")
		nextPath := journalPath + ".next"
		journalInfo, e := os.Lstat(journalPath)
		if errors.Is(e, os.ErrNotExist) {
			journalPath = nextPath
			journalInfo, e = os.Lstat(journalPath)
		}
		if e != nil || !journalInfo.Mode().IsRegular() {
			return fmt.Errorf("release.recovery: unsafe journal %s", filepath.Base(transaction))
		}
		data, e := os.ReadFile(journalPath)
		if e != nil {
		...
		if journal.State != "committed" && journal.State != "installing" {
		...
		for _, child := range children {
			if !allowed[child.Name()] {
				return fmt.Errorf("release.recovery: transaction contains unexpected file %s", child.Name())
			}
		}
		if journal.State == "installing" {
			if e = rollbackEntries(root, transaction, journal.Entries); e != nil {
				return fmt.Errorf("release.recovery: rollback: %w", e)
			}
		}
		for _, entry := range journal.Entries {
		...
		if e = os.Remove(transaction); e != nil {
			return e
		}
	}
	// Recovery is the explicit operator action that clears a crash-stale lock.
	removeErr := os.Remove(filepath.Join(root, lockName))
