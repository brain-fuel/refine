...
func Promote(input PromotionInput)(PromotionResult,error){
    ...
    available:=map[string]ContentID{};for _,r:=range input.Available{if !validFamily(r.Family)||!r.Version.Valid()||!r.Content.Valid(){return result,fmt.Errorf("release.available: invalid published release identity")};key:=releaseKey(r.Family,r.Version);if _,ok:=available[key];ok{return result,fmt.Errorf("release.available: duplicate published release %s %s",r.Family,r.Version.String())};available[key]=r.Content}
    ...
    for _,family:=range input.Families{
        ...
        key:=releaseKey(family.Family,family.Version);if _,published:=available[key];published{return result,fmt.Errorf("release.immutable: %s %s is already published",family.Family,family.Version.String())}
        ...
        if _,ok:=batch[key];ok{return result,fmt.Errorf("release.batch: duplicate release %s %s",family.Family,family.Version.String())};batch[key]=releaseDigest
