...
func IngestProjectResources(format Format, resources []Resource, options ProjectOptions) (*Project, error) {
			...
			if err == nil && hasSelectedAnnotation {
				...
				if strings.HasPrefix(document.Version(), "3.0.") {
					...
					if err == nil {
						...
						if err == nil {
							source, err = projectOpenAPICatalogRoot(catalog, options.Root)
						}
					}
				}
			}
	...
	if err := auditProjectExecutableAnnotations(format, ordered, options.Root, hasSelectedAnnotation); err != nil {
		return nil, err
	}
	...
	if format == OpenAPI {
		if err := installOpenAPIConstraintOrigins(project, options.Root.Resource); err != nil {
			return nil, err
		}
	}
...
func validateOpenAPIResources(resources map[string][]byte, root ResourceSelector) (document *Document, failure error) {
	...
	document, err := validateOpenAPIDocumentResources(resources, root.Resource)
	if err != nil {
		return nil, err
	}
	if strings.HasPrefix(document.Version(), "3.0.") {
	...
	canonical, err := canonicalOpenAPIResourceMap(resources)
	if err != nil {
		return nil, err
	}
	view, err := NewOpenAPIValidationView(canonical, root.Resource)
	if err != nil {
		return nil, err
	}
	mapped, err := view.Selector(root)
	if err != nil {
		return nil, err
	}
	if err := compileOpenAPIValidationRoot(view, mapped); err != nil {
		return nil, err
	}
	return document, nil
}

func canonicalOpenAPIResourceMap(resources map[string][]byte) ([]Resource, error) {
	uris := sortedResourceURIs(resources)
	out := make([]Resource, 0, len(uris))
	for _, uri := range uris {
		encoded, err := openAPIResourceJSON(resources[uri])
		if err != nil {
			return nil, wrap(OpenAPI, "native.structure", uri, err)
		}
		out = append(out, Resource{URI: uri, Source: string(encoded)})
	}
	return out, nil
}

func normalizeOpenAPIKinResources(resources []Resource) ([]Resource, error) {
	...
	for i, resource := range resources {
		...
		versionNode, document := doc.Root().Lookup("openapi")
		version, valid := nodeString(versionNode)
		if !document || !valid || version != "3.2.1" {
			continue
		}
		doc, err = doc.Replace("/openapi", []byte(`"3.2.0"`))
		if err != nil {
			return nil, wrap(OpenAPI, "native.structure", resource.URI, err)
		}
		out[i].Source = doc.Raw()
	}
	return out, nil
}
...
func validateOpenAPIDocumentResources(resources map[string][]byte, entryResource string) (document *Document, failure error) {
	...
	if strings.HasPrefix(version, "3.1.") || strings.HasPrefix(version, "3.2.") {
		...
		catalog, err := newOpenAPIProjectionCatalog(canonical, entryResource)
		if err != nil {
			return nil, err
		}
		kinResources, err := openAPIKinOracleResources(catalog)
		if err != nil {
			return nil, err
		}
		kinResources, err = normalizeOpenAPIKinResources(kinResources)
		if err != nil {
			return nil, err
		}
		oracleResources = map[string][]byte{}
		for _, resource := range kinResources {
			oracleResources[resource.URI] = []byte(resource.Source)
		}
		oracleInput = oracleResources[entryResource]
	} else {
		var err error
