// Code generated by goplus from annotations.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func checkAnnotation(format Format, pointer, source, root string, metadata WireMetadata, hasMetadata bool) (Annotation, error) {
	...
	if root != "" {
		if _, err := program.PayloadType(root); err != nil {
...
func jsonAnnotation(format Format, pointer string, node schemajson.Node) (Annotation, error) {
	...
	if !ok {
		return Annotation{}, &Error{Code: "native.refinement", Format: format, Pointer: pointer, Message: "x-refine.source is required"}
	}
	source, ok := nodeString(sourceNode)
	if !ok {
		return Annotation{}, &Error{Code: "native.refinement", Format: format, Pointer: pointer + "/source", Message: "source must be a string"}
	}
	root := ""
	if rootNode, exists := node.Lookup("root"); exists {
		root, ok = nodeString(rootNode)
		if !ok {
...
func jsonSchemaAnnotations(root schemajson.Node) ([]Annotation, error) {
	result := []Annotation{}
	var walk func(schemajson.Node, string) error
	walk = func(node schemajson.Node, path string) error {
		if schemajson.KindName(node.Kind()) != "object" {
			return nil
		}
		if ext, ok := node.Lookup("x-refine"); ok {
			annotation, err := jsonAnnotation(JSONSchema, path+"/x-refine", ext)
			if err != nil {
				return err
			}
		...
		for _, key := range []string{"$defs", "definitions", "properties", "patternProperties", "dependentSchemas"} {
			if children, ok := node.Lookup(key); ok && schemajson.KindName(children.Kind()) == "object" {
				for _, member := range children.Members() {
					...
					if err := walk(member.Value, path+"/"+key+"/"+escapePointer(name)); err != nil {
						return err
					}
				}
			}
		...
		for _, key := range []string{"allOf", "anyOf", "oneOf", "prefixItems"} {
			if children, ok := node.Lookup(key); ok && schemajson.KindName(children.Kind()) == "array" {
				for i, child := range children.Elements() {
					if err := walk(child, fmt.Sprintf("%s/%s/%d", path, key, i)); err != nil {
						return err
					}
				}
			}
...
func avroAnnotations(root schemajson.Node) ([]Annotation, error) {
	...
	walk = func(node schemajson.Node, path string) error {
		...
		if ext, ok := node.Lookup("x-refine"); ok {
			annotation, err := jsonAnnotation(Avro, path+"/x-refine", ext)
			if err != nil {
				return err
			}
			result = append(result, annotation)
			...
			if fields, ok := node.Lookup("fields"); ok && schemajson.KindName(fields.Kind()) == "array" {
				for i, field := range fields.Elements() {
					if child, ok := field.Lookup("type"); ok {
						if err := walk(child, fmt.Sprintf("%s/fields/%d/type", path, i)); err != nil {
							return err
						}
					}
				}
		...
		case "array":
			if child, ok := node.Lookup("items"); ok {
				return walk(child, path+"/items")
			}
		case "map":
			if child, ok := node.Lookup("values"); ok {
				return walk(child, path+"/values")
			}
		}
		return nil
	}
...
func yamlAnnotationValue(node *yaml.Node, pointer string) (string, string, error) {
	...
	if node.Kind != yaml.MappingNode {
		return "", "", &Error{Code: "native.refinement", Format: OpenAPI, Pointer: pointer, Message: "x-refine must be a source string or an object containing source and optional root strings"}
	}
	source, root := "", ""
	seenSource := false
	for i := 0; i < len(node.Content); i += 2 {
		key, value := node.Content[i].Value, node.Content[i+1]
		if key != "source" && key != "root" {
			return "", "", &Error{Code: "native.refinement", Format: OpenAPI, Pointer: pointer, Message: "x-refine object permits only source and root"}
		}
		if value.Kind != yaml.ScalarNode || value.Tag != "!!str" {
			return "", "", &Error{Code: "native.refinement", Format: OpenAPI, Pointer: pointer + "/" + key, Message: key + " must be a string"}
		}
		if key == "source" {
			source = value.Value
			seenSource = true
	...
	if !seenSource {
		return "", "", &Error{Code: "native.refinement", Format: OpenAPI, Pointer: pointer, Message: "x-refine.source is required"}
	}
