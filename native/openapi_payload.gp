...
func (p *Project) canonicalJSONResources()([]Resource,error){
    ...
    };if p.Format()==JSONSchema{return p.effectiveJSONSchemaResources(resources)};return resources,nil
}
...
func (p *Project) validateOpenAPIJSON(input []byte)(failure error){defer recoverRegexEvaluation(OpenAPI,&failure)
    ...
    compiler:=jsonoracle.NewCompiler();compiler.DefaultDraft(jsonoracle.Draft2020);compiler.UseRegexpEngine(jsonRegexp);dialect,err:=jsonoracle.UnmarshalJSON(strings.NewReader(openAPIBaseDialectAdapter));if err!=nil{panic(err)};if err:=compiler.AddResource(openAPIBaseDialect,dialect);err!=nil{panic(err)}
    resources,err:=p.CanonicalJSONResources();if err!=nil{return err};for _,resource:=range resources{schemaValue,err:=jsonoracle.UnmarshalJSON(strings.NewReader(resource.Source));if err!=nil{return wrap(OpenAPI,"native.structure",resource.URI,err)};if err:=compiler.AddResource(resource.URI,schemaValue);err!=nil{return wrap(OpenAPI,"native.structure",resource.URI,err)}}
    location:=p.root.Resource;if p.root.Pointer!=""{location+="#"+p.root.Pointer};schema,err:=compiler.Compile(location);if err!=nil{return wrap(OpenAPI,"native.enforcement",p.root.Pointer,err)};if err:=schema.Validate(value);err!=nil{return wrap(OpenAPI,"native.payload",p.root.Pointer,err)};return nil
}
...
func adaptOpenAPI30Schema(schema map[string]any)error{
    if _,hasRef:=schema["$ref"];hasRef{for key:=range schema{if key!="$ref"&&key!="x-refine"&&!strings.HasPrefix(key,"x-"){return fmt.Errorf("OpenAPI 3.0 Reference Object contains a non-extension sibling")};if key!="$ref"{delete(schema,key)}};return nil}
...
func (p *Project) validateOpenAPIDialects(resources []Resource)error{docs:=map[string]schemajson.Document{};for _,resource:=range resources{doc,err:=schemajson.Parse([]byte(resource.Source),schemajson.Limits{});if err!=nil{return err};docs[resource.URI]=doc};if root,ok:=docs[p.root.Resource];ok{if dialectNode,ok:=root.Root().Lookup("jsonSchemaDialect");ok{if err:=supportedOpenAPIDialectValue(dialectNode,p.root.Resource+"#/jsonSchemaDialect");err!=nil{return err}}};return p.walkJSONSchemaLocations(resources,func(resource,path string,node schemajson.Node)error{if dialectNode,ok:=node.Lookup("$schema");ok{return supportedOpenAPIDialectValue(dialectNode,resource+"#"+path+"/$schema")};return nil})}
