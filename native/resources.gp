...
func IngestProjectResources(format Format,resources []Resource,options ProjectOptions)(*Project,error){
    ...
    case OpenAPI:document,err=validateOpenAPIResources(byURI,options.Root);if err==nil{source,err=projectJSONResourceRoot(byURI,options.Root,true)}
    ...
    origins:=make(map[string]*provenance.JSONSchema);if format==JSONSchema{for _,resource:=range ordered{origin,e:=provenance.DiscoverJSONSchema([]byte(resource.Source),schemajson.Limits{});if e!=nil{return nil,wrap(JSONSchema,"native.provenance",resource.URI,e)};origins[resource.URI]=origin}}
    _=mainBytes;return &Project{document:document,root:options.Root,source:source,program:program,metadata:copyMetadata(options.Metadata),resources:ordered,jsonOrigins:origins},nil
}

func validateJSONResources(resources map[string][]byte,root ResourceSelector)(*Document,error){compiler:=jsonoracle.NewCompiler();compiler.DefaultDraft(jsonoracle.Draft2020);compiler.UseRegexpEngine(jsonRegexp);var rootDoc schemajson.Document
    for uri,input:=range resources{doc,err:=schemajson.Parse(input,schemajson.Limits{});if err!=nil{return nil,wrap(JSONSchema,"native.syntax",uri,err)};if err:=scalarJSON(doc.Root(),"");err!=nil{return nil,wrap(JSONSchema,"native.encoding",uri,err)};if err:=checkJSONDraft(doc.Root(),uri);err!=nil{return nil,err};value,err:=jsonoracle.UnmarshalJSON(strings.NewReader(doc.Raw()));if err!=nil{return nil,wrap(JSONSchema,"native.syntax",uri,err)};if err:=compiler.AddResource(uri,value);err!=nil{return nil,wrap(JSONSchema,"native.resource",uri,err)};if uri==root.Resource{rootDoc=doc}}
...
func validateOpenAPIResources(resources map[string][]byte,root ResourceSelector)(*Document,error){main:=resources[root.Resource];l,_:=limits(Options{});var yamlRoot *yaml.Node;for uri,input:=range resources{node,err:=parseYAML(input,l);if err!=nil{return nil,wrap(OpenAPI,"native.syntax",uri,err)};if strings.HasPrefix(strings.TrimSpace(string(input)),"{"){doc,err:=schemajson.Parse(input,l);if err!=nil{return nil,wrap(OpenAPI,"native.syntax",uri,err)};if err:=scalarJSON(doc.Root(),"");err!=nil{return nil,wrap(OpenAPI,"native.encoding",uri,err)}};if uri==root.Resource{yamlRoot=node}}
