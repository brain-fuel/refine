...
func IngestProjectResources(format Format, resources []Resource, options ProjectOptions) (*Project, error) {
	...
	if hasSelectedAnnotation {
		...
		if selectedAnnotation.Root != options.Root.TypeName {
			...
			if err != nil { return nil, wrap(format, "native.projection", options.Root.Pointer, err) }
		}
...
func validateJSONResources(resources map[string][]byte, root ResourceSelector) (document *Document, failure error) {
	...
	catalog,err:=newJSONProjectionCatalog(ordered);if err!=nil{return nil,err};compiler.UseLoader(jsonProjectionLoader{catalog:catalog})
...
func validateOpenAPIDocumentResources(resources map[string][]byte,entryResource string) (document *Document, failure error) {
	...
	if !exists{return nil,&Error{Code:"native.resource",Format:OpenAPI,Pointer:entryResource,Message:"OpenAPI entry resource is not in the explicit resource set"}}
	l, _ := limits(Options{})
	...
	scope:=newRegexScope()
	if err := scope.run(OpenAPI,false,func()error{validationOptions:=[]openapi3.ValidationOption{openapi3.SetRegexCompiler(scope.openAPIRegexp)};if strings.HasPrefix(version,"3.0."){validationOptions=append(validationOptions,openapi3.AllowExtraSiblingFields(openAPI30IgnoredRefSiblingFields(yamlRoots...)...))};return parsed.Validate(context.Background(),validationOptions...)}); err != nil {
