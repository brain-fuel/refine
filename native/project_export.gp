...
func (p *Project) Export(options LowerOptions)(*ProjectExport,error){
    ...
    companion,losses,err=projectOpenAPIExplanation(p.program,p.metadata,companion,losses);if err!=nil{return nil,wrap(p.Format(),"native.explain","",err)}
    ...
    resources,err:=p.EffectiveResources();if err!=nil{return nil,err}
    ...
    if addition!=nil{if p.Format()==OpenAPI{if err:=projectOpenAPIAddition(document,addition,p.root.Resource);err!=nil{return nil,err}};delete(addition,"description");existing,ok:=target["allOf"].([]any);if !ok&&target["allOf"]!=nil{return nil,&Error{Code:"native.export",Format:p.Format(),Pointer:targetPointer,Message:"selected schema has malformed allOf"}};target["allOf"]=append(existing,addition)}
    projectEmbedExplanation(target,p.Format(),companion,losses)
    if mode==Refined{annotation:=map[string]any{"source":p.source,"root":p.root.TypeName,"metadata":p.Metadata()};if p.Format()==OpenAPI{top,ok:=document.(map[string]any);if !ok{return nil,&Error{Code:"native.export",Format:OpenAPI,Message:"OpenAPI root is not an object"}};if _,scoped:=target["x-refine"];scoped{target["x-refine"]=annotation}else{top["x-refine"]=annotation}}else{target["x-refine"]=annotation}}
