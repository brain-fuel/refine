...
func (p *Project) DecodeAndValidateJSON(input []byte,limits validation.Limits)(value.Data,validation.Report,error){
    ...
    if p.Format()!=JSONSchema&&p.Format()!=OpenAPI{return value.Data{},validation.Report{},&Error{Code:"native.enforcement",Format:p.Format(),Message:"the refined JSON boundary supports JSON Schema and OpenAPI projects"}}
    if err:=p.ValidateJSON(input);err!=nil{return value.Data{},validation.Report{},err}
    document,err:=schemajson.Parse(input,schemajson.Limits{});if err!=nil{return value.Data{},validation.Report{},wrap(p.Format(),"native.payload","",err)}
    target,err:=p.PayloadType();if err!=nil{return value.Data{},validation.Report{},err};checked:=target.CheckedSyntax()
    ...
    data,err:=decoder.decode(checked.Type,document.Root(),nil,"",0,"");if err!=nil{return value.Data{},validation.Report{},err}
...
func (d *jsonValueDecoder) stringNumber(node schemajson.Node,path string,integer bool)(value.Data,error){text,ok:=node.Text();if !ok{return value.Data{},d.failure(path,"expected a JSON string containing an exact number")};raw,err:=text.UTF8();if err!=nil{return value.Data{},d.failure(path,"number string is not Unicode scalar text")};if integer&&!jsonIntegerToken(raw,true){return value.Data{},d.failure(path,"integer string must use canonical base-10 spelling")};number,err:=value.ParseNumber(raw);if err!=nil||integer&&!number.IsInteger(){return value.Data{},d.failure(path,"invalid exact number string")};return value.OfNumber(number),nil}

func (d *jsonValueDecoder) scalar(name string,encoding ScalarEncoding,node schemajson.Node,path string)(value.Data,error){switch encoding.Kind{
    case JSONNumber:return d.number(node,path,true)
    case DecimalString:return d.stringNumber(node,path,true)
    case TimestampString:text,ok:=node.Text();if !ok{return value.Data{},d.failure(path,"expected an RFC 3339 JSON string")};return value.OfText(text),nil
    case RationalRecord:
        if schemajson.KindName(node.Kind())!="object"{return value.Data{},d.failure(path,"expected a rational JSON object")};numerator,okN:=node.Lookup("numerator");denominator,okD:=node.Lookup("denominator");if !okN||!okD||len(node.Members())!=2{return value.Data{},d.failure(path,"rational JSON object must contain only numerator and denominator")};n,err:=d.number(numerator,jsonValuePath(path,"numerator"),true);if err!=nil{return value.Data{},err};den,err:=d.number(denominator,jsonValuePath(path,"denominator"),true);if err!=nil{return value.Data{},err};left,_:=n.Number();right,_:=den.Number();if right.Sign()<=0{return value.Data{},d.failure(jsonValuePath(path,"denominator"),"rational denominator must be positive")};result,err:=left.Divide(right);if err!=nil{return value.Data{},d.failure(path,"invalid rational JSON object")};return value.OfNumber(result),nil
    case AvroBytesDecimal:return value.Data{},d.failure(path,"Avro decimal encoding cannot be decoded from JSON")
    }
...
func (d *jsonValueDecoder) optional(t *language.Type,bindings map[string]*language.Type,depth int)bool{if t==nil||depth>d.maxDepth{return false};match t.Form{case language.RefinedType(base,_):return d.optional(base,bindings,depth+1);case language.NamedType(name):if bound,ok:=bindings[name];ok{return d.optional(bound,bindings,depth+1)};if decl,ok:=d.declarations[name];ok&&decl.Body!=nil&&len(decl.Parameters)==0{return d.optional(decl.Body,nil,depth+1)};case language.AppliedType(_,_):name,args,ok:=jsonApplied(t);if !ok{return false};if name=="Maybe"&&len(args)==1{return true};if decl,found:=d.declarations[name];found&&decl.Body!=nil&&len(args)==len(decl.Parameters){closed:=map[string]*language.Type{};for key,item:=range bindings{closed[key]=item};for i,param:=range decl.Parameters{closed[param]=args[i]};return d.optional(decl.Body,closed,depth+1)};case _:};return false}
...
func (d *jsonValueDecoder) union(name string,variants []language.Variant,bindings map[string]*language.Type,node schemajson.Node,path string,depth int)(value.Data,error){
    wire,ok:=d.metadata.Discriminators[name];if !ok{return value.Data{},d.failure(path,"tagged union JSON decoding requires explicit discriminator metadata for "+name)};if schemajson.KindName(node.Kind())!="object"{return value.Data{},d.failure(path,"expected a JSON object for tagged union")};tagNode,ok:=node.Lookup(wire.Field);if !ok{return value.Data{},d.failure(jsonValuePath(path,wire.Field),"union discriminator is absent")};tagText,ok:=tagNode.Text();if !ok{return value.Data{},d.failure(jsonValuePath(path,wire.Field),"union discriminator must be a string")};tag,err:=tagText.UTF8();if err!=nil{return value.Data{},d.failure(jsonValuePath(path,wire.Field),"union discriminator is not Unicode scalar text")}
    constructor:="";for candidate,wireValue:=range wire.Values{if wireValue==tag{constructor=candidate;break}};if constructor==""{return value.Data{},d.failure(jsonValuePath(path,wire.Field),"unknown union discriminator value")};var selected *language.Variant;for i:=range variants{if variants[i].Name==constructor{selected=&variants[i];break}};if selected==nil{return value.Data{},d.failure(path,"discriminator metadata names an unknown constructor")};names:=wire.Arguments[constructor];if len(names)!=len(selected.Arguments){return value.Data{},d.failure(path,"discriminator argument metadata has the wrong arity")};args:=make([]value.Data,len(names));for i,member:=range names{child,found:=node.Lookup(member);if !found{return value.Data{},d.failure(jsonValuePath(path,member),"union argument is absent")};typ:=selected.Arguments[i];if bindings!=nil{closed,subErr:=language.SubstituteType(typ,bindings);if subErr!=nil{return value.Data{},d.failure(path,"generic union argument cannot be closed")};typ=closed};decoded,decodeErr:=d.decode(typ,child,nil,jsonValuePath(path,member),depth+1,"");if decodeErr!=nil{return value.Data{},decodeErr};args[i]=decoded};return value.Variant(constructor,args)
}

func (d *jsonValueDecoder) extra(node schemajson.Node,path string,depth int)(value.Data,error){if err:=d.enter(path,depth);err!=nil{return value.Data{},err};switch schemajson.KindName(node.Kind()){
    case "null":return value.Variant("Null",nil)
    case "boolean":return value.OfBool(node.Raw()=="true"),nil
    case "number":return d.number(node,path,false)
    case "string":text,_:=node.Text();return value.OfText(text),nil
    case "array":elements:=node.Elements();items:=make([]value.Data,len(elements));for i,item:=range elements{decoded,err:=d.extra(item,jsonValuePath(path,fmt.Sprint(i)),depth+1);if err!=nil{return value.Data{},err};items[i]=decoded};return value.List(items),nil
    case "object":members:=node.Members();fields:=make([]value.DataField,len(members));for i,member:=range members{name,err:=member.Key.UTF8();if err!=nil{return value.Data{},d.failure(path,"preserved JSON object key is not Unicode scalar text")};decoded,err:=d.extra(member.Value,jsonValuePath(path,name),depth+1);if err!=nil{return value.Data{},err};fields[i]=value.DataField{Name:name,Value:decoded}};result,err:=value.Record(fields);if err!=nil{return value.Data{},d.failure(path,"preserved JSON object is not representable")};return result,nil
