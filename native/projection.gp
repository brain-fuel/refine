...
func projectAvro(doc schemajson.Document,selector ResourceSelector)(string,error){p:=&sourceProjector{format:Avro,root:doc,avroNames:make(map[string]string),emitted:make(map[string]bool)};expr,err:=p.avroType(doc.Root(),selector.TypeName);if err!=nil{return "",err};p.declarations=append(p.declarations,"type "+selector.TypeName+" = "+expr);return strings.Join(p.declarations,"\n\n")+"\n",nil}
