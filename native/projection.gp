...
func (p *sourceProjector) jsonType(node schemajson.Node,path string,openAPI bool)(string,error){
    ...
    if additional,ok:=node.Lookup("additionalProperties");ok&&schemajson.KindName(additional.Kind())=="object"{return "",&Error{Code:"native.projection",Format:p.format,Pointer:path+"/additionalProperties",Message:"schema-valued additionalProperties requires a language map type; Boolean permissive or closed extra-field policies remain supported"}}
    if patterns,ok:=node.Lookup("patternProperties");ok&&schemajson.KindName(patterns.Kind())=="object"&&len(patterns.Members())>0{return "",&Error{Code:"native.projection",Format:p.format,Pointer:path+"/patternProperties",Message:"patternProperties map values require a language map type; retain this schema in the native document until typed maps are implemented"}}
    ...
    case "object":properties,exists:=node.Lookup("properties");if !exists{result="{}";break};if schemajson.KindName(properties.Kind())!="object"{return "",&Error{Code:"native.projection",Format:p.format,Pointer:path+"/properties",Message:"properties must be an object"}};required:=make(map[string]bool);if list,exists:=node.Lookup("required");exists{if schemajson.KindName(list.Kind())!="array"{return "",&Error{Code:"native.projection",Format:p.format,Pointer:path+"/required",Message:"required must be an array"}};for _,item:=range list.Elements(){name,ok:=nodeString(item);if !ok{return "",&Error{Code:"native.projection",Format:p.format,Pointer:path+"/required",Message:"required names must be strings"}};required[name]=true}}
        fields:=[]string{};for _,member:=range properties.Members(){name,_:=member.Key.UTF8();if !memberPattern.MatchString(name){return "",&Error{Code:"native.projection",Format:p.format,Pointer:path+"/properties/"+escapePointer(name),Message:"property name is not a language field identifier"}};fieldType,err:=p.jsonType(member.Value,path+"/properties/"+escapePointer(name),openAPI);if err!=nil{return "",err};if !required[name]{fieldType="Maybe ("+fieldType+")"};fields=append(fields,name+" :: "+fieldType);delete(required,name)};if len(required)>0{return "",&Error{Code:"native.projection",Format:p.format,Pointer:path+"/required",Message:"required contains a property without a projectable declared schema"}};result="{"+strings.Join(fields,", ")+"}"
...
case "object":typeNode,ok:=node.Lookup("type");if !ok{return "",&Error{Code:"native.projection",Format:Avro,Message:"Avro schema object requires type"}};if schemajson.KindName(typeNode.Kind())!="string"{return p.avroType(typeNode,hint)};nativeType,_:=nodeString(typeNode);switch nativeType{
    ...
    case "map":return "",&Error{Code:"native.projection",Format:Avro,Message:"Avro maps need an explicit language map representation"}
