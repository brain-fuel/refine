// Code generated by goplus from lower_numbers.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func loweringNumberBounded(raw string) bool {
	if len(raw) > DefaultNumericExpansion {
		return false
	}
	...
	for ; index < len(raw); index++ {
		...
		if magnitude > (DefaultNumericExpansion-digit)/10 {
			return false
		}
		magnitude = magnitude*10 + digit
	}
	return magnitude <= DefaultNumericExpansion
}
func loweringNumberLiteral(expr *language.Expr) (string, bool) {
	raw, ok := numberLiteral(expr)
	return raw, ok && loweringNumberBounded(raw)
}
