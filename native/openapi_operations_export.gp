...
func (p *Project)exportOpenAPIOperations(options LowerOptions,mode ExportMode)(*ProjectExport,error){
    ...
    resources:=p.Resources();if len(p.nativeUnitSources)>0{resources,err=p.CanonicalJSONResources();if err!=nil{return nil,err}};entryIndex:=-1;for i,resource:=range resources{if resource.URI==p.EntryResource(){entryIndex=i;break}};if entryIndex<0{return nil,&Error{Code:"native.resource",Format:OpenAPI,Pointer:p.EntryResource(),Message:"OpenAPI entry resource is absent"}}
