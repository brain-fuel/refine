...
func (p *Project) ValidateJSON(input []byte)(failure error){defer recoverRegexEvaluation(p.Format(),&failure);if p==nil||p.document==nil{return &Error{Code:"native.project",Message:"a project is required"}};if p.Format()==OpenAPI{return p.validateOpenAPIJSON(input)};if p.Format()!=JSONSchema{return &Error{Code:"native.enforcement",Format:p.Format(),Message:"JSON payload validation is not implemented for this format"}}
