...
func (p *Project) WithEditedNativeConstraintSource(resource,source string)(*Project,error){if p==nil||p.jsonOrigins[resource]==nil{return nil,&Error{Code:"native.provenance",Format:p.Format(),Pointer:resource,Message:"resource has no JSON Schema provenance"}};if _,editable:=p.nativeUnitSources[resource];!editable{return nil,&Error{Code:"native.provenance",Format:p.Format(),Pointer:resource,Message:"resource has no editable native constraint units"}};if _,err:=p.jsonOrigins[resource].AuditSource(source);err!=nil{return nil,wrap(p.Format(),"native.provenance",resource,err)};copy:=*p;copy.nativeUnitSources=copyStringMap(p.nativeUnitSources);copy.nativeUnitsInEditable=copyBoolMap(p.nativeUnitsInEditable);copy.nativeUnitSources[resource]=source;copy.nativeUnitsInEditable[resource]=false;copy.resources=append([]Resource(nil),p.resources...);copy.metadata=copyMetadata(p.metadata);return &copy,nil}
...
func nativeConstraintUnitsUnchanged(origin *provenance.JSONSchema,source string)bool{if origin==nil{return false};findings,err:=origin.AuditSource(source);if err!=nil||len(findings)==0{return false};for _,finding:=range findings{if provenance.StatusName(finding.Status)!="unchanged"{return false}};return true}
...
func (p *Project) ValidateJSON(input []byte)(failure error){defer recoverRegexEvaluation(p.Format(),&failure);if p==nil||p.document==nil{return &Error{Code:"native.project",Message:"a project is required"}};if err:=p.requirePayloadRoot();err!=nil{return err};if p.Format()==OpenAPI{return p.validateOpenAPIJSON(input)};if p.Format()!=JSONSchema{return &Error{Code:"native.enforcement",Format:p.Format(),Message:"JSON payload validation is not implemented for this format"}}
    ...
    compiler:=jsonoracle.NewCompiler();compiler.DefaultDraft(jsonoracle.Draft2020);compiler.UseRegexpEngine(jsonRegexp);resources,err:=p.CanonicalJSONResources();if err!=nil{return err};for _,resource:=range resources{schemaValue,err:=jsonoracle.UnmarshalJSON(strings.NewReader(resource.Source));if err!=nil{return wrap(JSONSchema,"native.structure",resource.URI,err)};if err:=compiler.AddResource(resource.URI,schemaValue);err!=nil{return wrap(JSONSchema,"native.structure",resource.URI,err)}};location:=p.root.Resource;if p.root.Pointer!=""{location+="#"+p.root.Pointer};schema,err:=compiler.Compile(location);if err!=nil{return wrap(JSONSchema,"native.structure",p.root.Pointer,err)};if err:=schema.Validate(value);err!=nil{return wrap(JSONSchema,"native.payload",p.root.Pointer,err)};return nil
}
