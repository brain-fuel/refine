...
func validateProject(p *Project)(*Project,error){if p==nil||p.program==nil{return nil,&Error{Code:"native.project",Message:"a checked project is required"}};p.effectiveResources=nil;if err:=normalizeAndValidateProjectTarget(p);err!=nil{return nil,err};root:="";if p.HasPayloadRoot(){root=p.root.TypeName};report,err:=analysis.CheckSchemaRoots(p.program,SchemaEntrypoints(root,p.metadata),validation.Limits{});p.schemaChecks=copySchemaReport(report);if err!=nil{var proof *analysis.SchemaError;if errors.As(err,&proof){return nil,&Error{Code:"native.schema",Format:p.Format(),Pointer:proof.Type,Message:proof.Error(),Cause:err}};return nil,wrap(p.Format(),"native.schema","",err)};if err:=validateEffectiveConstraintUnits(p);err!=nil{return nil,err};checked,err:=validateAvroRefinementDefaults(p);if err!=nil{return nil,err};return validateOpenAPINativeBindings(checked)}
...
func validateEffectiveConstraintUnits(p *Project)error{
    if p.Format()==Avro{resources,err:=p.effectiveAvroResources(p.resources);if err!=nil{return err};if _,_,err=validateAvroResources(resources,p.root);err!=nil{return err};p.effectiveResources=append([]Resource(nil),resources...);return nil}
    if len(p.nativeUnitSources)==0{return nil};if p.Format()!=JSONSchema&&p.Format()!=OpenAPI{return nil};if p.Format()==OpenAPI&&p.metadata.OpenAPI!=nil&&p.metadata.OpenAPI.Native!=nil{return nil};_,err:=p.CanonicalJSONResources();return err
}
