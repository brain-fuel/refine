...
func TestOpenAPISchemaObjectAnnotationAuditFailsClosed(t *testing.T){
    ...
    if _,err:=IngestProject(OpenAPI,[]byte(rebased),ProjectOptions{ResourceID:"https://example.test/rebased.json",Root:ResourceSelector{Pointer:"/components/schemas/Root",TypeName:"Root"}});problemCode(err)!="native.projection"||!strings.Contains(err.Error(),"JSON Pointer does not identify a schema position"){t.Fatalf("$id scope was resolved as a physical URI: %v",err)}
}
