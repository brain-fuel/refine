// Code generated by goplus from openapi_oracle_view_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func (l oracleViewLoader) Load(uri string) (any, error) {
	source, ok := l.values[uri]
	if !ok {
		return nil, fmt.Errorf("unlisted oracle resource %s", uri)
	}
...
func oracleViewCompiler(t *testing.T, view *openAPIOracleView) *jsonoracle.Compiler {
	...
	loader := oracleViewLoader{values: map[string]string{openAPIBaseDialect: openAPIBaseDialectAdapter}}
	for _, alias := range view.aliases {
		loader.values[alias.URI] = alias.Source
	}
	compiler.UseLoader(loader)
	for _, resource := range view.resources {
		parsed, err := jsonoracle.UnmarshalJSON(strings.NewReader(resource.Source))
		if err != nil {
			t.Fatal(err)
		}
		if err := compiler.AddResource(resource.URI, parsed); err != nil {
			t.Fatal(err)
		}
	}
	return compiler
...
func assertOracleViewPayload(t *testing.T, compiler *jsonoracle.Compiler, view *openAPIOracleView, selector ResourceSelector, valid, invalid string) {
	t.Helper()
	mapped, err := view.selector(selector)
	if err != nil {
		t.Fatal(err)
	}
	...
	assertOracleLocationPayload(t, compiler, identity.String(), valid, invalid)
}
func assertOracleLocationPayload(t *testing.T, compiler *jsonoracle.Compiler, location, valid, invalid string) {
	...
	for _, test := range []struct {
		...
		value, err := jsonoracle.UnmarshalJSON(strings.NewReader(test.source))
		if err != nil {
			t.Fatal(err)
		}
		if err := compiled.Validate(value); (err == nil) != test.valid {
			t.Fatalf("validation of %s: %v (want valid=%v)", test.source, err, test.valid)
		}
	}
}
...
func TestOpenAPIOracleViewPreservesPhysicalAndLogicalScope(t *testing.T) {
	...
	view, err := newOpenAPIOracleView(catalog)
	if err != nil {
		t.Fatal(err)
	}
	compiler := oracleViewCompiler(t, view)
	assertOracleViewPayload(t, compiler, view, ResourceSelector{Resource: uri, Pointer: "/components/schemas/Root"}, `{"value":"ok"}`, `{"value":"x"}`)
	for _, name := range []string{"Scoped", "Pointer"} {
		assertOracleViewPayload(t, compiler, view, ResourceSelector{Resource: uri, Pointer: "/components/schemas/" + name}, "2", "1")
	}
	assertOracleLocationPayload(t, oracleViewCompiler(t, view), "https://schemas.test/value#n", "2", "1")
	if resources[0].Source != source || catalog.documents[uri].Raw() != source {
		t.Fatal("oracle materialization mutated exact source")
	}
	reversed, err := newOpenAPIProjectionCatalog([]Resource{resources[1], resources[0]}, uri)
	if err != nil {
		t.Fatal(err)
	}
	again, err := newOpenAPIOracleView(reversed)
	if err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(view, again) {
		t.Fatal("input order changed oracle view")
	}
	root, err := view.selector(ResourceSelector{Resource: uri, Pointer: "/components/schemas/Root", TypeName: "Root"})
	if err != nil || root.TypeName != "Root" {
		t.Fatal("selector lost its type name")
	}
	for _, resource := range view.resources {
		if resource.URI != uri {
			continue
		...
		node, err := document.At(root.Pointer)
		if err != nil {
			t.Fatal(err)
		}
		if _, present := node.Lookup("$id"); present {
			t.Fatal("no-ID root acquired a new resource boundary")
		}
		examples, _ := node.Lookup("examples")
		if examples.Raw() != `[{"$ref":"this is data","$id":"not a schema"}]` {
			t.Fatal("example was rewritten as schema")
		}
	}
	if _, err := view.selector(ResourceSelector{Resource: uri, Pointer: "/info"}); err == nil {
		t.Fatal("non-schema selector accepted")
	}
	if _, err := compiler.Compile("https://unlisted.test/schema"); err == nil {
		t.Fatal("unlisted offline resource accepted")
	}
}
...
func TestOpenAPIOracleViewChargesAggregateAliasesAndOutput(t *testing.T) {
	uri := "https://example.test/api.json"
	source := `{"openapi":"3.1.2","info":{"title":"Bounded","version":"1"},"paths":{},"components":{"schemas":{"Root":{"$id":"https://schemas.test/root","type":"string","description":"` + strings.Repeat("a", 1000) + `"}}}}`
	catalog, err := newOpenAPIProjectionCatalog([]Resource{{URI: uri, Source: source}}, uri)
	if err != nil {
		t.Fatal(err)
	}
	...
	total := 0
	for _, resource := range append(append([]Resource(nil), view.resources...), view.aliases...) {
		total += len(resource.Source)
	}
	if _, err := buildOpenAPIOracleView(catalog, total); err != nil {
		t.Fatal(err)
	}
	if partial, err := buildOpenAPIOracleView(catalog, total-1); problemCode(err) != "native.limit" || partial != nil {
		t.Fatalf("aggregate bound returned partial output: %v %v", partial, err)
	}
}

func TestOpenAPIOracleViewRejectsOverlappingDataInterpretations(t *testing.T) {
	uri := "https://example.test/api.json"
	fragment := "https://example.test/parts.json"
	for _, pointer := range []string{"/Root/foo", "/Root/data/Target"} {
		...
		catalog, err := newOpenAPIProjectionCatalog([]Resource{{URI: uri, Source: source}, {URI: fragment, Source: parts}}, uri)
		if err != nil {
			t.Fatal(err)
		}
		if view, err := newOpenAPIOracleView(catalog); err == nil || view != nil || !strings.Contains(err.Error(), "overlap opaque data") {
			t.Fatalf("overlapping interpretation %s did not fail closed: %v", pointer, err)
		}
	}
}
...
func TestOpenAPIOracleViewQuotesBeforeAllocatingEscapes(t *testing.T) {
	for _, text := range []string{"plain", "\"\\\b\f\n\r\t\x00<>&\u2028\u2029é😀", string([]byte{0xff, 0x80})} {
		...
		writer := &openAPIOracleWriter{limit: len(expected)}
		if err := writer.quote(&out, text); err != nil || out.String() != string(expected) || writer.bytes != len(expected) {
			t.Fatalf("quote size mismatch: %v", err)
		}
		var blocked strings.Builder
		writer = &openAPIOracleWriter{limit: len(expected) - 1}
		if err := writer.quote(&blocked, text); problemCode(err) != "native.limit" || blocked.Len() != 0 || writer.bytes != 0 {
			t.Fatalf("quote allocated output before admitting its escaped size: %v", err)
		}
	}
}
...
func TestOpenAPIOracleViewRetainsFragmentDialects(t *testing.T) {
	...
	for _, resource := range view.resources {
		...
		node, _ := document.Root().Lookup("$schema")
		if actual, _ := nodeString(node); actual != draft {
			t.Fatalf("fragment lost inherited dialect: %s", actual)
		}
	}
	...
	if view, err := newOpenAPIOracleView(catalog); view != nil || err == nil || !strings.Contains(err.Error(), "incompatible inherited schema dialects") {
		t.Fatalf("mixed scopes were silently collapsed: %v", err)
	}
}
