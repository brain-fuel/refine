// Code generated by goplus from analysis.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func Satisfiable(program *language.Program, root string, limits validation.Limits) (Finding, error) {
	...
	if domain.empty {
		return Finding{No, "analysis.empty", "The recognized numeric constraints have an empty intersection; additional predicates cannot restore a value."}, nil
	}
	for _, candidate := range candidates(domain) {
		if validation.StateName(target.ValidateData(value.OfNumber(candidate), limits).State()) == "valid" {
...
func analysisExpansionLimit(program *language.Program, limits validation.Limits) uint64 {
	...
	total, clause := declared.Total, declared.Clause
	if total == 0 {
		total = uint64(validation.DefaultTotalSteps)
	}
	if clause == 0 {
		clause = uint64(validation.DefaultClauseSteps)
	}
...
func (i *inspector) typ(t *language.Type, depth int) interval {
	...
	case language.NamedType:
		...
		if name == "Int" || name == "Real" {
			return interval{integer: name == "Int", numeric: true, exact: true}
		}
		if decl, ok := i.types[name]; ok {
			if i.active[name] || decl.Body == nil || len(decl.Parameters) > 0 {
				return interval{}
			}
			i.active[name] = true
			result := i.typ(decl.Body, depth+1)
			delete(i.active, name)
			return result
		}
		signed := true
		digits := ""
		...
		if err != nil || width <= 0 || width > 65536 {
			return interval{}
		}
		upper := new(big.Int).Lsh(big.NewInt(1), uint(width))
		lower := new(big.Int)
		if signed {
			upper.Rsh(upper, 1)
			lower.Neg(upper)
		...
		lo, _ := value.ParseNumber(lower.String())
		hi, _ := value.ParseNumber(upper.String())
...
func current(e *language.Expr) bool {
	switch __gp_m1 := any(e.Form).(type) {
...
func (i *inspector) predicate(d *interval, e *language.Expr) {
	...
	case language.Binary:
		...
		switch op {
		case "<", "<=":
			d.tightenUpper(bound{value: number, strict: op == "<"})
		case ">", ">=":
			d.tightenLower(bound{value: number, strict: op == ">"})
		case "==":
			d.tightenLower(bound{value: number})
			d.tightenUpper(bound{value: number})
		default:
			d.exact = false
		}
	default:
...
func (d *interval) tightenLower(b bound) {
	if d.lower == nil {
		d.lower = &b
		return
	}
	cmp := b.value.Compare(d.lower.value)
	if cmp > 0 || cmp == 0 && b.strict {
		d.lower = &b
	}
}
func (d *interval) tightenUpper(b bound) {
	if d.upper == nil {
		d.upper = &b
		return
	}
	cmp := b.value.Compare(d.upper.value)
	if cmp < 0 || cmp == 0 && b.strict {
		d.upper = &b
	}
}
func (d *interval) normalize() {
	if d.integer {
		if d.lower != nil && d.lower.value.IsInteger() && d.lower.strict {
			d.lower = &bound{value: d.lower.value.Add(value.Integer(1))}
		}
		if d.upper != nil && d.upper.value.IsInteger() && d.upper.strict {
			d.upper = &bound{value: d.upper.value.Subtract(value.Integer(1))}
		}
	}
	if d.lower != nil && d.upper != nil {
		cmp := d.lower.value.Compare(d.upper.value)
		if cmp > 0 || cmp == 0 && (d.lower.strict || d.upper.strict) {
			d.empty = true
		}
	}
}
func within(d interval, n value.Number) bool {
	if !d.numeric || d.empty || d.integer && !n.IsInteger() {
		return false
	}
	if d.lower != nil {
		cmp := n.Compare(d.lower.value)
		if cmp < 0 || cmp == 0 && d.lower.strict {
			return false
		}
	}
	if d.upper != nil {
		cmp := n.Compare(d.upper.value)
		if cmp > 0 || cmp == 0 && d.upper.strict {
			return false
		}
	}
	return true
...
func candidates(d interval) []value.Number {
	if !d.numeric || d.empty {
		return nil
	}
	raw := []value.Number{value.Integer(0), value.Integer(1), value.Integer(-1)}
	half, _ := value.ParseNumber("1/2")
	if !d.integer {
		raw = append(raw, half, half.Negate())
	}
	for _, b := range []*bound{d.lower, d.upper} {
		if b != nil {
			raw = append(raw, b.value, b.value.Add(value.Integer(1)), b.value.Subtract(value.Integer(1)))
			if !d.integer {
				raw = append(raw, b.value.Add(half), b.value.Subtract(half))
			}
		}
	}
	if d.lower != nil && d.upper != nil {
		mid, _ := d.lower.value.Add(d.upper.value).Divide(value.Integer(2))
		raw = append(raw, mid)
	}
	seen := map[string]bool{}
	out := []value.Number{}
	for _, n := range raw {
		if !seen[n.Show()] && within(d, n) {
			seen[n.Show()] = true
			out = append(out, n)
		}
	}
...
func subset(a, b interval) bool {
	...
	if !a.numeric || !b.numeric || !b.exact || b.empty {
		return false
	}
	if b.integer && !a.integer {
		if a.lower == nil || a.upper == nil || a.lower.strict || a.upper.strict || a.lower.value.Compare(a.upper.value) != 0 || !a.lower.value.IsInteger() {
			return false
		}
	}
	if b.lower != nil {
		...
		cmp := a.lower.value.Compare(b.lower.value)
		if cmp < 0 || cmp == 0 && b.lower.strict && !a.lower.strict {
			return false
		}
	}
	if b.upper != nil {
		...
		cmp := a.upper.value.Compare(b.upper.value)
		if cmp > 0 || cmp == 0 && b.upper.strict && !a.upper.strict {
			return false
		}
	}
	return true
...
func includes(from *language.PayloadType, a interval, to *language.PayloadType, b interval, same bool, limits validation.Limits) Finding {
	if same {
		return Finding{Yes, "analysis.identical", "The checked source and selected target are identical."}
	}
	if subset(a, b) {
		return Finding{Yes, "analysis.inclusion", "The source's recognized numeric superset lies entirely in the target's fully recognized numeric interval. This is logical payload inclusion, not a native-wire or Java-ABI guarantee."}
	}
	...
	for _, n := range points {
		...
		if validation.StateName(from.ValidateData(data, limits).State()) != "valid" {
			continue
		}
