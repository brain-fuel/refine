...
type interval struct { lower *bound; upper *bound; integer bool; numeric bool; exact bool; empty bool }
type inspector struct { types map[string]language.TypeDecl; active map[string]bool }
...
func Satisfiable(program *language.Program,root string,limits validation.Limits)(Finding,error){
    target,domain,err:=inspect(program,root);if err!=nil{return Finding{},err}
...
func Compare(oldProgram *language.Program,oldRoot string,newProgram *language.Program,newRoot string,limits validation.Limits)(Compatibility,error){
    oldTarget,oldDomain,err:=inspect(oldProgram,oldRoot);if err!=nil{return Compatibility{},err}
    newTarget,newDomain,err:=inspect(newProgram,newRoot);if err!=nil{return Compatibility{},err}
...
func (i *inspector) typ(t *language.Type,depth int)interval{
    ...
    case language.RefinedType(base,rules):
        ...
        for _,rule:=range rules{domain.predicate(rule.Predicate)}
...
func literal(e *language.Expr)(value.Number,bool){
    ...
    case language.NumberLiteral(text):
        ...
        if len(text)>10000{return value.Number{},false}
        if pos:=strings.IndexAny(text,"eE");pos>=0{exp,err:=strconv.Atoi(text[pos+1:]);if err!=nil||exp>10000||exp< -10000{return value.Number{},false}}
        n,err:=value.ParseNumber(text);return n,err==nil
    case language.Unary(op,arg):if op!="-"{return value.Number{},false};n,ok:=literal(arg);return n.Negate(),ok
    case _:return value.Number{},false
    }
}
...
func (d *interval) predicate(e *language.Expr){
    ...
    case language.Binary(op,left,right):
        if op=="&&"{d.predicate(left);d.predicate(right);return}
        number,ok:=literal(right)
        if !current(left)||!ok{
            number,ok=literal(left);if !current(right)||!ok{d.exact=false;return}
