// Code generated by goplus from change.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func CompareContractSyntax(baseline *language.Program, baselineRoot string, candidate *language.Program, candidateRoot string) (ContractSyntaxEvidence, error) {
	if baseline == nil || candidate == nil {
		return ContractSyntaxEvidence{}, fmt.Errorf("analysis: two checked programs are required")
	}
	if _, err := baseline.PayloadType(baselineRoot); err != nil {
		return ContractSyntaxEvidence{}, err
	}
	if _, err := candidate.PayloadType(candidateRoot); err != nil {
		return ContractSyntaxEvidence{}, err
	}
	...
	for _, decl := range next.Types {
		nextTypes[decl.Name] = language.Format(&language.Module{Types: []language.TypeDecl{decl}})
	}
	oldFunctions, nextFunctions := map[string]string{}, map[string]string{}
	for _, fn := range old.Functions {
		oldFunctions[fn.Name] = language.Format(&language.Module{Functions: []language.Function{fn}})
	}
	for _, fn := range next.Functions {
		nextFunctions[fn.Name] = language.Format(&language.Module{Functions: []language.Function{fn}})
	}
	result.Differences = append(result.Differences, namedSyntaxDifferences("type", oldTypes, nextTypes)...)
	result.Differences = append(result.Differences, namedSyntaxDifferences("function", oldFunctions, nextFunctions)...)
	if !sameTypeOrder(old.Types, next.Types) {
		result.Differences = append(result.Differences, SyntaxDifference{Kind: "type-order"})
	}
	if !sameFunctionOrder(old.Functions, next.Functions) {
		result.Differences = append(result.Differences, SyntaxDifference{Kind: "function-order"})
	}
	return result, nil
}

func contractSyntaxDigest(root, canonical string) string {
	sum := sha256.Sum256([]byte(ContractSyntaxVersion + "\x00" + root + "\x00" + canonical))
	return hex.EncodeToString(sum[:])
}
func sameSyntaxImports(a, b []language.Import) bool {
	if len(a) != len(b) {
		return false
	}
	for i, item := range a {
		if item.Path != b[i].Path {
			return false
		}
	}
	return true
}
func sameTypeOrder(a, b []language.TypeDecl) bool {
	if len(a) != len(b) {
		return false
	}
	for i, item := range a {
...
func sameFunctionOrder(a, b []language.Function) bool {
	if len(a) != len(b) {
		return false
	}
	for i, item := range a {
...
func namedSyntaxDifferences(kind string, a, b map[string]string) []SyntaxDifference {
	...
	result := []SyntaxDifference{}
	for _, name := range ordered {
		old, oldExists := a[name]
		next, nextExists := b[name]
		if oldExists && nextExists && old == next {
			continue
		}
		change := "changed"
		if !oldExists {
		...
		result = append(result, SyntaxDifference{Kind: kind + "-" + change, Name: name})
	}
	return result
}
