...
func (e *SchemaError) Error() string {
	return "schema.unsatisfiable: " + e.Type + ": " + e.Finding.Explanation
}
...
func CheckSchema(program *language.Program, limits validation.Limits) (SchemaReport, error) {
	if program == nil {
		return SchemaReport{Roots: []string{}, Findings: []SchemaFinding{}}, fmt.Errorf("schema.program: a checked program is required")
	}
	roots := []string{}
	...
	return checkSchemaRoots(program, roots, limits, false)
}
...
func CheckSchemaRoots(program *language.Program, roots []string, limits validation.Limits) (SchemaReport, error) {
	return checkSchemaRoots(program, roots, limits, true)
}
...
func checkSchemaRoots(program *language.Program, roots []string, limits validation.Limits, requireRoots bool) (SchemaReport, error) {
	...
	if program == nil {
		return report, fmt.Errorf("schema.program: a checked program is required")
	}
	...
	if requireRoots && len(report.Roots) == 0 {
		return report, fmt.Errorf("schema.root: at least one closed entrypoint is required")
	}
	for _, root := range report.Roots {
		decl, ok := declarations[root]
		if !ok {
			return report, fmt.Errorf("schema.root: %s is not a declared type", root)
		}
		if len(decl.Parameters) > 0 {
			return report, fmt.Errorf("schema.root: %s is not a closed declaration", root)
		}
	}
	var rejected error
	for _, decl := range program.Syntax().Types {
		...
		if len(decl.Parameters) == 0 {
			var err error
			finding, err = Satisfiable(program, decl.Name, limits)
			if err != nil {
				return report, err
			}
		}
		report.Findings = append(report.Findings, SchemaFinding{Type: decl.Name, Finding: finding})
		if selected[decl.Name] && finding.Outcome == No && rejected == nil {
			rejected = &SchemaError{Type: decl.Name, Finding: finding}
		}
	}
